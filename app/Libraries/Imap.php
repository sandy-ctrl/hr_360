...
class Imap {
    ...
    private function _create_ticket_from_imap($message_info = "") {
        if ($message_info) {
            ...
            if ($ticket_id) {
                $existing_ticket_info = $this->ci->Tokens_model->get_one_where(array("id" => $ticket_id, "deleted" => 0));
            ...
            if ($ticket_id) {
                ...
                $ticket_id = $this->ci->Tokens_model->ci_save($ticket_data);
    ...
    private function _save_tickets_comment($ticket_id, $message_info, $client_info, $is_reply = false) {
        if ($ticket_id) {
            ...
            if ($is_reply) {
                ...
                $this->ci->Tokens_model->ci_save($ticket_data, $ticket_id);
            }

            $ticket_comment_id = $this->ci->Token_comments_model->ci_save($comment_data);
