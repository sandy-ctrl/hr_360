...
trait App_folders
{
    ...
    private function init_permissions_value_memory()
    {
        ...
        if (is_null($this->permissions_value_memory)) {
            ...
            foreach ($this->Unit_model->get_id_and_title()->getResult() as $team) {
                $team_list[$team->id] = $team->title;
            }

            foreach ($this->Patrons_model->get_clients_id_and_name(array("limit" => 2000))->getResult() as $client) {
    ...
    private function _get_permission_options()
    {
        ...
        if (in_array("team", $shareable_options)) {
            $teams = $this->Unit_model->get_id_and_title()->getResult();
        ...
        if (in_array("client", $shareable_options)) {
            ...
            $clients = $this->Patrons_model->get_clients_id_and_name($client_options)->getResult();
    ...
    private function _preapare_folder_params($client_id = 0)
    {
        ...
        if ($this->login_user->user_type == "staff") {
            ...
            $client_info = $this->Patrons_model->get_one($this->login_user->client_id);
    ...
    function get_file_modal_form()
    {
        ...
        return $this->template->view('files_directory/file_modal_form', $view_data);
    }
    ...
    function get_view_file($file_id = 0, $client_id = 0)
    {
        ...
        if ($file_info) {
            ...
            return $this->template->view("files_directory/view_file", $view_data);
        } else {
            show_404();
        }
    }
