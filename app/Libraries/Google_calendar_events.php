...
class Google_calendar_events {
    ...
    public function save_event($user_id = 0, $id = 0) {
        if ($user_id && $id && get_setting("enable_google_calendar_api") && get_setting("module_event")) {
            $event_info = $this->ci->Schedules_model->get_one($id);
            ...
            if ($event_info) {
                ...
                if (isset($calendar_event_info->id) || isset($calendar_event_info->recurringEventId)) {
                    ...
                    $this->ci->Schedules_model->ci_save($data, $id);
                }
            }
        }
    }
    ...
    private function _get_share_with_emails($event_info = "") {
        if ($event_info && $event_info->share_with) {
            ...
            $users = $this->ci->Schedules_model->get_share_with_users_of_event($event_info)->getResult();
    ...
    public function get_google_calendar_events() {
        if (get_setting("enable_google_calendar_api") && get_setting("module_event")) {
            $enabled_users_settings = $this->ci->Schedules_model->get_integrated_users_with_google_calendar()->getResult();
    ...
    private function _delete_calendar_events($user_id = 0, $google_event_id = "") {
        $existing_event = $this->ci->Schedules_model->get_one_where(array("google_event_id" => $google_event_id, "created_by" => $user_id));
        if ($existing_event->id) {
            $this->ci->Schedules_model->delete($existing_event->id);
        }
    }
    ...
    private function _save_calendar_events($data = array(), $google_event_id = "") {
        ...
        $existing_event = $this->ci->Schedules_model->get_one_where(array("google_event_id" => $google_event_id, "created_by" => get_array_value($data, "created_by")));
        $this->ci->Schedules_model->ci_save($data, $existing_event->id);
    }
