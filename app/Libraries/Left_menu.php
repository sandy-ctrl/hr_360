...
class Left_menu {
    ...
    private function _get_sidebar_menu_items($type = "") {
        $dashboard_menu = array("name" => "dashboard", "url" => "dashboard", "class" => "airplay");
        ...
        if ($this->ci->login_user->user_type == "staff" && $type !== "client_default") {
            ...
            $access_client = get_array_value($permissions, "patrons");
            ...
            if (get_setting("module_event") == "1") {
                $sidebar_menu["events"] = array("name" => "events", "url" => "schedules", "class" => "calendar");
            }


            if ($this->ci->login_user->is_admin || $access_client) {
                $sidebar_menu["clients"] = array("name" => "clients", "url" => "patrons", "class" => "briefcase");
            }


            if ($this->ci->login_user->is_admin || !get_array_value($this->ci->login_user->permissions, "do_not_show_projects")) {
                $sidebar_menu["projects"] = array("name" => "projects", "url" => "enterprises", "class" => "command");
            }

            $sidebar_menu["tasks"] = array("name" => "tasks", "url" => "responsibility/all_tasks", "class" => "file-text");

            if (get_setting("module_lead") == "1" && ($this->ci->login_user->is_admin || $access_lead)) {
                $sidebar_menu["leads"] = array("name" => "leads", "url" => "initiates", "class" => "layers");
            }

            if (get_setting("module_subscription") && ($this->ci->login_user->is_admin || $access_subscription)) {
                $sidebar_menu["subscriptions"] = array("name" => "subscriptions", "url" => "membership", "class" => "repeat");
            }
            ...
            if (get_setting("module_invoice") == "1" && ($this->ci->login_user->is_admin || $access_invoice)) {
                $sales_submenu[] = array("name" => "invoices", "url" => "bills", "class" => "file-text");
            }

            if (get_setting("module_order") == "1" && ($this->ci->login_user->is_admin || $access_order)) {
                $sales_submenu[] = array("name" => "orders_list", "url" => "arrangements", "class" => "truck");
                $sales_submenu[] = array("name" => "store", "url" => "outlet", "class" => "list");
            }

            if (get_setting("module_invoice") == "1" && ($this->ci->login_user->is_admin || $access_invoice)) {
                $sales_submenu[] = array("name" => "invoice_payments", "url" => "bill_payments", "class" => "dollar-sign");
                $show_payments_menu = true;
            }

            if ($access_items && (get_setting("module_invoice") == "1" || get_setting("module_estimate") == "1" )) {
                $sales_submenu[] = array("name" => "items", "url" => "elements", "class" => "list");
            }

            if (get_setting("module_contract") && ($this->ci->login_user->is_admin || $access_contract)) {
                $sales_submenu[] = array("name" => "contracts", "url" => "agreements", "class" => "book-open");
            }
            ...
            if (get_setting("module_estimate") && ($this->ci->login_user->is_admin || $access_estimate)) {
                ...
                $prospects_submenu["estimates"] = array("name" => "estimate_list", "url" => "evaluations", "class" => "file");

                if (get_setting("module_estimate_request")) {
                    $prospects_submenu["estimate_requests"] = array("name" => "estimate_requests", "url" => "evaluation_requests", "class" => "file");
                    $prospects_submenu["estimate_forms"] = array("name" => "estimate_forms", "url" => "evaluation_requests/estimate_forms", "class" => "file");
                }
            }

            if (get_setting("module_proposal") && ($this->ci->login_user->is_admin || $access_proposal)) {
                $prospects_submenu["proposals"] = array("name" => "proposals", "url" => "plans", "class" => "coffee");
            }

            if (count($prospects_submenu)) {
                $sidebar_menu["prospects"] = array("name" => "prospects", "url" => "evaluations", "class" => "anchor", "submenu" => $prospects_submenu);
            }



            if (get_setting("module_note") == "1") {
                $sidebar_menu["notes"] = array("name" => "notes", "url" => "notations", "class" => "book");
            }

            if (get_setting("module_message") == "1" && $access_messages) {
                $sidebar_menu["messages"] = array("name" => "messages", "url" => "memos", "class" => "message-square", "badge" => count_unread_message(), "badge_class" => "bg-primary");
            }
            ...
            if (get_array_value($this->ci->login_user->permissions, "hide_team_members_list") != "1") {
                $team_submenu["team_members"] = array("name" => "team_members", "url" => "unit_members", "class" => "users");
            }


            if (get_setting("module_attendance") == "1" && ($this->ci->login_user->is_admin || $access_timecard)) {
                $team_submenu["attendance"] = array("name" => "attendance", "url" => "timesheet", "class" => "clock");
            } else if (get_setting("module_attendance") == "1") {
                $team_submenu["attendance"] = array("name" => "attendance", "url" => "timesheet/attendance_info", "class" => "clock");
            }


            if (get_setting("module_leave") == "1" && ($this->ci->login_user->is_admin || $access_leave)) {
                $team_submenu["leaves"] = array("name" => "leaves", "url" => "vacates", "class" => "log-out");
            } else if (get_setting("module_leave") == "1") {
                $team_submenu["leaves"] = array("name" => "leaves", "url" => "vacates/leave_info", "class" => "log-out");
            }



            if (get_setting("module_timeline") == "1" && $access_timeline) {
                $team_submenu["timeline"] = array("name" => "timeline", "url" => "agenda", "class" => "send");
            }


            if (get_setting("module_announcement") == "1") {
                $team_submenu["announcements"] = array("name" => "announcements", "url" => "notices", "class" => "bell");
            }


            if (count($team_submenu)) {
                $sidebar_menu["team"] = array("name" => "team", "url" => "unit_members", "class" => "users", "submenu" => $team_submenu);
            }
            ...
            if (get_setting("module_ticket") == "1" && ($this->ci->login_user->is_admin || $access_ticket)) {
                ...
                $sidebar_menu["tickets"] = array("name" => "tickets", "url" => "tokens", "class" => "life-buoy", "badge" => $ticket_badge, "badge_class" => "bg-primary");
            }


            if (get_setting("module_expense") == "1" && ($this->ci->login_user->is_admin || $access_expense)) {
                $sidebar_menu["expenses"] = array("name" => "expenses", "url" => "budgets", "class" => "arrow-right-circle");
                $show_expenses_menu = true;
            }

            $sidebar_menu["reports"] = array("name" => "reports", "url" => "outlines/index", "class" => "pie-chart",
                "sub_pages" => array(
                    "invoices/invoices_summary",
                    "orders/orders_summary",
                    "enterprises/all_timesheets",
                    "budgets/income_vs_expenses",
                    "invoice_payments/payments_summary",
                    "budgets/summary",
                    "enterprises/team_members_summary",
                    "initiates/converted_to_client_report",
                    "token/tickets_chart_report"
                )
            );
            ...
            if (get_setting("module_file_manager") == "1" && ($this->ci->login_user->is_admin || $access_file_manager)) {
                $sidebar_menu["file_manager"] = array("name" => "files", "url" => "files_directory", "class" => "folder");
                $show_expenses_menu = true;
            }
            
            
            $module_help = get_setting("module_help") == "1" ? false : false;
            ...
            if ($module_help || $module_knowledge_base) {
                ...
                if ($manage_help_and_knowledge_base && $module_help) {
                    $help_knowledge_base_menues[] = array("name" => "articles", "url" => "support/help_articles", "class" => "help-circle");
                    $help_knowledge_base_menues[] = array("name" => "categories", "url" => "support/help_categories", "class" => "help-circle");
                }

                if ($module_knowledge_base) {
                    $help_knowledge_base_menues[] = array("name" => "knowledge_base", "url" => "information_hub", "class" => "help-circle");
                }

                //push the knowledge_base manage menu if user has access
                if ($manage_help_and_knowledge_base && $module_knowledge_base) {
                    $help_knowledge_base_menues[] = array("name" => "articles", "category" => "help", "url" => "support/knowledge_base_articles", "class" => "help-circle");
                    $help_knowledge_base_menues[] = array("name" => "categories", "category" => "help", "url" => "support/knowledge_base_categories", "class" => "help-circle");
                }
            ...
            if ($this->ci->can_client_access("event")) {
                $sidebar_menu[] = array("name" => "events", "url" => "schedules", "class" => "calendar");
            }

            if ($this->ci->can_client_access("note") && get_setting("client_can_access_notes")) {
                $sidebar_menu[] = array("name" => "notes", "url" => "notations", "class" => "book");
            }

            //check message access settings for clients
            if ($this->ci->can_client_access("message") && get_setting("client_message_users")) {
                $sidebar_menu[] = array("name" => "messages", "url" => "memos", "class" => "message-circle", "badge" => count_unread_message());
            }

            if ($this->ci->can_client_access("project", false)) {
                $sidebar_menu[] = array("name" => "projects", "url" => "enterprises/all_projects", "class" => "command");
            }

            if ($this->ci->can_client_access("contract")) {
                $sidebar_menu[] = array("name" => "contracts", "url" => "agreements", "class" => "book-open");
            }

            if ($this->ci->can_client_access("proposal")) {
                $sidebar_menu[] = array("name" => "proposals", "url" => "plans", "class" => "coffee");
            }

            if ($this->ci->can_client_access("estimate")) {
                $sidebar_menu[] = array("name" => "estimates", "url" => "evaluation", "class" => "file");
            }
            ...
            if ($this->ci->can_client_access("invoice")) {
                if ($this->ci->can_client_access("invoice")) {
                    $sidebar_menu[] = array("name" => "invoices", "url" => "bills", "class" => "file-text");
                }
                if ($this->ci->can_client_access("payment", false)) {
                    $sidebar_menu[] = array("name" => "invoice_payments", "url" => "bill_payments", "class" => "dollar-sign");
                }
            }

            if ($this->ci->can_client_access("store", false) && get_setting("client_can_access_store")) {
                $sidebar_menu[] = array("name" => "store", "url" => "outlet", "class" => "truck");
                $sidebar_menu[] = array("name" => "orders", "url" => "arrangements", "class" => "shopping-cart");
            }

            if ($this->ci->can_client_access("ticket")) {
                $sidebar_menu[] = array("name" => "tickets", "url" => "tokens", "class" => "life-buoy");
            }

            if ($this->ci->can_client_access("announcement")) {
                $sidebar_menu[] = array("name" => "announcements", "url" => "notices", "class" => "bell");
            }

            $sidebar_menu[] = array("name" => "users", "url" => "patrons/users", "class" => "users");

            if (get_setting("client_can_view_files")) {
                $sidebar_menu[] = array("name" => "files", "url" => "patrons/files/" . $this->ci->login_user->id . "/page_view", "class" => "image");
            }

            $sidebar_menu[] = array("name" => "my_profile", "url" => "patrons/contact_profile/" . $this->ci->login_user->id, "class" => "settings");

            if ($this->ci->can_client_access("knowledge_base")) {
                $sidebar_menu[] = array("name" => "knowledge_base", "url" => "information_hub", "class" => "help-circle");
            }
    ...
    function _get_active_menu($sidebar_menu = array()) {
        ...
        foreach ($sidebar_menu as $key => $menu) {
            if (isset($menu["name"])) {
                ...
                if ($submenu && count($submenu)) {
                    foreach ($submenu as $iSubKey => $sub_menu) {
                        if (isset($sub_menu['name'])) {
                            ...
                            if (get_array_value($sub_menu, "url") === $controller_name) {
                                $sub_found_url_active_key = $iSubKey;
                            } else if (get_array_value($sub_menu, "url") === $controller_name) {
                                $sub_found_url_active_key = $iSubKey;
                            }
                        }
                    }
        ...
        if (!is_null($found_url_active_key)) {
            ...
            if(!is_null($sub_found_url_active_key)){
                $sidebar_menu[$found_url_active_key]['submenu'][$sub_found_url_active_key]["is_sub_active_menu"] = 1;
            }
        }
