...
class Cron_job {
    ...
    private function send_invoice_due_pre_reminder() {
        ...
        $invoices = $this->ci->Bills_model->get_details(array(
    ...
    private function create_recurring_invoices() {
        $recurring_invoices = $this->ci->Bills_model->get_renewable_invoices($this->today);
    ...
    private function _create_new_invoice($invoice) {
        ...
        //create new invoice
        $new_invoice_id = $this->ci->Bills_model->ci_save($new_invoice_data);

        //create invoice items
        $items = $this->ci->Bill_items_model->get_details(array("invoice_id" => $invoice->id))->getResult();
        foreach ($items as $item) {
            ...
            $this->ci->Bill_items_model->ci_save($new_invoice_item_data);
        }
        ...
        $this->ci->Bills_model->ci_save($recurring_invoice_data, $invoice->id);
    ...
    private function create_subscription_invoices() {
        $subscriptions = $this->ci->Membership_model->get_renewable_subscriptions($this->today);
    ...
    private function _create_new_invoice_of_subscription($subscription_info) {
        ...
        $this->ci->Membership_model->ci_save($subscription_data, $subscription_info->id);
    ...
    private function create_recurring_tasks() {
        ...
        $recurring_tasks = $this->ci->Responsibility_model->get_renewable_tasks($date);
    ...
    private function _create_new_task($task) {
        ...
        $new_task_data["sort"] = $this->ci->Responsibility_model->get_next_sort_value($task->project_id, $new_task_data["status_id"]);

        //create new task
        $new_task_id = $this->ci->Responsibility_model->ci_save($new_task_data);
        ...
        //create sub tasks
        $sub_tasks = $this->ci->Responsibility_model->get_all_where(array("parent_task_id" => $task->id, "deleted" => 0))->getResult();
        foreach ($sub_tasks as $sub_task) {
            ...
            $sub_task_data["sort"] = $this->ci->Responsibility_model->get_next_sort_value(get_array_value($sub_task_data, "project_id"), $sub_task_data["status_id"]);

            $sub_task_save_id = $this->ci->Responsibility_model->ci_save($sub_task_data);
        ...
        $this->ci->Responsibility_model->save_reminder_date($recurring_task_data, $task->id);
    ...
    private function close_inactive_tickets() {
        ...
        if ($auto_close_ticket_after_days) {
            ...
            $tickets = $this->ci->Tokens_model->get_details(array(
            ...
            foreach ($tickets as $ticket) {
                ...
                $this->ci->Tokens_model->ci_save($ticket_data, $ticket->id);
    ...
    private function create_recurring_expenses() {
        $recurring_expenses = $this->ci->Budgets_model->get_renewable_expenses($this->today);
    ...
    private function _create_new_expense($expense) {
        ...
        //create new expense
        $new_expense_id = $this->ci->Budgets_model->ci_save($new_expense_data);
        ...
        $this->ci->Budgets_model->ci_save($recurring_expense_data, $expense->id);
    ...
    private function create_recurring_reminders() {
        ...
        $recurring_reminders = $this->ci->Schedules_model->get_details($options)->getResult();
        foreach ($recurring_reminders as $reminder) {
            ...
            if ($target_time < $now && (!$reminder->no_of_cycles || $reminder->no_of_cycles_completed < $reminder->no_of_cycles)) {
                ...
                $this->ci->Schedules_model->ci_save($data, $reminder->id);
            }
        }
    }
