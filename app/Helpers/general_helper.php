...
if (!function_exists('get_team_member_profile_link')) {
    ...
    function get_team_member_profile_link($id = 0, $name = "", $attributes = array())
    {
        ...
        if ($ci->login_user->user_type === "staff") {
            return anchor("unit_members/view/" . $id, $name ? $name : "", $attributes);
        } else {
...
if (!function_exists('get_client_contact_profile_link')) {
    ...
    {
        return anchor("patrons/contact_profile/" . $id, $name ? $name : "", $attributes);
    }
}
...
if (!function_exists('get_invoice_making_data')) {
    ...
    function get_invoice_making_data($invoice_id)
    {
        ...
        $invoice_info = $ci->Bills_model->get_details(array("id" => $invoice_id))->getRow();
        if ($invoice_info) {
            $data['invoice_info'] = $invoice_info;
            $data['client_info'] = $ci->Patrons_model->get_one($data['invoice_info']->client_id);
            $data['invoice_items'] = $ci->Bill_items_model->get_details(array("invoice_id" => $invoice_id))->getResult();
            $data['invoice_status_label'] = get_invoice_status_label($invoice_info);
            $data["invoice_total_summary"] = $ci->Bills_model->get_invoice_total_summary($invoice_id);
...
if (!function_exists('prepare_invoice_pdf')) {
    ...
    function prepare_invoice_pdf($invoice_data, $mode = "download")
    {
        ...
        if ($invoice_data) {
            ...
            $html = view("bills/invoice_pdf", $invoice_data);
...
if (!function_exists('prepare_estimate_pdf')) {
    ...
    function prepare_estimate_pdf($estimate_data, $mode = "download")
    {
        ...
        if ($estimate_data) {
            ...
            $html = view("evaluations/estimate_pdf", $estimate_data);
            if ($mode != "html") {
                $pdf->writeHTML($html, true, false, true, false, '');
            }
...
if (!function_exists('prepare_order_pdf')) {
    ...
    function prepare_order_pdf($order_data, $mode = "download")
    {
        ...
        if ($order_data) {
            ...
            $html = view("arrangements/order_pdf", $order_data);
            if ($mode != "html") {
                $pdf->writeHTML($html, true, false, true, false, '');
            }
...
if (!function_exists('get_estimate_making_data')) {
    ...
    function get_estimate_making_data($estimate_id)
    {
        ...
        $ci = new App_Controller();
        $estimate_info = $ci->Evaluations_model->get_details(array("id" => $estimate_id))->getRow();
        if ($estimate_info) {
            $data['estimate_info'] = $estimate_info;
            $data['client_info'] = $ci->Patrons_model->get_one($data['estimate_info']->client_id);
            $data['estimate_items'] = $ci->Evaluation_items_model->get_details(array("estimate_id" => $estimate_id))->getResult();
            $data["estimate_total_summary"] = $ci->Evaluations_model->get_estimate_total_summary($estimate_id);
...
if (!function_exists('get_contract_making_data')) {
    ...
    function get_contract_making_data($contract_id)
    {
        ...
        if ($contract_info) {
            ...
            $data['client_info'] = $ci->Patrons_model->get_one($data['contract_info']->client_id);
...
if (!function_exists('get_proposal_making_data')) {
    ...
    function get_proposal_making_data($proposal_id)
    {
        ...
        if ($proposal_info) {
            ...
            $data['client_info'] = $ci->Patrons_model->get_one($data['proposal_info']->client_id);
            ...
            $data['total_read_count'] = $ci->Schedule_tracker_model->total_read_count(array("context" => "proposal", "context_id" => $proposal_id));
...
if (!function_exists('get_order_making_data')) {
    ...
    function get_order_making_data($order_id = 0, $cookie_hash = "")
    {
        ...
        $data = array();
        if ($order_id) {
            $order_info = $ci->Arrangements_model->get_details(array("id" => $order_id))->getRow();
            if ($order_info) {
                $data['order_info'] = $order_info;
                $data['client_info'] = $ci->Patrons_model->get_one($data['order_info']->client_id);
                $data['order_items'] = $ci->Arrangement_items_model->get_details(array("order_id" => $order_id))->getResult();
                $data["order_total_summary"] = $ci->Arrangements_model->get_order_total_summary($order_id);
            ...
            $data["order_total_summary"] = $ci->Arrangements_model->get_processing_order_total_summary($login_user_id, $cookie_hash);
        }
        return $data;
    }
}
...
if (!function_exists('get_team_members_and_teams_select2_data_list')) {
    ...
    function get_team_members_and_teams_select2_data_list($exclude_inactive_users = false)
    {
        ...
        $team = $ci->Unit_model->get_all_where(array("deleted" => 0))->getResult();
    ...
    function update_custom_fields_changes($related_to_type, $related_to_id, $changes, $activity_log_id = 0)
    {
        if ($changes && count($changes)) {
            ...
            if ($related_to_type == "tasks") {
                $related_to_data = $ci->Responsibility_model->get_one($related_to_id);
                $log_type = "task";
                $log_for = "project";
...
if (!function_exists('get_lead_contact_profile_link')) {
    ...
    {
        return anchor("initiates/contact_profile/" . $id, $name, $attributes);
    }
}
...
if (!function_exists('send_message_via_pusher')) {
    ...
    function send_message_via_pusher($to_user_id, $message_data, $message_id, $message_type = "message")
    {
        ...
        if ($message_type == "message") {
            ...
            $message_info = $ci->Memos_model->get_one($message_id);
...
if (!function_exists('add_auto_reply_to_ticket')) {
    ...
    function add_auto_reply_to_ticket($ticket_id = 0)
    {
        ...
        $Token_comments_model = model("App\Models\Token_comments_model");
        ...
        $comment_id = $Token_comments_model->ci_save($data);
...
if (!function_exists('prepare_contract_view')) {
    ...
    function prepare_contract_view($contract_data)
    {
        if ($contract_data) {
            ...
            $parser_data["CONTRACT_ITEMS"] = view("agreements/contract_parts/contract_items_table", $contract_data);
            ...
            $parser_data["COMPANY_INFO"] = view("agreements/contract_parts/contract_from");
            ...
            $parser_data["CONTRACT_TO_INFO"] = view("agreements/contract_parts/contract_to", $view_data);
            ...
            foreach ($link_codes as $key => $link_code) {
                ...
                if ($convert_links) {
                    ...
                    if ($context === "recording") {
                        ...
                        $links[] = anchor(get_uri("responsibility/view/" . $context . "/#comment-" . $context_id), $comment_text ? $comment_text : "", array('class' => 'comment-highlight-link', 'data-comment-id' => $context_id, 'data-task-id' => $context));
                    }
                } else {
                    $links[] = $comment_text;
                }
...
if (!function_exists('prepare_proposal_view')) {
    ...
    function prepare_proposal_view($proposal_data)
    {
        if ($proposal_data) {
            ...
            $parser_data["PROPOSAL_ITEMS"] = view("plans/proposal_parts/proposal_items_table", $proposal_data);
            $parser_data["PROPOSAL_NOTE"] = $proposal_info->note;
            $parser_data["APP_TITLE"] = get_setting("app_title");

            $parser_data["COMPANY_INFO"] = view("plans/proposal_parts/proposal_from");
            ...
            $view_data["client_info"] = $client_info;
            $view_data["is_preview"] = true;
            $parser_data["PROPOSAL_TO_INFO"] = view("plans/proposal_parts/proposal_to", $view_data);
...
if (!function_exists('prepare_allowed_members_array')) {
    ...
    function prepare_allowed_members_array($permissions, $user_id)
    {
        ...
        if (count($allowed_teams)) {
            $Unit_model = model("App\Models\Unit_model");
            $team = $Unit_model->get_members($allowed_teams)->getResult();
...
if (!function_exists('get_subscription_making_data')) {
    ...
    {
        $ci = new App_Controller();
        $subscription_info = $ci->Membership_model->get_details(array("id" => $subscription_id))->getRow();
        if ($subscription_info) {
            $data['subscription_info'] = $subscription_info;
            $data['client_info'] = $ci->Patrons_model->get_one($data['subscription_info']->client_id);
            ...
            $data["subscription_total_summary"] = $ci->Membership_model->get_subscription_total_summary($subscription_id);
...
if (!function_exists('create_invoice_from_subscription')) {
    ...
    {
        $ci = new App_Controller();
        $subscription_info = $ci->Membership_model->get_one($subscription_id);
        ...
        if ($subscription_info) {
            ...
            $ci->Membership_model->ci_save($subscription_data, $subscription_info->id);
        }
        ...
        //create new invoice
        $new_invoice_id = $ci->Bills_model->ci_save($invoice_data);
        ...
        foreach ($items as $item) {
            ...
            $ci->Bill_items_model->ci_save($new_invoice_item_data);
        }
        $ci->Bills_model->update_invoice_total_meta($new_invoice_id);
...
if (!function_exists('show_clients_of_this_client_contact')) {
    ...
    function show_clients_of_this_client_contact($login_user, $show_icon = false)
    {
        ...
        if (count($clients)) {
            ...
            $view_data["login_user_company_name"] = $Patrons_model->get_one($login_user->client_id)->company_name;
            $view_data["show_icon"] = $show_icon;
            echo view("patrons/clients_dropdown_of_this_client_contact", $view_data);
        }
    }
}
...
if (!function_exists('get_reminder_context_info')) {
    ...
    function get_reminder_context_info($reminder_info)
    {
        ...
        if ($reminder_info->lead_id) {
            $context_url = get_uri("initiates/view/$reminder_info->lead_id");
            $context_icon = "layers";
        } else if ($reminder_info->client_id) {
            $context_url = get_uri("patrons/view/$reminder_info->client_id");
            $context_icon = "briefcase";
        } else if ($reminder_info->task_id) {
            $context_url = get_uri("responsibility/view/$reminder_info->task_id");
            $context_icon = "check-circle";
        } else if ($reminder_info->project_id) {
            $context_url = get_uri("enterprises/view/$reminder_info->project_id");
            $context_icon = "grid";
        } else if ($reminder_info->ticket_id) {
            $context_url = get_uri("tokens/view/$reminder_info->ticket_id");
            $context_icon = "life-buoy";
        } else if ($reminder_info->proposal_id) {
            $context_url = get_uri("plans/view/$reminder_info->proposal_id");
            $context_icon = "file-text";
        } else if ($reminder_info->contract_id) {
            $context_url = get_uri("agreements/view/$reminder_info->contract_id");
            $context_icon = "file-plus";
        } else if ($reminder_info->subscription_id) {
            $context_url = get_uri("membership/view/$reminder_info->subscription_id");
            $context_icon = "repeat";
        } else if ($reminder_info->invoice_id) {
            $context_url = get_uri("bills/view/$reminder_info->invoice_id");
            $context_icon = "file-text";
        } else if ($reminder_info->order_id) {
            $context_url = get_uri("arrangements/view/$reminder_info->order_id");
            $context_icon = "shopping-cart";
        } else if ($reminder_info->estimate_id) {
            $context_url = get_uri("arrangements/view/$reminder_info->estimate_id");
            $context_icon = "file";
        }
...
if (!function_exists('create_invoice_from_order')) {
    ...
    function create_invoice_from_order($order_id)
    {
        ...
        $order_info = $ci->Arrangements_model->get_one($order_id);
        $now = get_my_local_time("Y-m-d");
        ...
        $invoice_id = $ci->Bills_model->ci_save($invoice_data);

        //copy items
        $order_items = $ci->Arrangement_items_model->get_details(array("order_id" => $order_id))->getResult();
        foreach ($order_items as $data) {
            ...
            $ci->Bill_items_model->ci_save($invoice_item_data);
        }
        $ci->Bills_model->update_invoice_total_meta($invoice_id);
        return $invoice_id;
    }
}
...
if (!function_exists('prepare_invoice_display_id_data')) {
    function prepare_invoice_display_id_data($invoice_due_date, $invoice_bill_date)
    {
        ...
        $last_invoice_id = $ci->Bills_model->get_last_invoice_id();
        ...
        if (get_setting("reset_invoice_number_every_year") && in_array("YEAR", $invoice_number_format_parts)) {
            $last_sequence = $ci->Bills_model->get_last_invoice_sequence($invoice_year);
...
if (!function_exists('prepare_proposal_pdf')) {
    ...
    function prepare_proposal_pdf($proposal_data, $mode = "download")
    {
        ...
        if ($proposal_data) {
            ...
            $html = view("plans/proposal_pdf", $proposal_data);
            $html = rebuild_html($html, $pageWidthInPixels);
...
if (!function_exists('prepare_contract_pdf')) {
    ...
    function prepare_contract_pdf($contract_data, $mode = "download")
    {
        ...
        if ($contract_data) {
            ...
            $html = view("agreements/contract_pdf", $contract_data);
            $html = rebuild_html($html, $pageWidthInPixels);
