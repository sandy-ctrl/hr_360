...
if (!function_exists('get_reports_topbar')) {
    ...
    function get_reports_topbar($return_array = false) {
        ...
        if (get_setting("module_invoice") == "1" && ($ci->login_user->is_admin || $access_invoice)) {
            $sales_dropdown_button["invoices_summary"] = array("name" => "invoices_summary", "url" => "bills/invoices_summary");
            $show_payments_button = true;
        }

        /*
          if (get_setting("module_order") == "1" && ($ci->login_user->is_admin || $access_order)) {
          $sales_dropdown_button["orders_summary"] = array("name" => "orders_summary", "url" => "arrangements/orders_summary");
          }
         */

        if (count($sales_dropdown_button)) {
            $reports_menu["sales"] = array("name" => "sales", "url" => "bills", "class" => "shopping-cart", "dropdown_item" => $sales_dropdown_button);
        }
        ...
        if ($show_expenses_button && $show_payments_button) {
            $finance_dropdown_button["income_vs_expenses"] = array("name" => "income_vs_expenses", "url" => "budgets/income_vs_expenses");
        }
        if ($show_expenses_button) {
            $finance_dropdown_button["expenses_summary"] = array("name" => "expenses_summary", "url" => "budgets/summary");
        }
        if ($show_payments_button) {
            $finance_dropdown_button["payments_summary"] = array("name" => "payments_summary", "url" => "bill_payments/payments_summary");
        }



        if (count($finance_dropdown_button)) {
            $reports_menu["finance"] = array("name" => "finance", "url" => "budgets_summary", "class" => "bar-chart-2", "dropdown_item" => $finance_dropdown_button);
        }

        if (get_setting("module_project_timesheet") && ($ci->login_user->is_admin || !get_array_value($permissions, "do_not_show_projects"))) {
            $reports_menu[] = array("name" => "timesheets", "url" => "enterprises/all_timesheets", "class" => "clock", "single_button" => true);
        }

        if ($ci->login_user->is_admin || get_array_value($permissions, "can_manage_all_projects") == "1") {
            $reports_menu[] = array("name" => "projects", "url" => "enterprises/team_members_summary", "class" => "command", "single_button" => true);
        }

        if (get_setting("module_lead") == "1" && ($ci->login_user->is_admin || $access_lead == "all")) {
            $reports_menu[] = array("name" => "leads", "url" => "initiates/converted_to_client_report", "class" => "layers", "single_button" => true);
        }

        if (get_setting("module_ticket") == "1" && ($ci->login_user->is_admin || $access_ticket == "all")) {
            $reports_menu[] = array("name" => "tickets", "url" => "tokens/tickets_chart_report", "class" => "life-buoy", "single_button" => true);
        }
        ...
        if ($return_array) {
            ...
            return $template->view("outlines/topbar", $view_data);
        }
    }
