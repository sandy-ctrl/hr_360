...
if (!function_exists('clock_widget')) {
    ...
    function clock_widget() {
        ...
        return $template->view("timesheet/clock_widget", $view_data);
    }
...
if (!function_exists('timeline_widget')) {
    ...
    function timeline_widget($params = array()) {
        ...
        if ($user_id && !count($logs->result)) {
            ...
            $template = new Template();
            return $template->view("agenda/no_post_message");
        } else {
            $template = new Template();
            return $template->view("agenda/post_list", $view_data);
        }
    }
...
if (!function_exists('announcements_alert_widget')) {
    ...
    function announcements_alert_widget() {
        ...
        if ($ci->login_user->user_type === "client") {
            $client_group_ids = $ci->Patrons_model->get_one($ci->login_user->client_id)->group_ids;
        }

        $announcements = $ci->Notices_model->get_unread_announcements($ci->login_user->id, $ci->login_user->user_type, $client_group_ids)->getResult();
        $view_data["announcements"] = $announcements;
        $template = new Template();
        return $template->view("notices/alert", $view_data);
    }
...
if (!function_exists('my_open_tasks_widget')) {
    ...
    function my_open_tasks_widget() {
        $ci = new Security_Controller(false);
        $view_data["total"] = $ci->Responsibility_model->count_my_open_tasks($ci->login_user->id);
        $template = new Template();
        return $template->view("responsibility/open_tasks_widget", $view_data);
    }
...
if (!function_exists('my_task_stataus_widget')) {
    ...
    function my_task_stataus_widget($custom_class = "") {
        $ci = new Security_Controller(false);
        $view_data["task_statuses"] = $ci->Responsibility_model->get_task_statistics(array("user_id" => $ci->login_user->id))->task_statuses;
        $view_data["custom_class"] = $custom_class;

        $template = new Template();
        return $template->view("responsibility/my_task_status_widget", $view_data);
    }
...
if (!function_exists('events_today_widget')) {
    ...
    function events_today_widget() {
        ...
        $view_data["total"] = $ci->Schedules_model->count_events_today($options);
        $template = new Template();
        return $template->view("schedules/events_today", $view_data);
    }
...
if (!function_exists('new_posts_widget')) {
    ...
    function new_posts_widget() {
        ...
        $template = new Template();
        return $template->view("agenda/new_posts_widget", $view_data);
    }
...
if (!function_exists('events_widget')) {
    ...
    function events_widget() {
        ...
        $view_data["events"] = $ci->Schedules_model->get_upcomming_events($options);

        $template = new Template();
        return $template->view("schedules/events_widget", $view_data);
    }
...
if (!function_exists('income_vs_expenses_widget')) {
    ...
    function income_vs_expenses_widget($custom_class = "") {
        $Budgets_model = model("App\Models\Budgets_model");
        $info = $Budgets_model->get_income_expenses_info();

        $today = explode('-', get_today_date());
        $current_year = get_array_value($today, 0);
        $previous_year = get_array_value($today, 0) - 1;
        $view_data["current_year_info"] = $Budgets_model->get_income_expenses_info(array("year" => $current_year));
        $view_data["previous_year_info"] = $Budgets_model->get_income_expenses_info(array("year" => $previous_year));

        $view_data["income"] = $info->income ? $info->income : 0;
        $view_data["expenses"] = $info->expneses ? $info->expneses : 0;
        $view_data["custom_class"] = $custom_class;
        $template = new Template();
        return $template->view("budgets/income_expenses_widget", $view_data);
    }
...
if (!function_exists('ticket_status_widget')) {
    ...
    function ticket_status_widget($data = array()) {
        $ci = new Security_Controller(false);
        $Tokens_model = model("App\Models\Tokens_model");
        ...
        $tickets_result = $Tokens_model->get_ticket_statistics($created_date_options)->getResult();
        ...
        $view_data["tickets_info"] = $Tokens_model->get_ticket_statistics($options)->getResult();

        $options["group_by"] = "ticket_status";
        $ticket_status_info = $Tokens_model->get_ticket_statistics($options)->getResult();
        $view_data["new"] = 0;
        $view_data["open"] = 0;
        $view_data["closed"] = 0;
        ...
        $template = new Template();
        return $template->view("tokens/ticket_status_widget", $view_data);
    }
...
if (!function_exists('invoice_statistics_widget')) {
    ...
    function invoice_statistics_widget($options = array()) {
        ...
        if ($ci->login_user->user_type == "client") {
            ...
            $client_info = $ci->Patrons_model->get_one($ci->login_user->client_id);
        ...
        $info = $ci->Bills_model->invoice_statistics($options);
        ...
        $template = new Template();
        return $template->view("bills/invoice_statistics_widget/index", $view_data);
    }
...
if (!function_exists('project_timesheet_statistics_widget')) {
    ...
    function project_timesheet_statistics_widget($type = "", $options = array()) {
        ...
        if ($type == "my_timesheet_statistics") {
            return $template->view("enterprises/timesheets/timesheet_wedget", $view_data);
        } else {
            return $template->view("enterprises/timesheets/all_timesheet_wedget", $view_data);
        }
    }
...
if (!function_exists('timecard_statistics_widget')) {
    ...
    function timecard_statistics_widget() {
        ...
        $view_data["ticks"] = json_encode($ticks);
        $template = new Template();
        return $template->view("timesheet/timecard_statistics", $view_data);
    }
...
if (!function_exists('count_project_status_widget')) {
    ...
    function count_project_status_widget($user_id = 0) {
        ...
        $info = $ci->Enterprises_model->count_project_status($options);
        $view_data["project_open"] = $info->open;
        $view_data["project_completed"] = $info->completed;
        $template = new Template();
        return $template->view("enterprises/widgets/project_status_widget", $view_data);
    }
...
if (!function_exists('count_total_time_widget')) {
    ...
    function count_total_time_widget($user_id = 0) {
        ...
        $template = new Template();
        return $template->view("timesheet/total_time_widget", $view_data);
    }
...
if (!function_exists('count_total_time_widget_small')) {
    ...
    function count_total_time_widget_small($user_id = 0, $widget_type = "") {
        ...
        $template = new Template();
        return $template->view("timesheet/total_time_widget_small", $view_data);
    }
...
if (!function_exists('count_unread_message')) {
    ...
    function count_unread_message() {
        $ci = new Security_Controller(false);
        return $ci->Memos_model->count_unread_message($ci->login_user->id, $ci->get_allowed_user_ids());
    }
...
if (!function_exists('count_new_tickets')) {
    ...
    function count_new_tickets($ticket_types = "", $show_assigned_tickets_only_user_id = 0) {
        $Tokens_model = model("App\Models\Tokens_model");
        return $Tokens_model->count_new_tickets($ticket_types, $show_assigned_tickets_only_user_id);
    }
...
if (!function_exists('all_tasks_kanban_widget')) {
    ...
    function all_tasks_kanban_widget() {
        ...
        $projects = $ci->Responsibility_model->get_my_projects_dropdown_list($ci->login_user->id)->getResult();
        ...
        $view_data['task_statuses'] = $ci->Responsibility_status_model->get_details()->getResult();
        ...
        $Responsibility_priority_model = model("App\Models\Responsibility_priority_model");
        $priorities = $Responsibility_priority_model->get_details()->getResult();
        ...
        $template = new Template();
        return $template->view("responsibility/kanban/all_tasks_kanban_widget", $view_data);
    }
...
if (!function_exists('open_projects_widget')) {
    ...
    function open_projects_widget($user_id = 0) {
        ...
        $view_data["project_open"] = $ci->Enterprises_model->count_project_status($options)->open;
        $template = new Template();
        return $template->view("enterprises/widgets/open_projects_widget", $view_data);
    }
...
if (!function_exists('completed_projects_widget')) {
    ...
    function completed_projects_widget($user_id = 0) {
        ...
        $view_data["project_completed"] = $ci->Enterprises_model->count_project_status($options)->completed;
        $template = new Template();
        return $template->view("enterprises/widgets/completed_projects_widget", $view_data);
    }
...
if (!function_exists('count_clock_in_out_widget_small')) {
    ...
    function count_clock_in_out_widget_small($data = array()) {
        ...
        if ($widget === "members_clocked_in") {
            ...
            return $template->view("timesheet/count_clock_in_widget", $view_data);
        } else if ($widget == "members_clocked_out") {
            $view_data["members_clocked_out"] = $info->members_clocked_out ? $info->members_clocked_out : 0;
            return $template->view("timesheet/count_clock_out_widget", $view_data);
        }
    }
...
if (!function_exists('my_open_projects_widget')) {
    ...
    function my_open_projects_widget($client_id = 0) {
        ...
        $view_data["projects"] = $ci->Enterprises_model->get_details($options)->getResult();
        $template = new Template();
        return $template->view("enterprises/widgets/my_open_projects_widget", $view_data);
    }
...
if (!function_exists('my_starred_projects_widget')) {
    ...
    function my_starred_projects_widget($user_id = 0) {
        ...
        $view_data["projects"] = $ci->Enterprises_model->get_details($options)->getResult();
        $template = new Template();
        return $template->view("enterprises/widgets/my_starred_projects_widget", $view_data);
    }
...
if (!function_exists('ticket_status_widget_small')) {
    ...
    function ticket_status_widget_small($data = array()) {
        ...
        $view_data["total_tickets"] = $ci->Tokens_model->count_tickets($options);
        $view_data["status"] = $status;

        $template = new Template();
        return $template->view("tokens/ticket_status_widget_small", $view_data);
    }
...
if (!function_exists('all_team_members_widget')) {
    ...
    function all_team_members_widget() {
        ...
        $template = new Template();
        return $template->view("unit_members/team_members_widget", $view_data);
    }
...
if (!function_exists('clocked_in_team_members_widget')) {
    ...
    function clocked_in_team_members_widget($data = array()) {
        ...
        $template = new Template();
        return $template->view("unit_members/clocked_in_team_members_widget", $view_data);
    }
...
if (!function_exists('clocked_out_team_members_widget')) {
    ...
    function clocked_out_team_members_widget($data = array()) {
        ...
        $template = new Template();
        return $template->view("unit_members/clocked_out_team_members_widget", $view_data);
    }
...
if (!function_exists('active_members_and_clients_widget')) {
    ...
    function active_members_and_clients_widget($user_type = "", $show_own_clients_only_user_id = "", $allowed_client_groups = "") {
        ...
        $template = new Template();
        return $template->view("unit_members/active_members_and_clients_widget", $view_data);
    }
...
if (!function_exists('get_invoices_value_widget')) {
    ...
    function get_invoices_value_widget($type = "") {
        ...
        $view_data["invoices_info"] = $Bills_model->get_invoices_total_and_paymnts($options);
        $view_data["type"] = $type;
        $template = new Template();
        return $template->view("bills/total_invoices_value_widget", $view_data);
    }
...
if (!function_exists('my_tasks_list_widget')) {
    ...
    function my_tasks_list_widget() {
        $Responsibility_status_model = model("App\Models\Responsibility_status_model");
        $view_data['task_statuses'] = $Responsibility_status_model->get_details()->getResult();
        $template = new Template();
        return $template->view("responsibility/my_tasks_list_widget", $view_data);
    }
...
if (!function_exists('pending_leave_approval_widget')) {
    ...
    function pending_leave_approval_widget($data = array()) {
        ...
        $view_data["total"] = count($ci->Vacate_applications_model->get_list($options)->getResult());

        $template = new Template();
        return $template->view("vacates/pending_leave_approval_widget", $view_data);
    }
...
if (!function_exists('total_clients_widget')) {
    ...
    function total_clients_widget($show_own_clients_only_user_id = "", $allowed_client_groups = "") {
        $Patrons_model = model("App\Models\Patrons_model");
        $view_data["total"] = $Patrons_model->count_total_clients(array("show_own_clients_only_user_id" => $show_own_clients_only_user_id, "client_groups" => $allowed_client_groups));
        $template = new Template();
        return $template->view("patrons/total_clients_widget", $view_data);
    }
...
if (!function_exists('total_contacts_widget')) {
    ...
    function total_contacts_widget($show_own_clients_only_user_id = "", $allowed_client_groups = "") {
        ...
        $template = new Template();
        return $template->view("patrons/total_contacts_widget", $view_data);
    }
...
if (!function_exists('active_members_on_projects_widget')) {
    ...
    function active_members_on_projects_widget() {
        ...
        $template = new Template();
        return $template->view("unit_members/active_members_on_projects_widget", $view_data);
    }
...
if (!function_exists('open_tickets_list_widget')) {
    ...
    function open_tickets_list_widget() {
        ...
        if ($ci->login_user->user_type == "client") {
            ...
            $template = new Template();
            return $template->view("patrons/tickets/open_tickets_list_widget", $view_data);
        } else {
            $template = new Template();
            return $template->view("tokens/open_tickets_list_widget");
        }
    }
...
if (!function_exists('total_leads_widget')) {
    ...
    function total_leads_widget($returen_as_data = false, $show_own_leads_only_user_id = "") {
        $Patrons_model = model("App\Models\Patrons_model");
        $view_data["total"] = $Patrons_model->count_total_leads(array("show_own_leads_only_user_id" => $show_own_leads_only_user_id));

        $template = new Template();
        return $template->view('initiates/total_leads_widget', $view_data, $returen_as_data);
    }
...
if (!function_exists('client_contacts_logged_in_widget')) {
    ...
    function client_contacts_logged_in_widget($widget_type = "", $show_own_clients_only_user_id = "", $allowed_client_groups = "", $return_as_data = false) {
        ...
        $template = new Template();
        return $template->view("patrons/widgets/client_contacts_logged_in_widget", $view_data, $return_as_data);
    }
...
if (!function_exists('client_invoices_widget')) {
    ...
    function client_invoices_widget($widget_type = "", $show_own_clients_only_user_id = "", $allowed_client_groups = "", $return_as_data = false) {
        ...
        $view_data["total"] = $Patrons_model->count_total_clients($options);
        $view_data["total_clients"] = $Patrons_model->count_total_clients();
        $view_data["widget_type"] = $widget_type;

        $template = new Template();
        return $template->view("patrons/widgets/client_invoices_widget", $view_data, $return_as_data);
    }
...
if (!function_exists('client_projects_widget')) {
    ...
    function client_projects_widget($show_own_clients_only_user_id = "", $allowed_client_groups = "", $return_as_data = false) {
        ...
        $view_data["clients_has_open_projects"] = $Patrons_model->count_total_clients(array("filter" => "has_open_projects", "show_own_clients_only_user_id" => $show_own_clients_only_user_id, "client_groups" => $allowed_client_groups));
        $view_data["clients_has_completed_projects"] = $Patrons_model->count_total_clients(array("filter" => "has_completed_projects", "show_own_clients_only_user_id" => $show_own_clients_only_user_id, "client_groups" => $allowed_client_groups));
        $view_data["clients_has_any_hold_projects"] = $Patrons_model->count_total_clients(array("filter" => "has_any_hold_projects", "show_own_clients_only_user_id" => $show_own_clients_only_user_id, "client_groups" => $allowed_client_groups));
        $view_data["clients_has_canceled_projects"] = $Patrons_model->count_total_clients(array("filter" => "has_canceled_projects", "show_own_clients_only_user_id" => $show_own_clients_only_user_id, "client_groups" => $allowed_client_groups));

        $template = new Template();
        return $template->view("patrons/widgets/projects_info_widget", $view_data, $return_as_data);
    }
...
if (!function_exists('client_estimates_widget')) {
    ...
    function client_estimates_widget($show_own_clients_only_user_id = "", $allowed_client_groups = "", $return_as_data = false) {
        ...
        $view_data["clients_has_open_estimates"] = $Patrons_model->count_total_clients(array("filter" => "has_open_estimates", "show_own_clients_only_user_id" => $show_own_clients_only_user_id, "client_groups" => $allowed_client_groups));
        $view_data["clients_has_accepted_estimates"] = $Patrons_model->count_total_clients(array("filter" => "has_accepted_estimates", "show_own_clients_only_user_id" => $show_own_clients_only_user_id, "client_groups" => $allowed_client_groups));
        $view_data["clients_has_new_estimate_requests"] = $Patrons_model->count_total_clients(array("filter" => "has_new_estimate_requests", "show_own_clients_only_user_id" => $show_own_clients_only_user_id, "client_groups" => $allowed_client_groups));
        $view_data["clients_has_estimate_requests_in_progress"] = $Patrons_model->count_total_clients(array("filter" => "has_estimate_requests_in_progress", "show_own_clients_only_user_id" => $show_own_clients_only_user_id, "client_groups" => $allowed_client_groups));

        $template = new Template();
        return $template->view("patrons/widgets/client_estimates_widget", $view_data, $return_as_data);
    }
...
if (!function_exists('clients_has_open_tickets_widget')) {
    ...
    function clients_has_open_tickets_widget($show_own_clients_only_user_id = "", $allowed_client_groups = "") {
        ...
        $view_data["clients_has_open_tickets"] = $Patrons_model->count_total_clients($options);
        $view_data["total_clients"] = $Patrons_model->count_total_clients(array("show_own_clients_only_user_id" => $show_own_clients_only_user_id, "client_groups" => $allowed_client_groups));
        $template = new Template();
        return $template->view("patrons/widgets/clients_has_open_tickets_widget", $view_data);
    }
...
if (!function_exists('clients_has_new_orders_widget')) {
    ...
    function clients_has_new_orders_widget($show_own_clients_only_user_id = "", $allowed_client_groups = "") {
        ...
        $view_data["clients_has_new_orders"] = $Patrons_model->count_total_clients($options);
        $view_data["total_clients"] = $Patrons_model->count_total_clients();
        $template = new Template();
        return $template->view("patrons/widgets/clients_has_new_orders_widget", $view_data);
    }
...
if (!function_exists('client_proposals_widget')) {
    ...
    function client_proposals_widget($show_own_clients_only_user_id = "", $allowed_client_groups = "", $return_as_data = false) {
        ...
        $view_data["clients_has_open_proposals"] = $Patrons_model->count_total_clients(array("filter" => "has_open_proposals", "show_own_clients_only_user_id" => $show_own_clients_only_user_id, "client_groups" => $allowed_client_groups));
        $view_data["clients_has_accepted_proposals"] = $Patrons_model->count_total_clients(array("filter" => "has_accepted_proposals", "show_own_clients_only_user_id" => $show_own_clients_only_user_id, "client_groups" => $allowed_client_groups));
        $view_data["clients_has_rejected_proposals"] = $Patrons_model->count_total_clients(array("filter" => "has_rejected_proposals", "show_own_clients_only_user_id" => $show_own_clients_only_user_id, "client_groups" => $allowed_client_groups));

        $template = new Template();
        return $template->view("patrons/widgets/client_proposals_widget", $view_data, $return_as_data);
    }
...
if (!function_exists('projects_overview_widget')) {
    ...
    function projects_overview_widget() {
        ...
        $view_data["count_project_status"] = $ci->Enterprises_model->count_project_status($options);

        $view_data["projects_info"] = $ci->Enterprises_model->count_task_points($options);

        $template = new Template();
        return $template->view("enterprises/widgets/projects_overview_widget", $view_data);
    }
...
if (!function_exists('reminders_widget')) {
    ...
    function reminders_widget($return_reminders_only = false) {
        ...
        $Schedules_model = model('App\Models\Schedules_model');
        $local_time = get_my_local_time("Y-m-d H:i") . ":00";
        $reminders = $Schedules_model->get_details(array(
                    "user_id" => $ci->login_user->id,
                    "type" => "all",
                    "reminder_status" => "new",
...
if (!function_exists('estimate_sent_statistics_widget')) {
    ...
    function estimate_sent_statistics_widget($options = array()) {
        ...
        $info = $ci->Evaluations_model->estimate_sent_statistics($options);
        ...
        $template = new Template();
        return $template->view("evaluations/estimate_sent_statistics_widget/index", $view_data);
    }
...
if (!function_exists('last_announcement_widget')) {
    ...
    function last_announcement_widget() {
        ...
        if ($ci->login_user->user_type === "client") {
            $group_ids = $ci->Patrons_model->get_one($ci->login_user->client_id)->group_ids;
            if ($group_ids) {
                $options["client_group_ids"] = $group_ids;
            }
        }

        $Announcement_model = model("App\Models\Notices_model");
        $view_data["last_announcement"] = $Announcement_model->get_last_announcement($options);

        $template = new Template();
        return $template->view("notices/last_announcement_widget", $view_data);
    }
...
if (!function_exists('team_members_overview_widget')) {
    ...
    function team_members_overview_widget($data = array()) {
        ...
        $view_data["on_leave_today"] = count($ci->Vacate_applications_model->get_list($leave_options)->getResult());
        ...
        $template = new Template();
        return $template->view("unit_members/team_members_overview_widget", $view_data);
    }
...
if (!function_exists('tasks_overview_widget')) {
    ...
    function tasks_overview_widget($type = "") {
        ...
        $view_data["task_statuses"] = $ci->Responsibility_model->get_task_statistics($options)->task_statuses;
        $view_data["task_priorities"] = $ci->Responsibility_model->get_task_statistics($options)->task_priorities;
        $view_data["expired_tasks"] = $ci->Responsibility_model->get_task_statistics($options)->expired_tasks;
        $view_data["type"] = $type;
        
        $template = new Template();
        return $template->view("responsibility/tasks_overview_widget", $view_data);
    }
...
if (!function_exists('invoice_overview_widget')) {
    ...
    function invoice_overview_widget($options = array()) {
        ...
        $invoice_info = $ci->Bills_model->get_invoices_total_and_paymnts(array("currency" => $currency));
        ...
        $template = new Template();
        return $template->view("bills/invoice_overview_widget", $view_data);
    }
...
if (!function_exists('next_reminder_widget')) {
    ...
    function next_reminder_widget() {
        ...
        $reminders_of_today = $Schedules_model->get_details(array_merge($options, array(
        ...
        $next_reminder = $Schedules_model->get_details(array_merge($options, array(
...
if (!function_exists('leads_overview_widget')) {
    ...
    function leads_overview_widget() {
        ...
        $view_data["lead_statuses"] = $ci->Patrons_model->get_lead_statistics($options)->lead_statuses;
        $view_data["total_leads"] = $ci->Patrons_model->count_total_leads($options);
        $view_data["converted_to_client"] = $ci->Patrons_model->get_lead_statistics($options)->converted_to_client;

        $template = new Template();
        return $template->view("initiates/leads_overview_widget", $view_data);
    }
...
if (!function_exists('projects_widget')) {
    ...
    function projects_widget() {
        ...
        $template = new Template();
        return $template->view("patrons/projects/index", $view_data);
    }
