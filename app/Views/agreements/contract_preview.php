    ...
    <div class="contract-preview">
        ...
        if (!isset($is_editor_preview)) {
            ...
            if ($show_close_preview) {
                echo "<div class='text-center'>" . anchor("agreements/view/" . $contract_info->id, app_lang("close_preview"), array("class" => "btn btn-default round mb20 mr5")) . "</div>";
            }

            $action_buttons .= "<div class='float-start'>" . anchor(get_uri("agreements/download_pdf/" . $contract_info->id), "<i data-feather='download' class='icon-16'></i> " . app_lang('download_pdf'), array("title" => app_lang('download_pdf'), "class" => "btn btn-default round mr10")) . "</div>";
                        ...
                        <div class="clearfix contract-preview-button">
                            <div class="mr15 strong float-start grid-button-group">
                                ...
                                if ($login_user->user_type === "client" && get_setting("add_signature_option_on_accepting_contract")) {
                                    ...
                                    echo ajax_anchor(get_uri("agreements/update_contract_status/$contract_info->id/accepted"), "<i data-feather='check-circle' class='icon-16'></i> " . app_lang('mark_as_accepted'), array("class" => "btn btn-success mr15", "title" => app_lang('mark_as_accepted'), "data-reload-on-success" => "1"));
                                }
                                ?>

                                <?php echo ajax_anchor(get_uri("agreements/update_contract_status/$contract_info->id/declined"), "<i data-feather='x-circle' class='icon-16'></i> " . app_lang('mark_as_rejected'), array("class" => "btn btn-danger mr15", "title" => app_lang('mark_as_rejected'), "data-reload-on-success" => "1")); ?>
                            </div>
        ...
        <?php
        if (!isset($is_editor_preview)) {
            echo view("agreements/signer_info", array("contract_status" => $contract_info->status));
        }
        ?>
