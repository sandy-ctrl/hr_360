            ...
            <div class="proposal-preview">
                ...
                if (!isset($is_editor_preview)) {
                    ...
                    if ($show_close_preview) {
                        echo "<div class='text-center'>" . anchor("plans/view/" . $proposal_info->id, app_lang("close_preview"), array("class" => "btn btn-default round mb20 mr5")) . "</div>";
                    }

                    $action_buttons .= "<div class='float-start'>" . anchor(get_uri("plans/download_pdf/" . $proposal_info->id), "<i data-feather='download' class='icon-16'></i> " . app_lang('download_pdf'), array("title" => app_lang('download_pdf'), "class" => "btn btn-default round mr10")) . "</div>";
                                ...
                                <div class="clearfix proposal-preview-button">
                                    <div class="mr15 strong float-start grid-button-group">
                                        ...
                                        if ($login_user->user_type === "client" && get_setting("add_signature_option_on_accepting_proposal")) {
                                            ...
                                            echo ajax_anchor(get_uri("plans/update_proposal_status/$proposal_info->id/accepted"), "<i data-feather='check-circle' class='icon-16'></i> " . app_lang('mark_as_accepted'), array("class" => "btn btn-success mr15", "title" => app_lang('mark_as_accepted'), "data-reload-on-success" => "1"));
                                        }
                                        ?>

                                        <?php echo ajax_anchor(get_uri("plans/update_proposal_status/$proposal_info->id/declined"), "<i data-feather='x-circle' class='icon-16'></i> " . app_lang('mark_as_rejected'), array("class" => "btn btn-danger mr15", "title" => app_lang('mark_as_rejected'), "data-reload-on-success" => "1")); ?>
                                    </div>
                ...
                <?php
                if (!isset($is_editor_preview)) {
                    echo view("plans/signer_info", array("proposal_status" => $proposal_info->status));
                }
                ?>
            </div>
    ...
    <?php if (get_setting("enable_comments_on_proposals") && $proposal_info->status != "draft" && !isset($is_editor_preview)) { ?>
        <div class="hidden-xs box-content bg-white" style="width: 400px; min-height: 100%;">
            <div id="proposal-comment-container">
                <?php echo view("plans/comment_form"); ?>
            </div>
        </div>
    <?php } ?>
