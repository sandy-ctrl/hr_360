...
foreach ($comments as $comment) {
    ...
    if ($comment->file_id) {
        ...
        $file_download_url = "responsibility/download_comment_files";
                    ...
                    <div class="mb5">
                        ...
                        <span class="float-end comment-like-top">
                            ...
                            if (isset($comment->like_status)) {
                                ...
                                echo ajax_anchor(get_uri("enterprises/like_comment/" . $comment->id), "<i data-feather='$like_icon' class='icon-14 $like_icon_fill'></i> " . app_lang('like') . " ", array("class" => "mr5 like-button", "data-real-target" => "#comment-like-container-$comment->id"));
                            }
                            ?>
                        </span>
                        ...
                        <div class="mb15 clearfix">
                            <?php if (isset($comment->like_status)) { ?>
                                <span id="comment-like-container-<?php echo $comment->id; ?>">
                                    <?php echo view("enterprises/comments/like_comment", array("comment" => $comment)); ?>
                                </span>
                            <?php } ?>
                            ...
                            if ($type === "project" || $type === "customer_feedback") {
                                $can_reply = true;
                                echo ajax_anchor(get_uri("enterprises/comment_reply_form/" . $comment->id . "/" . $type . "/" . $type_id), "<i data-feather='corner-up-left' class='icon-16'></i> " . app_lang('reply'), array("data-real-target" => "#reply-form-container-" . $comment->id));
                            }
                            ?>
                            <?php
                            $reply_caption = "";
                            ...
                            if ($reply_caption) {
                                echo ajax_anchor(get_uri("enterprises/view_comment_replies/" . $comment->id), "<i data-feather='message-circle' class='icon-16'></i> " . app_lang("view") . " " . $comment->total_replies . " " . $reply_caption, array("class" => "btn btn-default btn-xs view-replies", "id" => "show-replies-" . $comment->id, "data-remove-on-success" => "#show-replies-" . $comment->id, "data-real-target" => "#reply-list-" . $comment->id));
                            }

                            //create link for reply success. trigger this link after submit any reply
                            echo ajax_anchor(get_uri("enterprises/view_comment_replies/" . $comment->id), "", array("class" => "hide", "id" => "reload-reply-list-button-" . $comment->id, "data-real-target" => "#reply-list-" . $comment->id));
...
<script>
    $(document).ready(function() {
        ...
        $(".pin-comment-button").click(function() {
            ...
            appLoader.show();
            $.ajax({
                url: "<?php echo get_uri("enterprises/pin_comment/"); ?>/" + comment_id,
                type: 'POST',
                dataType: "json",
    ...
    $(".copy-comment-link-button").click(function() {
        ...
        tempInput.value = "<?php echo get_uri("responsibility/view"); ?>/" + taskId + "/#comment-" + commentId;
