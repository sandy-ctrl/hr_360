                ...
                <div class="project-title-section">
                    <div class="page-title no-bg clearfix mb5 no-border">
                            ...
                            <h1 class="pl0">
                                ...
                                <?php if (!(get_setting("disable_access_favorite_project_option_for_clients") && $login_user->user_type == "client")) { ?>
                                    <span id="star-mark">
                                        <?php
                                        if ($is_starred) {
                                            echo view('enterprises/star/starred', array("project_id" => $project_info->id));
                                        } else {
                                            echo view('enterprises/star/not_starred', array("project_id" => $project_info->id));
                                        }
                                        ?>
                                    </span>
                                <?php } ?>
                        ...
                        <div class="project-title-button-group-section">
                            <div class="title-button-group mr0" id="project-timer-box">
                                <?php echo view("enterprises/project_title_buttons"); ?>
                            </div>
                        </div>
                    </div>
                    <ul id="project-tabs" data-bs-toggle="ajax-tab" class="nav nav-tabs rounded classic mb20 scrollable-tabs border-white" role="tablist">
                        ...
                        if ($login_user->user_type === "staff") {
                            //default tab order
                            $project_tabs = array(
                                "overview" => "enterprises/overview/" . $project_info->id,
                                "tasks_list" => "responsibility/project_tasks/" . $project_info->id,
                                "tasks_kanban" => "responsibility/project_tasks_kanban/" . $project_info->id,
                            );

                            if ($show_milestone_info) {
                                $project_tabs["milestones"] = "enterprises/milestones/" . $project_info->id;
                            }

                            if ($show_gantt_info) {
                                $project_tabs["gantt"] = "responsibility/gantt/" . $project_info->id;
                            }

                            if ($show_note_info) {
                                $project_tabs["notes"] = "enterprises/notes/" . $project_info->id;
                            }

                            if ($show_files) {
                                $project_tabs["files"] = "enterprises/files/" . $project_info->id;
                            }

                            if ($can_comment_on_projects) {
                                $project_tabs["comments"] = "enterprises/comments/" . $project_info->id;
                            }

                            if ($project_info->project_type === "client_project" && $show_customer_feedback) {
                                $project_tabs["customer_feedback"] = "enterprises/customer_feedback/" . $project_info->id;
                            }

                            if ($show_timesheet_info) {
                                $project_tabs["timesheets"] = "enterprises/timesheets/" . $project_info->id;
                            }

                            if ($show_invoice_info && $project_info->project_type === "client_project") {
                                $project_tabs["invoices"] = "enterprises/invoices/" . $project_info->id;
                                $project_tabs["payments"] = "enterprises/payments/" . $project_info->id;
                            }

                            if ($show_expense_info) {
                                $project_tabs["expenses"] = "enterprises/expenses/" . $project_info->id;
                            }

                            if ($show_contract_info && $project_info->project_type === "client_project") {
                                $project_tabs["contracts"] = "enterprises/contracts/" . $project_info->id;
                            }

                            if ($show_ticket_info && $project_info->project_type === "client_project") {
                                $project_tabs["tickets"] = "enterprises/tickets/" . $project_info->id;
                            }
                            ...
                            //default tab order
                            $project_tabs = array(
                                "overview" => "enterprises/overview_for_client/" . $project_info->id
                            );

                            if ($show_tasks) {
                                $project_tabs["tasks_list"] = "responsibility/project_tasks/" . $project_info->id;
                                $project_tabs["tasks_kanban"] = "responsibility/project_tasks_kanban/" . $project_info->id;
                            }
                            ...
                            $project_tabs["comments"] = "enterprises/customer_feedback/" . $project_info->id;
                            ...
                            if (get_setting("module_invoice") && $show_invoice_info) {
                                ...
                                if (!$left_menu || in_array("invoices", array_column($left_menu, "name"))) {
                                    $project_tabs["invoices"] = "enterprises/invoices/" . $project_info->id . "/" . $login_user->client_id;
                                }
                            }
...
if ($preview_task_id) {
    echo modal_anchor(get_uri("responsibility/view"), "", array("id" => "preview_task_link", "title" => app_lang('task_info') . " #$preview_task_id", "data-post-id" => $preview_task_id, "data-modal-lg" => "1"));
}
?>
...
<?php echo view("responsibility/sub_tasks_helper_js"); ?>