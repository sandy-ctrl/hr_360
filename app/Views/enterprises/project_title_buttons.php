        ...
        <ul class="dropdown-menu" role="menu">
            ...
            foreach ($project_statuses as $status) {
                if ($can_edit_projects && $project_info->status_id != $status->id) {
                    echo '<li role="presentation">' . ajax_anchor(get_uri("enterprises/change_status/$project_info->id/$status->id"), "<i data-feather='$status->icon' class='icon-16'></i> " . app_lang('mark_project_as') . " " . ($status->title_language_key ? app_lang($status->title_language_key) : $status->title), array("data-reload-on-success" => true, "class" => "dropdown-item")) . '</li>';
                }
            }

            if ($login_user->user_type == "staff" && $can_create_projects) {
                echo "<li role='presentation'>" . modal_anchor(get_uri("enterprises/clone_project_modal_form"), "<i data-feather='copy' class='icon-16'></i> " . app_lang('clone_project'), array("class" => "dropdown-item", "data-post-id" => $project_info->id, "title" => app_lang('clone_project'))) . " </li>";
            }

            if ($can_edit_projects) {
                echo "<li role='presentation'>" . modal_anchor(get_uri("enterprises/modal_form"), "<i data-feather='edit' class='icon-16'></i> " . app_lang('edit_project'), array("class" => "dropdown-item edit", "title" => app_lang('edit_project'), "data-post-id" => $project_info->id)) . " </li>";
            }
            ?>
...
<?php
if ($show_timmer) {
    echo view("enterprises/project_timer");
}
?>