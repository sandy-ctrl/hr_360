...
<div class="rise-chat-footer">
    <div id="chat-reply-form-dropzone" class="post-dropzone">
        <?php echo form_open(get_uri("memos/reply/1"), array("id" => "chat-message-reply-form", "class" => "general-form", "role" => "form")); ?>
    ...
    $(document).ready(function () {
        ...
        if ("<?php echo get_setting('enable_chat_via_pusher') ?>" && "<?php echo get_setting('enable_push_notification') ?>") {
            ...
            channel.bind('rise-chat-event',
                    function (data) {
                        $.ajax({
                            url: "<?php echo get_uri('memos/view_chat'); ?>",
                            type: "POST",
                            data: {
    ...
    if ("<?php echo get_setting('enable_chat_via_pusher') ?>" && "<?php echo get_setting('enable_push_notification') ?>") {
        ...
        function addKeyup() {
            $("#chat-message-reply-form").one('keyup', function (e) {
                $.ajax({
                    url: '<?php echo get_uri("memos/send_typing_indicator_to_pusher"); ?>',
                    type: "POST",
