<div class="message-container-<?php echo $message_info->id; ?>" data-total_messages="<?php echo $found_rows ?>">
                    ...
                    <div class="w-100">
                        <div class="clearfix">
                            ...
                            <span class="float-end dropdown">
                                ...
                                <ul class="dropdown-menu" role="menu">
                                    <li role="presentation"><?php echo ajax_anchor(get_uri("memos/delete_my_messages/$message_info->id"), "<i data-feather='x' class='icon-16'></i> " . app_lang('delete'), array("class" => "dropdown-item", "title" => app_lang('delete'), "data-fade-out-on-success" => ".message-container-$message_info->id")); ?> </li>
                                </ul>
                            </span>
                        </div>
                        ...
                        <div class="comment-image-box clearfix">
                            ...
                            if ($total_files) {
                                ...
                                echo anchor(get_uri("memos/download_message_files/" . $message_info->id), $download_caption, array("class" => "float-end", "title" => $download_caption));
                            }
                            ?>
                        </div>
    ...
    foreach ($replies as $reply_info) {
        ?>
        <?php echo view("memos/reply_row", array("reply_info" => $reply_info)); ?>
    <?php } ?>

    <div id="reply-form-container">
        <div id="reply-form-dropzone" class="post-dropzone">
            <?php echo form_open(get_uri("memos/reply"), array("id" => "message-reply-form", "class" => "general-form", "role" => "form")); ?>
    ...
    <script type="text/javascript">
        ...
        function loadMoreMessages(callback) {
            ...
            $.ajax({
                url: "<?php echo get_uri('memos/view_messages'); ?>",
                type: "POST",
                data: {
