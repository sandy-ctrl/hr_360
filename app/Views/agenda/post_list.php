                        ...
                        <div class="card clearfix mt15">
                            ...
                            <div class="card-body">
                                            ...
                                            <div class="flex-shrink-0">
                                                <span class="float-end dropdown">
                                                    ...
                                                    <ul class="dropdown-menu" role="menu">
                                                        <li role="presentation"><?php echo ajax_anchor(get_uri("agenda/delete/$post->id"), "<i data-feather='x' class='icon-16'></i> " . app_lang('delete'), array("class" => "dropdown-item", "title" => app_lang('delete'), "data-fade-out-on-success" => "#post-content-container-$post->id")); ?> </li>
                                                    </ul>
                                                </span>
                                            </div>
                                        <?php } ?>
                                ...
                                <div class="mb15 clearfix">
                                    <?php
                                    echo ajax_anchor(get_uri("agenda/post_reply_form/" . $post->id), "<i data-feather='corner-up-left' class='icon-16'></i> " . app_lang('reply'), array("data-real-target" => "#reply-form-container-" . $post->id, "class" => "dark"));
                                    ?>
                                    <?php
                                    $reply_caption = "";
                                    ...
                                    if ($reply_caption) {
                                        echo ajax_anchor(get_uri("agenda/view_post_replies/" . $post->id), "<i data-feather='message-circle' class='icon-16'></i> " . app_lang("view") . " " . $post->total_replies . " " . $reply_caption, array("class" => "btn btn-default btn-sm view-replies", "id" => "show-replies-button-$post->id", "data-remove-on-success" => "#show-replies-button-$post->id", "data-real-target" => "#reply-list-" . $post->id));
                                    }
                                    //create link for reply success. trigger this link after submit any reply
                                    echo ajax_anchor(get_uri("agenda/view_post_replies/" . $post->id), "", array("class" => "hide", "id" => "reload-reply-list-button-" . $post->id, "data-real-target" => "#reply-list-" . $post->id));
                                    ...
                                    if ($total_files) {
                                        ...
                                        echo anchor(get_uri("agenda/download_files/" . $post->id), $download_caption, array("class" => "float-end", "title" => $download_caption));
                                    }
                                    ?>
...
if ($result_remaining > 0) {
    ...
    <div id="loader-<?php echo $next_container_id; ?>">
        <div class="text-center ml30">
            <?php
            echo ajax_anchor(get_uri("agenda/load_more_posts/" . $next_page_offset), app_lang("load_more"), array("class" => "btn btn-default load-more mt15 p10 spinning-btn pr0", "data-remove-on-success" => "#loader-" . $next_container_id, "title" => app_lang("load_more"), "data-inline-loader" => "1", "data-real-target" => "#" . $next_container_id));
            ?>
        </div>
    </div>
    <?php
