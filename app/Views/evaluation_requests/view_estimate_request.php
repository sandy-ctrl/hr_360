                        ...
                        <ul class="dropdown-menu float-end" role="menu">
                            <?php if ($login_user->user_type == "staff") { ?>
                                <li role="presentation">
                                    <?php echo modal_anchor(get_uri("evaluation_requests/edit_estimate_request_modal_form"), "<i data-feather='edit' class='icon-16'></i> " . app_lang('edit'), array("title" => app_lang('estimate_request'), "data-post-view" => "details", "data-post-id" => $model_info->id, "class" => "dropdown-item")); ?>
                                </li>
                                <?php
                            }

                            echo view("evaluation_requests/estimate_request_status_options");
                            ?>

                            <?php if ($login_user->user_type == "staff") { ?>
                                <li role="presentation">
                                    <?php echo modal_anchor(get_uri("evaluations/modal_form"), "<i data-feather='plus-circle' class='icon-16'></i> " . app_lang('add_estimate'), array("title" => app_lang('add_estimate'), "data-post-estimate_request_id" => $model_info->id, "data-post-client_id" => $model_info->client_id, "class" => "dropdown-item")); ?>    
                                </li>
                            <?php } ?>
                        </ul>
                    </span>
            ...
            <div class="card-body">
                ...
                <?php if ($show_client_info && $model_info->company_name) { ?>
                    ...
                    <?php if ($model_info->is_lead) { ?>
                        ...
                        <?php echo (anchor(get_uri("initiates/view/" . $model_info->client_id), $model_info->company_name)); ?>
                    <?php } else { ?>
                        <span class="text-off ml15"><?php echo app_lang("client") . ": "; ?></span>
                        <?php echo (anchor(get_uri("patrons/view/" . $model_info->client_id), $model_info->company_name)); ?>
                    <?php } ?>
                ...
                if ($login_user->user_type == "staff" && $estimates) {
                    ...
                    foreach ($estimates as $estimate) {
                        ...
                        echo anchor(get_uri("evaluations/view/" . $estimate->id), get_estimate_id($estimate->id)) . $seperation;
                    }
                    ?>
                <?php } ?>  
            </div>
                ...
                <div class="p15">
                    ...
                    if ($model_info->files) {
                        ...
                        if ($total_files && $show_download_option) {
                            ...
                            echo anchor(get_uri("evaluation_requests/download_estimate_request_files/" . $model_info->id), $download_caption, array("class" => "float-end", "title" => $download_caption));
                        }
                    }
                    ?>
                </div>
...
<script type="text/javascript">
    $(document).ready(function () {
        $("#estimate-request-table").appTable({
            source: '<?php echo_uri("evaluation_requests/estimate_request_filed_list_data/" . $model_info->id) ?>',
            order: [[1, "asc"]],
            hideTools: true,
            displayLength: 100,
            columns: [
