...
if ($can_order) {
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $("body").on("click", ".cart-item-quantity-btn", function () {
                ...
                if (action === "minus" && $itemRow.find(".cart-item-quantity").attr("data-quantity") === "1") {
                    ...
                    $.ajax({
                        url: "<?php echo get_uri('outlet/change_cart_item_quantity'); ?>",
                        cache: false,
                        type: 'POST',
        ...
        function deleteCartItem($itemRow, itemId) {
            if (itemId) {
                $.ajax({
                    url: "<?php echo get_uri('outlet/delete_cart_item'); ?>",
                    cache: false,
                    type: 'POST',
                    data: {id: itemId},
                    dataType: 'json',
        ...
        function loadCartTabs() {
            ...
            $.ajax({
                url: "<?php echo get_uri("outlet/load_cart_items"); ?>",
                success: function (response) {
        ...
        window.countCartItems = function () {
            $.ajax({
                url: "<?php echo get_uri('outlet/count_cart_items'); ?>",
                cache: false,
                type: 'POST',
                dataType: 'json',
                success: function (response) {
