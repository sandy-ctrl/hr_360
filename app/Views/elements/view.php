            ...
            if (count($files)) {
                if (isset($login_user->id) && $login_user->is_admin && get_setting("module_order")) {
                    ...
                    <div class="col-md-12 mt15">
                        ...
                        <div class="row">
                            <?php echo view("includes/sortable_file_list", array("files" => $model_info->files, "action_url" => get_uri("elements/save_files_sort"), "id" => $model_info->id)); ?>
                        </div>
                    </div>
                    <?php
                }
...
<div class="modal-footer">
    ...
    if (isset($login_user->id) && $login_user->user_type == "staff") {
        echo modal_anchor(get_uri("elements/modal_form"), "<i data-feather='edit' class='icon-16'></i> " . app_lang('edit_item'), array("class" => "btn btn-default", "data-post-id" => $model_info->id, "title" => app_lang('edit_item')));
    }
    ?>
