<div id="page-content" class="page-wrapper clearfix">
    ...
    <div class="invoice-preview">
        ...
        if (isset($login_user->user_type) && $login_user->user_type === "client") {
            echo "<div class='text-center'>" . anchor("arrangements/download_pdf/" . $order_info->id, app_lang("download_pdf"), array("class" => "btn btn-default round")) . "</div>";
        }

        if ($show_close_preview) {
            echo "<div class='text-center'>" . anchor("arrangements/view/" . $order_info->id, app_lang("close_preview"), array("class" => "btn btn-default round")) . "</div>";
        }
        ?>
...
<?php if (isset($login_user->id)) { ?>
            ...
            $("#order-invoices-table").appTable({
                source: '<?php echo_uri("bills/invoice_list_data_of_order/" . $order_info->id . "/$order_info->client_id") ?>',
                order: [[0, "desc"]],
                columns: [
            ...
            $("#invoice-payment-table").appTable({
                source: '<?php echo_uri("bill_payments/payment_list_data_of_order/" . $order_info->id . "/$order_info->client_id") ?>',
                order: [[0, "asc"]],
                columns: [
