        ...
        <div class="row">
            <div class="col-md-12">
                <div class="card">
                            ...
                            <div class="title-button-group">
                                <span class="dropdown inline-block">
                                    ...
                                    <ul class="dropdown-menu float-end" role="menu">
                                        <li role="presentation"><?php echo modal_anchor(get_uri("responsibility/modal_form"), "<i data-feather='edit-2' class='icon-16'></i> " . app_lang('edit_task'), array("title" => app_lang('edit_task'), "data-post-id" => $model_info->id, "data-post-view_type" => "details", "id" => "task-details-edit-btn", "class" => "dropdown-item")); ?></li>
                                        <li role="presentation"><?php echo modal_anchor(get_uri("responsibility/modal_form"), "<i data-feather='copy' class='icon-16'></i> " . app_lang('clone_task'), array("title" => app_lang('clone_task'), "data-post-id" => $model_info->id, "data-post-is_clone" => true, "data-post-view_type" => "details", "class" => "dropdown-item")); ?></li>
                                    </ul>
                                </span>
                            </div> 
                        <?php } ?>
                    ...
                    <div class="card-body">
                        <?php echo view("responsibility/task_view_data"); ?>
                    </div>
...
<?php } else { ?>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            var browserState = {Url: "<?php echo get_uri("responsibility/view/" . $model_info->id); ?>"};
    ...
    <div class="modal-footer">
        <?php
        if ($can_edit_tasks) {
            echo modal_anchor(get_uri("responsibility/modal_form"), "<i data-feather='copy' class='icon-16'></i> " . app_lang('clone_task'), array("class" => "btn btn-default float-start", "data-post-is_clone" => true, "data-post-id" => $model_info->id, "title" => app_lang('clone_task')));
            echo modal_anchor(get_uri("responsibility/modal_form"), "<i data-feather='edit-2' class='icon-16'></i> " . app_lang('edit_task'), array("class" => "btn btn-default", "data-post-id" => $model_info->id, "title" => app_lang('edit_task')));
        }
        ?>
...
<?php
$task_link = anchor(get_uri("responsibility/view/$model_info->id"), '<i data-feather="external-link" class="icon-16 task-link-btn"></i>', array("target" => "_blank", "class" => "p15"));
?>
    ...
    $(document).ready(function () {
        ...
        Sortable.create($selector[0], {
            ...
            onUpdate: function (e) {
                ...
                //update sort indexes
                $.ajax({
                    url: '<?php echo_uri("responsibility/save_checklist_items_sort") ?>',
                    type: "POST",
        ...
        $('body').on('click', '[data-act=update-checklist-item-status-checkbox]', function () {
            ...
            $.ajax({
                url: '<?php echo_uri("responsibility/save_checklist_item_status") ?>/' + $(this).attr('data-id'),
                type: 'POST',
                dataType: 'json',
...
<?php } ?>
        ...
        function showFormAndArea(type) {
            ...
            $.ajax({
                url: '<?php echo_uri("responsibility/get_existing_dependency_tasks") ?>' + "/" + "<?php echo $task_id; ?>",
                type: "POST",
                dataType: 'json',
                success: function (response) {
                    if (response.success) {
    ...
    function applySelect2OnChecklistTemplate() {
        $("#checklist-add-item").select2({
            showSearchBox: true,
            ajax: {
                url: "<?php echo get_uri("responsibility/get_checklist_template_suggestion"); ?>",
                type: 'POST',
                dataType: 'json',
                quietMillis: 250,
    ...
    function applySelect2OnChecklistGroup() {
        $("#checklist-add-item").select2({
            showSearchBox: true,
            ajax: {
                url: "<?php echo get_uri("responsibility/get_checklist_group_suggestion"); ?>",
                type: 'POST',
                dataType: 'json',
                quietMillis: 250,
                data: function (term, page) {
...
<?php
if ($can_edit_tasks) {
    echo view("responsibility/update_task_info_script");
}
?>