        ...
        <div class="collapse navbar-collapse">
            <ul class="navbar-nav me-auto mb-lg-0">
                ...
                if (!in_array("favorite_projects", $hidden_topbar_menus) && !(get_setting("disable_access_favorite_project_option_for_clients") && $login_user->user_type == "client") && !($login_user->user_type == "staff" && get_array_value($login_user->permissions, "do_not_show_projects"))) {
                    echo view("enterprises/star/topbar_icon");
                }
                if (!in_array("favorite_clients", $hidden_topbar_menus)) {
                    echo view("patrons/star/topbar_icon");
                }
                ...
                if (has_my_open_timers()) {
                    echo view("enterprises/open_timers_topbar_icon");
                }
                ...
                <ul class="navbar-nav">
                   ...
                   /* if ($login_user->user_type == "staff") {
                            ...
                            <ul class="dropdown-menu dropdown-menu-end language-dropdown">
                                <li>
                                    ...
                                    foreach (get_language_list() as $language) {
                                        ...
                                        if ($login_user->user_type == "staff") {
                                            echo ajax_anchor(get_uri("unit_members/save_personal_language/$language"), $language_text . $language_status, array("class" => "dropdown-item clearfix", "data-reload-on-success" => "1"));
                                        } else {
                                            echo ajax_anchor(get_uri("patrons/save_personal_language/$language"), $language_text . $language_status, array("class" => "dropdown-item clearfix", "data-reload-on-success" => "1"));
                                        }
                                    }
                                    ?>
                                </li>
                    ...
                    <?php if (can_access_reminders_module()) { ?>
                        <li class="nav-item dropdown">
                            <?php echo modal_anchor(get_uri("schedules/reminders"), "<i data-feather='clock' class='icon'></i>", array("class" => "nav-link", "id" => "reminder-icon", "data-post-reminder_view_type" => "global", "title" => app_lang('reminders') . " (" . app_lang('private') . ")")); ?>
                        </li>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        messageOptions.notificationUrl = "<?php echo_uri('memos/count_notifications'); ?>";
        messageOptions.notificationStatusUpdateUrl = "<?php echo_uri('memos/update_notification_checking_status'); ?>";
        ...
        $('body').on('show.bs.dropdown', messageIcon, function () {
            messageOptions.notificationUrl = "<?php echo_uri('memos/get_notifications'); ?>";
            checkNotifications(messageOptions, true);
        });
