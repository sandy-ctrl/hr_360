<div id="invoice-status-bar">
    <div class="col-md-12 mb15">
        ...
        <?php echo (anchor(get_uri("patrons/view/" . $invoice_info->client_id), $invoice_info->company_name)); ?>
    </div>

    <?php if ($invoice_info->project_id) { ?>
        <div class="col-md-12 mb15">
            <strong><?php echo app_lang("project") . ": "; ?></strong>
            <?php echo anchor(get_uri("enterprises/view/" . $invoice_info->project_id), $invoice_info->project_title); ?>
        </div>
    <?php } ?>
    ...
    <?php if ($invoice_info->recurring_invoice_id) { ?>
        <div class="col-md-12 mb15">
            ...
            <?php echo anchor(get_uri("bills/view/" . $invoice_info->recurring_invoice_id), $invoice_info->recurring_invoice_display_id); ?>
        </div>
    <?php } ?>
    <?php if ($invoice_info->subscription_id) { ?>
        <div class="col-md-12 mb15">
            <strong><?php echo app_lang('created_from') . ": "; ?></strong>
            <?php echo anchor(get_uri("membership/view/" . $invoice_info->subscription_id), get_subscription_id($invoice_info->subscription_id)); ?>
        </div>
    <?php } ?>
    <?php if ($invoice_info->estimate_id && $login_user->is_admin) { ?>
        <div class="col-md-12 mb15">
            <strong><?php echo app_lang('created_from') . ": "; ?></strong>
            <?php echo anchor(get_uri("evaluations/view/" . $invoice_info->estimate_id), get_estimate_id($invoice_info->estimate_id)); ?>
        </div>
    <?php } ?>
