<?php echo form_open(get_uri("bills/send_invoice"), array("id" => "send-invoice-form", "class" => "general-form", "role" => "form")); ?>
    ...
    <div class="modal-body clearfix">
        <div class="container-fluid">
            ...
            <div class="form-group ml15">
                <i data-feather="check-circle" class='icon-16' style="color: #5CB85C;"></i> <?php echo app_lang('attached') . ' ' . anchor(get_uri("bills/download_pdf/" . $invoice_info->id . "/download/$user_language"), preg_replace('/[^A-Za-z0-9\-]/', '-', $invoice_info->display_id) . ".pdf", array("target" => "_blank", "id" => "attachment-url")); ?> 
            </div>
    ...
    $(document).ready(function () {
        ...
        $("#contact_id").select2().on("change", function () {
            ...
            if (contact_id) {
                ...
                appLoader.show();
                $.ajax({
                    url: "<?php echo get_uri('bills/get_send_invoice_template/' . $invoice_info->id) ?>" + "/" + contact_id + "/json",
                    dataType: "json",
                    success: function (result) {
                        if (result.success) {
                            ...
                            $("#attachment-url").attr("href", "<?php echo get_uri('bills/download_pdf/' . $invoice_info->id . '/download/'); ?>" + result.user_language);
