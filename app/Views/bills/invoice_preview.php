    ...
    <div class="invoice-preview">
        <?php if ($login_user->user_type === "client" && $invoice_total_summary->balance_due >= 1 && count($payment_methods) && !$client_info->disable_online_payment && $invoice_info->status !== "credited" && $invoice_info->status !== "cancelled") { ?>
            <div class="card d-block p15 no-border clearfix invoice-payment-button pb-0">
                ...
                foreach ($payment_methods as $payment_method) {
                    ...
                    if ($invoice_total_summary->balance_due >= get_array_value($payment_method, "minimum_payment_amount")) {
                        if ($method_type == "stripe") {
                            echo view("bills/_stripe_payment_form", $pass_variables);
                        } else if ($method_type == "paypal_payments_standard") {
                            echo view("bills/_paypal_payments_standard_form", $pass_variables);
                        } else if ($method_type == "paytm") {
                            echo view("bills/_paytm_payment_form", $pass_variables);
                        }
                ...
                <div class="float-end">
                    <?php
                    echo "<div class='text-center'>" . anchor("bills/download_pdf/" . $invoice_info->id, app_lang("download_pdf"), array("class" => "btn btn-default round")) . "</div>"
                    ?>
                </div>
        ...
        if ($show_close_preview) {
            echo "<div class='text-center'>" . anchor("bills/view/" . $invoice_info->id, app_lang("close_preview"), array("class" => "btn btn-default round")) . "</div>";
        }
        ?>
