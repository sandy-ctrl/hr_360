            ...
            <div class="tab-title clearfix">
                ...
                <div class="title-button-group">
                    ...
                    if ($can_edit_invoices) {
                        echo modal_anchor(get_uri("bills/modal_form"), "<i data-feather='plus-circle' class='icon-16'></i> " . app_lang('add_invoice'), array("class" => "btn btn-default mb0", "data-post-client_id" => $client_id, "title" => app_lang('add_invoice')));
                    }
                    ?>
                </div>
            </div>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $("#invoice-table").appTable({
            source: '<?php echo_uri("bills/invoice_list_data_of_client/" . $client_id) ?>',
            order: [[0, "desc"]],
            filterDropdown: [
                {name: "type", class: "w150", options: <?php echo $types_dropdown; ?>},
                {name: "status", class: "w150", options: <?php echo view("bills/invoice_statuses_dropdown"); ?>},
