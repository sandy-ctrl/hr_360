<div class="card rounded-bottom">
    <div class="tab-title clearfix">
        ...
        <div class="title-button-group">
            <?php
            if ($can_create_task) {
                echo modal_anchor(get_uri("responsibility/modal_form"), "<i data-feather='plus-circle' class='icon-16'></i> " . app_lang('add_task'), array("class" => "btn btn-default mb0", "data-post-client_id" => $client_id, "title" => app_lang('add_task')));
            }
            ?>
        </div>
    </div>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $("#task-table").appTable({
            source: '<?php echo_uri("responsibility/list_data/client/" . $client_id) ?>',
            order: [[0, "desc"]],
            serverSide: true,
            columns: [
