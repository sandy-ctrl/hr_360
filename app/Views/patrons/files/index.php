            ...
            <div class="tab-title clearfix no-border">
                <div class="title-button-group">
                    ...
                    if ($login_user->user_type == "staff" || ($login_user->user_type == "client" && get_setting("client_can_add_files"))) {
                        echo modal_anchor(get_uri("patrons/file_modal_form"), "<i data-feather='plus-circle' class='icon-16'></i> " . app_lang('add_files'), array("class" => "btn btn-default", "id" => "general-add-files-button", "title" => app_lang('add_files'), "data-post-client_id" => $client_id));
                    }
                    ?>
                </div>
            </div>
...
<script type="text/javascript">
    $(document).ready(function() {
        $("#client-file-table").appTable({
            source: '<?php echo_uri("patrons/files_list_data/" . $client_id) ?>',
            order: [
                [0, "desc"]
            ],
            columns: [{
        ...
        $("[data-bs-target='#file-grid-tab']").click(function() {
            ...
            if (!window.location.href.includes('file_manager') && !"<?php echo $page_view; ?>") {
                var browserState = {
                    Url: window.location.href + '/files_directory/#'
                };
