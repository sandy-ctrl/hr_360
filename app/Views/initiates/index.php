    ...
    <ul id="leads-tab" class="nav nav-tabs title" role="tablist">
        ...
        <div class="tab-title clearfix no-border">
            <div class="title-button-group">
                ...
                <?php echo modal_anchor(get_uri("labels/modal_form"), "<i data-feather='tag' class='icon-16'></i> ", array("class" => "btn btn-default", "title" => app_lang('manage_labels'), "data-post-type" => "client")); ?>
            </div>
        </div>
    </ul>
...
<script type="text/javascript">
    ...
    $("#lead-table").appTable({
    source: '<?php echo_uri("initiates/list_data") ?>',
            serverSide: true,
...
<?php } ?>
                {name: "status", class: "w200", options: <?php echo view("initiates/lead_statuses"); ?>},
            {name: "label_id", class: "w200", options: <?php echo $labels_dropdown; ?>},

            {name: "source", class: "w200", options: <?php echo view("initiates/lead_sources"); ?>} ,
...
<?php echo view("initiates/update_lead_status_script"); ?>