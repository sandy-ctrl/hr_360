<script type="text/javascript">
    ...
    getMissedRemindersCount = function () {
        $.ajax({
            url: "<?php echo get_uri("schedules/count_missed_reminders") ?>",
            dataType: "json",
    ...
    getReminders = function () {
        $.ajax({
            url: "<?php echo get_uri("schedules/get_reminders_for_current_user") ?>",
            dataType: "json",
            success: function (result) {
    ...
    $(document).ready(function () {
        ...
        function showReminder(reminder) {
            ...
            if (AppHelper.https === "1") {
                ...
                var data = {
                    ...
                    url_attributes: "href='#' data-act='ajax-modal' data-title='<?php echo app_lang("reminder_details"); ?>' data-action-url='<?php echo get_uri("schedules/reminder_view"); ?>' data-post-id='" + reminder.id + "'", //open small modal for action
                    isReminder: true,
                ...
                //app notification
                var detailsDom = '<a class="color-white" data-act="ajax-modal" data-title="<?php echo app_lang("reminder_details"); ?>" data-action-url="<?php echo get_uri("schedules/reminder_view"); ?>" data-post-id="' + reminder.id + '" href="javascript:;" ><?php echo app_lang("details"); ?></a>';
        ...
        $("body").on("click", "[data-act='snooze-reminder']", function () {
            appLoader.show();
            $.ajax({url: "<?php echo get_uri('schedules/snooze_reminder') ?>",
                type: 'POST',
                dataType: 'json',
        ...
        $("body").on("click", "[data-act='dismiss-reminder']", function () {
            ...
            $.ajax({url: "<?php echo get_uri('schedules/save_reminder_status') ?>/" + $(this).attr('data-id'),
                type: 'POST',
                dataType: 'json',
                data: {value: "shown"},
                success: function (result) {
