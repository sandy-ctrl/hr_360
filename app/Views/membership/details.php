                ...
                <div class="clearfix">
                    <?php if (!$has_item_in_this_subscription && $subscription_info->status != "active") { ?>
                        <div class="float-start mt20 ml15" id="subscription-add-item-btn">
                            <?php echo modal_anchor(get_uri("membership/item_modal_form"), "<i data-feather='plus-circle' class='icon-16'></i> " . app_lang('add_item'), array("class" => "btn btn-info text-white", "title" => app_lang('add_item'), "data-post-subscription_id" => $subscription_info->id)); ?>
                        </div>
                    <?php } ?>
                    <div class="float-end pr15" id="subscription-total-section">
                        <?php echo view("membership/subscription_total_section", array("subscription_id" => $subscription_info->id, "can_edit_subscriptions" => $can_edit_subscriptions)); ?>
                    </div>
                </div>
...
<?php } ?>
        ...
        $("#subscription-item-table").appTable({
            source: '<?php echo_uri("membership/item_list_data/" . $subscription_info->id . "/") ?>',
            order: [[0, "asc"]],
            hideTools: true,
            displayLength: 100,
            columns: [
