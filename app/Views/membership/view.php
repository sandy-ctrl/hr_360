            ...
            <div class="col-md-12">
                <div clss="subscription-title-section">
                                ...
                                <ul class="dropdown-menu" role="menu">
                                    ...
                                    <?php if ($can_edit_subscriptions) { ?>
                                        <?php if ($subscription_status !== "cancelled" && $subscription_status !== "active" && !$subscription_info->stripe_subscription_id && get_setting("enable_stripe_subscription")) { ?>
                                            <li role="presentation"><?php echo modal_anchor(get_uri("membership/activate_as_stripe_subscription_modal_form/" . $subscription_info->id), "<i data-feather='credit-card' class='icon-16'></i> " . app_lang('activate_as_stripe_subscription'), array("title" => app_lang('activate_as_stripe_subscription'), "data-post-id" => $subscription_info->id, "class" => "dropdown-item")); ?> </li>
                                        <?php } ?>

                                        <?php if ($subscription_status == "draft" && $subscription_status !== "cancelled" && $subscription_info->type === "app") { ?>
                                            <li role="presentation"><?php echo modal_anchor(get_uri("membership/activate_as_internal_subscription_modal_form/" . $subscription_info->id), "<i data-feather='check' class='icon-16'></i> " . app_lang('activate_as_internal_subscription'), array("title" => app_lang("activate_as_internal_subscription"), "data-post-id" => $subscription_info->id, "class" => "dropdown-item")); ?> </li>
                                        <?php } else if ($subscription_status == "pending" || $subscription_status == "active") { ?>
                                            <li role="presentation"><?php echo js_anchor("<i data-feather='x' class='icon-16'></i> " . app_lang('cancel_subscription'), array('title' => app_lang('cancel_subscription'), "data-action-url" => get_uri("membership/update_subscription_status/" . $subscription_info->id . "/cancelled"), "data-action" => "delete-confirmation", "data-reload-on-success" => "1", "class" => "dropdown-item")); ?> </li>
                                        <?php } ?>

                                        <?php if ($subscription_status !== "active" && $subscription_status !== "cancelled") { ?>
                                            <li role="presentation"><?php echo modal_anchor(get_uri("membership/modal_form"), "<i data-feather='edit' class='icon-16'></i> " . app_lang('edit_subscription'), array("title" => app_lang('edit_subscription'), "data-post-id" => $subscription_info->id, "role" => "menuitem", "tabindex" => "-1", "class" => "dropdown-item")); ?> </li>
                                        <?php } ?>
                                    <?php } ?>
                    ...
                    <ul id="subscription-tabs" data-bs-toggle="ajax-tab" class="nav nav-pills rounded classic mb20 scrollable-tabs border-white nav-pills-restructre" role="tablist" style="background: transparent;">
                        ...
                        <?php } ?>
                        <li><a role="presentation" data-bs-toggle="tab" href="<?php echo_uri("membership/tasks/" . $subscription_info->id); ?>" data-bs-target="#subscription-tasks-section"><?php echo app_lang('tasks'); ?></a></li>
                    </ul>
                </div>
