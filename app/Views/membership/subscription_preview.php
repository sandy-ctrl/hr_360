    ...
    <div class="invoice-preview">
        <?php if ($login_user->user_type === "client") { ?>
                    ...
                    <div class="title-button-group">
                        <span class="dropdown inline-block mt10">
                            ...
                            <ul class="dropdown-menu" role="menu">
                                <li role="presentation"><?php echo ajax_anchor(get_uri("membership/update_subscription_status/" . $subscription_info->id . "/cancelled"), "<i data-feather='x' class='icon-16'></i> " . app_lang('cancel_subscription'), array("class" => "dropdown-item", "title" => app_lang('cancel_subscription'), "data-reload-on-success" => "1")); ?> </li>
                            </ul>
                        </span>
                    </div>
                <?php } ?>
            ...
            <div class="pt15 bg-white mb15">
                <?php echo view("membership/subscription_recurring_info_bar"); ?>
            </div>
                    ...
                    foreach ($payment_methods as $payment_method) {
                        ...
                        if ($subscription_total_summary->balance_due >= get_array_value($payment_method, "minimum_payment_amount")) {
                            if ($method_type == "stripe") {
                                echo view("membership/_stripe_payment_form", $pass_variables);
                            }
                        }
                    }
                    ?>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $("#subscription-invoices-table").appTable({
            source: '<?php echo_uri("bills/invoice_list_data_of_subscription/$subscription_info->id/$client_info->id") ?>',
            order: [[0, "desc"]],
