    ...
    function get_search_suggestion() {
        ...
        if ($search && $search_field) {
            ...
            if ($search_field == "task") { //task
                ...
                $result = $this->Responsibility_model->get_search_suggestion($search, $options)->getResult();
            } else if ($search_field == "project") { //project
                if (!$this->can_manage_all_projects()) {
                    $options["user_id"] = $this->login_user->id;
                }
                $result = $this->Enterprises_model->get_search_suggestion($search, $options)->getResult();
                ...
                $result = $this->Patrons_model->get_search_suggestion($search, $options)->getResult();
