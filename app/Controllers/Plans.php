...
class Plans extends Security_Controller
{
    ...
    function index()
    {
        ...
        if ($this->login_user->user_type === "staff") {
            ...
            return $this->template->rander("plans/index", $view_data);
        } else {
            //client view
            ...
            return $this->template->rander("patrons/proposals/client_portal", $view_data);
        }
    }
    ...
    function yearly()
    {
        return $this->template->view("plans/yearly_proposals");
    }
    ...
    function modal_form()
    {
        ...
        $client_info = $this->Patrons_model->get_one($view_data['model_info']->client_id);
        ...
        return $this->template->view('plans/modal_form', $view_data);
    }

    private function get_proposal_clients_and_leads_dropdown()
    {
        $clients_dropdown = array("" => "-");
        $clients = $this->Patrons_model->get_all_where(array("deleted" => 0), 0, 0, "is_lead")->getResult();
    ...
    private function _make_row($data, $custom_fields)
    {
        $proposal_url = "";
        if ($this->login_user->user_type == "staff") {
            $proposal_url = anchor(get_uri("plans/view/" . $data->id), get_proposal_id($data->id));
        } else {
            //for client client
            $proposal_url = anchor(get_uri("plans/preview/" . $data->id), get_proposal_id($data->id));
        }

        $client = anchor(get_uri("patrons/view/" . $data->client_id), $data->company_name ? $data->company_name : "");
        if ($data->is_lead) {
            $client = anchor(get_uri("initiates/view/" . $data->client_id), $data->company_name ? $data->company_name : "");
        }
        ...
        if (get_setting("enable_comments_on_proposals") && $data->status !== "draft") {
            $comment_link = modal_anchor(get_uri("plans/comment_modal_form"), $total_comments . " <i data-feather='message-circle' class='icon-16'></i>", array("class" => "text-muted", "title" => app_lang("proposal") . " #" . $data->id . " " . app_lang("comments"), "data-post-proposal_id" => $data->id));
        }
        ...
        if ($this->_is_proposal_editable($data)) {
            $edit = modal_anchor(get_uri("plans/modal_form"), "<i data-feather='edit' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('edit_proposal'), "data-post-id" => $data->id));
        }
        ...
        $row_data[] = anchor(get_uri("offer/preview/" . $data->id . "/" . $data->public_key), "<i data-feather='external-link' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('proposal') . " " . app_lang("url"), "target" => "_blank"))
            . $edit
            . js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete_proposal'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("plans/delete"), "data-action" => "delete-confirmation"));
    ...
    function view($proposal_id = 0)
    {
        ...
        if ($proposal_id) {
            ...
            if ($view_data) {
                ...
                return $this->template->rander("plans/view", $view_data);
            } else {
                show_404();
            }
        }
    ...
    private function _get_proposal_total_view($proposal_id = 0)
    {
        ...
        $view_data["proposal_id"] = $proposal_id;
        $view_data["is_proposal_editable"] = $this->_is_proposal_editable($proposal_id);
        return $this->template->view('plans/proposal_total_section', $view_data);
    }
    ...
    function discount_modal_form()
    {
        ...
        return $this->template->view('plans/discount_modal_form', $view_data);
    }
    ...
    function item_modal_form()
    {
        ...
        $view_data['proposal_id'] = $proposal_id;
        return $this->template->view('plans/item_modal_form', $view_data);
    }
    ...
    private function _make_item_row($data)
    {
        ...
        return array(
            ...
            modal_anchor(get_uri("plans/item_modal_form"), "<i data-feather='edit' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('edit_proposal'), "data-post-id" => $data->id, "data-post-proposal_id" => $data->proposal_id))
                . js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("plans/delete_item"), "data-action" => "delete"))
        );
    }
    ...
    function preview($proposal_id = 0, $show_close_preview = false, $is_editor_preview = false)
    {
        ...
        if ($proposal_id) {
            ...
            if ($is_editor_preview) {
                ...
                return $this->template->view("plans/proposal_preview", $view_data);
            } else {
                return $this->template->rander("plans/proposal_preview", $view_data);
            }
        } else {
            show_404();
        }
    ...
    function get_proposal_status_bar($proposal_id = 0)
    {
        ...
        $view_data['proposal_status_label'] = $this->_get_proposal_status_label($view_data["proposal_info"]);
        $view_data['total_read_count'] = $this->Schedule_tracker_model->total_read_count(array("context" => "proposal", "context_id" => $proposal_id));

        return $this->template->view('plans/proposal_status_bar', $view_data);
    }
    ...
    function send_proposal_modal_form($proposal_id)
    {
        ...
        if ($proposal_id) {
            ...
            return $this->template->view('plans/send_proposal_modal_form', $view_data);
        } else {
            show_404();
        }
    }
    ...
    function get_send_proposal_template($proposal_id = 0, $contact_id = 0, $return_type = "", $proposal_info = "", $contact_info = "")
    {
        ...
        $parser_data["PROPOSAL_URL"] = get_uri("plans/preview/" . $proposal_info->id);
    ...
    function send_proposal()
    {
        ...
        $Schedule_tracker_model = model("App\Models\Schedule_tracker_model");
        $Schedule_tracker_model->ci_save($event_tracker_data);
    ...
    function editor($proposal_id = 0)
    {
        validate_numeric_value($proposal_id);
        $view_data['proposal_info'] = $this->Proposals_model->get_details(array("id" => $proposal_id))->getRow();
        return $this->template->view("plans/proposal_editor", $view_data);
    }
    ...
    function tasks($proposal_id)
    {
        ...
        return $this->template->view("plans/tasks/index", $view_data);
    }
    ...
    function email_view_report($proposal_id)
    {
        ...
        $tracking_info = $this->Schedule_tracker_model->get_details($options)->getResult();
        ...
        return $this->template->view("plans/email_view_report", $view_data);
    }
    ...
    function comment_modal_form()
    {
        ...
        return $this->template->view('plans/comment_form', $view_data);
    }
    ...
    function save_comment()
    {
        ...
        if ($comment_id) {
            ...
            $comment_view = $this->template->view("plans/comment_row", $view_data, true);
