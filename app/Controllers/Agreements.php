...
class Agreements extends Security_Controller {
    ...
    function index() {
        ...
        if ($this->login_user->user_type === "staff") {
            ...
            return $this->template->rander("patrons/contracts/client_portal", $view_data);
        }
    }

    //load the yearly view of contract list
    function yearly() {
        return $this->template->view("agreements/yearly_contracts");
    }
    ...
    private function _make_row($data, $custom_fields) {
        $contract_id = "";
        if ($this->login_user->user_type == "staff") {
            $contract_id = anchor(get_uri("agreements/view/" . $data->id), get_contract_id($data->id));
        } else {
            //for client
            $contract_id = anchor(get_uri("agreements/preview/" . $data->id), get_contract_id($data->id));
        }

        $contract_url = "";
        if ($this->login_user->user_type == "staff") {
            $contract_url = anchor(get_uri("agreements/view/" . $data->id), $data->title);
        } else {
            //for client
            $contract_url = anchor(get_uri("agreements/preview/" . $data->id), $data->title);
        }
        ...
        if ($this->_is_contract_editable($data)) {
            $edit = modal_anchor(get_uri("agreements/modal_form"), "<i data-feather='edit' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('edit_contract'), "data-post-id" => $data->id));
        }
        ...
        $row_data[] = anchor(get_uri("contract/preview/" . $data->id . "/" . $data->public_key), "<i data-feather='external-link' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('contract') . " " . app_lang("url"), "target" => "_blank"))
                . $edit
                . js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete_contract'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("agreements/delete"), "data-action" => "delete-confirmation"));
    ...
    private function _get_contract_total_view($contract_id = 0) {
        ...
        $view_data["contract_id"] = $contract_id;
        $view_data["is_contract_editable"] = $this->_is_contract_editable($contract_id);
        return $this->template->view('agreements/contract_total_section', $view_data);
    }
    ...
    function discount_modal_form() {
        ...
        return $this->template->view('agreements/discount_modal_form', $view_data);
    }
    ...
    function item_modal_form() {
        ...
        $view_data['contract_id'] = $contract_id;
        return $this->template->view('agreements/item_modal_form', $view_data);
    }
    ...
    private function _make_item_row($data) {
        ...
        return array(
            ...
            modal_anchor(get_uri("agreements/item_modal_form"), "<i data-feather='edit' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('edit_contract'), "data-post-id" => $data->id, "data-post-contract_id" => $data->contract_id))
            . js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("agreements/delete_item"), "data-action" => "delete"))
        );
    }
    ...
    function preview($contract_id = 0, $show_close_preview = false, $is_editor_preview = false) {
        ...
        if ($contract_id) {
            ...
            if ($is_editor_preview) {
                ...
                return $this->template->view("agreements/contract_preview", $view_data);
            } else {
                return $this->template->rander("agreements/contract_preview", $view_data);
            }
        } else {
            show_404();
        }
    ...
    function get_contract_status_bar($contract_id = 0) {
        ...
        $view_data['contract_status_label'] = $this->_get_contract_status_label($view_data["contract_info"]);
        return $this->template->view('agreements/contract_status_bar', $view_data);
    }
    ...
    function send_contract_modal_form($contract_id) {
        ...
        if ($contract_id) {
            ...
            return $this->template->view('agreements/send_contract_modal_form', $view_data);
        } else {
            show_404();
        }
    }
    ...
    function get_send_contract_template($contract_id = 0, $contact_id = 0, $return_type = "", $contract_info = "", $contact_info = "") {
        ...
        $parser_data["CONTRACT_URL"] = get_uri("agreements/preview/" . $contract_info->id);
    ...
    function editor($contract_id = 0) {
        $this->access_only_allowed_members();
        $view_data['contract_info'] = $this->Contracts_model->get_details(array("id" => $contract_id))->getRow();
        return $this->template->view("agreements/contract_editor", $view_data);
    }
