...
class Notations extends Security_Controller {
    ...
    protected function validate_access_to_note($note_info, $edit_mode = false) {
        ...
        if ($note_info->is_public) {
            ...
            if ($note_info->client_id) {
                ...
                $client_info = $this->Patrons_model->get_one($note_info->client_id);
    ...
    function index() {
        ...
        return $this->template->rander("notations/index");
    }

    function modal_form() {
        $view_data['model_info'] = $this->Notations_model->get_one($this->request->getPost('id'));
        ...
        return $this->template->view('notations/modal_form', $view_data);
    }
    ...
    function save() {
        ...
        if ($id) {
            $note_info = $this->Notations_model->get_one($id);
        ...
        $save_id = $this->Notations_model->ci_save($data, $id);
        if ($save_id) {
    ...
    function delete() {
        ...
        if ($this->Notations_model->delete($id)) {
            //delete the files
    ...
    function list_data($type = "", $id = 0) {
        ...
        $list_data = $this->Notations_model->get_details($options)->getResult();
        $result = array();
    ...
    private function _row_data($id) {
        $options = array("id" => $id);
        $data = $this->Notations_model->get_details($options)->getRow();
    ...
    private function _make_row($data) {
        ...
        $title = modal_anchor(get_uri("notations/view/" . $data->id), $public_icon . $data->title, array("title" => app_lang('note'), "data-post-id" => $data->id));
        ...
        if ($data->files) {
            ...
            if (count($files)) {
                foreach ($files as $key => $value) {
                    ...
                    $file_download_link = anchor(get_uri("notations/download_files/" . $data->id), "<i data-feather='download-cloud' class='icon-14'></i>", array("title" => app_lang("download"), "class" => "file-list-view file-download"));
                    $files_link .= js_anchor("<i data-feather='$link' class='icon-14'></i>", array('title' => "", "data-toggle" => "app-modal", "data-sidebar" => "0", "class" => "file-list-view", "title" => remove_file_prefix($file_name), "data-url" => get_uri("notations/file_preview/" . $data->id . "/" . $key)));
                }
            }
        }

        //only creator and admin can edit/delete notes
        $actions = modal_anchor(get_uri("notations/view/" . $data->id), "<i data-feather='cloud-lightning' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('note_details'), "data-modal-title" => app_lang("note"), "data-post-id" => $data->id));
        if ($data->created_by == $this->login_user->id || $this->login_user->is_admin || $data->client_id) {
            $actions = modal_anchor(get_uri("notations/modal_form"), "<i data-feather='edit' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('edit_note'), "data-post-id" => $data->id))
                    . js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete_note'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("notations/delete"), "data-action" => "delete-confirmation"));
        }
    ...
    function view() {
        ...
        $model_info = $this->Notations_model->get_details(array("id" => $this->request->getPost('id')))->getRow();

        $this->validate_access_to_note($model_info);

        $view_data['model_info'] = $model_info;
        return $this->template->view('notations/view', $view_data);
    }
    ...
    function file_preview($id = "", $key = "") {
        if ($id) {
            ...
            return $this->template->view("notations/file_preview", $view_data);
        } else {
            show_404();
        }
    }
    ...
    function download_files($id) {
        ...
        $files = $this->Notations_model->get_one($id)->files;
