...
class Schedules extends Security_Controller {
    ...
    function modal_form() {
        ...
        $model_info = $this->Schedules_model->get_one($event_id);
        ...
        $client_info = $this->Patrons_model->get_one($model_info->client_id);
        ...
        return $this->template->view('schedules/modal_form', $view_data);
    }
    ...
    function save() {
        ...
        if ($recurring) {
            $no_of_cycles = $this->Schedules_model->get_no_of_cycles($repeat_type, $no_of_cycles);
        ...
        if ($id && !$this->login_user->is_admin) {
            $event_info = $this->Schedules_model->get_one($id);
        ...
        $save_id = $this->Schedules_model->ci_save($data, $id);
        if ($save_id) {
            ...
            if ($type === "reminder") {
                $reminder_info = $this->Schedules_model->get_one($save_id);
    ...
    function delete() {
        ...
        if ($this->Schedules_model->delete($id)) {
    ...
    function calendar_events($filter_values = "", $event_label_id = 0, $client_id = 0) {
        ...
        if (in_array("events", $filter_values_array)) {
            ...
            $list_data_of_events = $this->Schedules_model->get_details($options_of_events)->getResult();
            ...
            foreach ($list_data_of_events as $data) {
                ...
                if ($data->recurring) {
                    $no_of_cycles = $this->Schedules_model->get_no_of_cycles($data->repeat_type, $data->no_of_cycles);
        ...
        if (in_array("leave", $filter_values_array) && $this->login_user->user_type == "staff") {
            ...
            $list_data_of_leaves = $this->Vacate_applications_model->get_list($options_of_leaves)->getResult();
        ...
        if (in_array("project_deadline", $filter_values_array) || in_array("project_start_date", $filter_values_array)) {
            ...
            if (in_array("project_start_date", $filter_values_array)) {
                ...
                $list_data_of_projects = $this->Enterprises_model->get_details($options)->getResult();
        ...
        if ($this->login_user->user_type == "staff" && (in_array("task_deadline", $filter_values_array) || in_array("task_start_date", $filter_values_array))) {
            ...
            if (in_array("task_deadline", $filter_values_array)) {
                ...
                $list_data_of_tasks = $this->Responsibility_model->get_details($options)->getResult();
    ...
    function view() {
        ...
        return $this->template->view('schedules/view', $view_data);
    }

    private function _make_view_data($encrypted_event_id, $cycle = "0") {
        $event_id = decode_id($encrypted_event_id, "event_id");

        $model_info = $this->Schedules_model->get_details(array("id" => $event_id))->getRow();
        ...
        if ($event_id && $model_info->id) {
            ...
            $status_confirm = modal_anchor(get_uri("schedules/save_event_status/"), "<i data-feather='check-circle' class='icon-16'></i> " . app_lang('confirm'), array("class" => "btn btn-success float-start", "data-post-encrypted_event_id" => $encrypted_event_id, "title" => app_lang('event_details'), "data-post-status" => "confirmed", "data-post-editable" => "1"));
            $status_reject = modal_anchor(get_uri("schedules/save_event_status/"), "<i data-feather='x-circle' class='icon-16'></i> " . app_lang('reject'), array("class" => "btn btn-danger float-start", "data-post-encrypted_event_id" => $encrypted_event_id, "title" => app_lang('event_details'), "data-post-status" => "rejected", "data-post-editable" => "1"));
    ...
    private function _get_confirmed_and_rejected_users_list($confirmed_by_array, $rejected_by_array) {
        ...
        $response_by_users = $this->Schedules_model->get_response_by_users(($confirmed_by_array + $rejected_by_array));
    ...
    function save_event_status() {
        ...
        $this->Schedules_model->save_event_status($event_id, $user_id, $status);
    ...
    function google_calendar_settings_modal_form() {
        if (get_setting("enable_google_calendar_api") && (get_setting("google_calendar_authorized") || get_setting('user_' . $this->login_user->id . '_google_calendar_authorized'))) {
            ...
            return $this->template->view("schedules/google_calendar_settings_modal_form", array("calendar_ids" => $calendar_ids));
        }
    }
    ...
    function file_preview($id = "", $key = "") {
        if ($id) {
            ...
            return $this->template->view("schedules/file_preview", $view_data);
        } else {
            show_404();
        }
    }
    ...
    function reminders_list_data($type = "", $reminder_context = "", $reminder_context_id = 0) {
        ...
        $list_data = $this->Schedules_model->get_details($options)->getResult();
    ...
    private function _make_reminder_row($data = array()) {
        ...
        $time_value = view("schedules/event_time", array("model_info" => $data, "is_reminder" => true));
        ...
        $delete = '<li role="presentation">' . js_anchor("<i data-feather='x' class='icon-16'></i>" . app_lang('delete'), array('title' => app_lang('delete_reminder'), "class" => "delete dropdown-item reminder-action", "data-id" => $data->id, "data-post-id" => $data->id, "data-action-url" => get_uri("schedules/delete"), "data-action" => "delete", "data-undo" => "0")) . '</li>';

        $status = '<li role="presentation">' . js_anchor("<i data-feather='check-circle' class='icon-16'></i> " . app_lang('mark_as_done'), array('title' => app_lang('mark_as_done'), "class" => "dropdown-item reminder-action", "data-action-url" => get_uri("schedules/save_reminder_status/$data->id/done"), "data-action" => "delete", "data-undo" => "0")) . '</li>';
        if ($data->reminder_status === "done" || $data->reminder_status === "shown") {
            $status = "";
        }
        ...
        if ($missed_reminder_class) {
            ...
            $options = js_anchor("<i data-feather='check-circle' class='icon-16'></i>", array('title' => app_lang('mark_as_done'), "class" => "reminder-action p10", "data-action-url" => get_uri("schedules/save_reminder_status/$data->id/done"), "data-action" => "delete", "data-undo" => "0"));
        }
    ...
    private function can_access_this_reminder($reminder_info) {
        ...
        if ($reminder_info->share_with) {
            ...
            $shared_users = $this->Schedules_model->get_share_with_users_of_event($reminder_info)->getResult();
    ...
    function save_reminder_status($id = 0, $status = "") {
        ...
        $reminder_info = $this->Schedules_model->get_one($id);
    ...
    function count_missed_reminders() {
        $this->can_create_reminders();
        $reminders = $this->Schedules_model->count_missed_reminders($this->login_user->id, $this->login_user->notification_checked_at);
