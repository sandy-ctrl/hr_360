...
class Responsibility extends Security_Controller {
    ...
    private function can_edit_tasks($_task = null) {
        $task_info = is_object($_task) ? $_task : $this->Responsibility_model->get_one($_task); //the $_task is either task id or task info
        $permissions = $this->login_user->permissions;
    ...
    function modal_form() {
        ...
        $model_info = $this->Responsibility_model->get_one($id);
        ...
        if ($add_type == "multiple" && $last_id) {
            ...
            $model_info = $this->Responsibility_model->get_one($last_id);
        }
        ...
        $view_data['has_sub_task'] = count($this->Responsibility_model->get_all_where(array("parent_task_id" => $id, "deleted" => 0))->getResult());
        ...
        return $this->template->view('responsibility/modal_form', $view_data);
    }
    ...
    private function _get_task_related_dropdowns($context = "", $context_id = 0, $return_empty_context = false) {
        ...
        $statuses = $this->Responsibility_status_model->get_details($task_status_options)->getResult();
        ...
        //properties dropdown 
        $priorities = $this->Responsibility_priority_model->get_details()->getResult();
        ...
        if ($context == "project" && !$return_empty_context) {
            ...
            $projects = $this->Enterprises_model->get_details($project_options)->getResult();
        ...
        if ($context === "client" && !$return_empty_context) {
            ...
            $clients = $this->Patrons_model->get_details($options)->getResult();
        ...
        if ($context === "lead" && !$return_empty_context) {
            ...
            $leads = $this->Patrons_model->get_details($options)->getResult();
        ...
        if ($context === "invoice" && !$return_empty_context) {
            ...
            $invoices = $this->Bills_model->get_all_where(array("deleted" => 0))->getResult();
        ...
        if ($context === "estimate" && !$return_empty_context) {
            ...
            $estimates = $this->Evaluations_model->get_details($options)->getResult();
        ...
        if ($context === "order" && !$return_empty_context) {
            //get orders dropdown
            $orders = $this->Arrangements_model->get_all_where(array("deleted" => 0))->getResult();
        ...
        if ($context === "subscription" && !$return_empty_context) {
            ...
            $subscriptions = $this->Membership_model->get_all_where(array("deleted" => 0))->getResult();
        ...
        if ($context === "expense" && !$return_empty_context) {
            ...
            $expenses = $this->Budgets_model->get_all_where(array("deleted" => 0))->getResult();
        ...
        if ($context === "ticket" && !$return_empty_context) {
            ...
            $tickets = $this->Tokens_model->get_details($options)->getResult();
    ...
    private function _get_project_deadline_for_task($project_id = 0) {
        ...
        $project_deadline = $this->Enterprises_model->get_one($project_id)->deadline;
    ...
    function save() {
        ...
        if ($id) {
            $task_info = $this->Responsibility_model->get_one($id);
            if (!$this->can_edit_tasks($task_info)) {
                app_redirect("forbidden");
            }
        } else {
        ...
        if (!$id) {
            ...
            $data["sort"] = $this->Responsibility_model->get_next_sort_value($project_id, $status_id);
        }
        ...
        $save_id = $this->Responsibility_model->ci_save($data, $id);
        if ($save_id) {
            ...
            if ($is_clone && $main_task_id) {
                ...
                if ($this->request->getPost("copy_sub_tasks")) {
                    $sub_tasks = $this->Responsibility_model->get_all_where(array("parent_task_id" => $main_task_id, "deleted" => 0))->getResult();
                    foreach ($sub_tasks as $sub_task) {
                        ...
                        $sub_task_data["sort"] = $this->Responsibility_model->get_next_sort_value($sub_task_data["project_id"], $sub_task_data['status_id']);

                        $sub_task_save_id = $this->Responsibility_model->ci_save($sub_task_data);
            ...
            if ($next_recurring_date) {
                ...
                $this->Responsibility_model->save_reminder_date($recurring_task_data, $save_id);
            }

            // if created from ticket then save the task id
            if ($ticket_id) {
                $data = array("task_id" => $save_id);
                $this->Tokens_model->ci_save($data, $ticket_id);
            }
    ...
    function list_data($context = "", $context_id = 0) {
        ...
        $result = $this->Responsibility_model->get_details($all_options);
    ...
    private function _row_data($id) {
        ...
        $data = $this->Responsibility_model->get_details($options)->getRow();
    ...
    private function _make_row($data, $custom_fields, $show_time_with_task, $tasks_edit_permissions, $tasks_status_edit_permissions) {
        ...
        $title .= modal_anchor(get_uri("responsibility/view"), $data->title . $icon, array("title" => app_lang('task_info') . " #$data->id", "data-post-id" => $data->id, "data-search" => $sub_task_search_column, "class" => $unread_comments_class, "data-modal-lg" => "1"));
        ...
        $context_title = "";
        if ($data->project_id) {
            $context_title = anchor(get_uri("enterprises/view/" . $data->project_id), $data->project_title ? $data->project_title : "");
        } else if ($data->client_id) {
            $context_title = anchor(get_uri("patrons/view/" . $data->client_id), $data->company_name ? $data->company_name : "");
        } else if ($data->lead_id) {
            $context_title = anchor(get_uri("initiates/view/" . $data->lead_id), $data->company_name ? $data->company_name : "");
        } else if ($data->invoice_id) {
            $context_title = anchor(get_uri("bills/view/" . $data->invoice_id), $data->invoice_display_id);
        } else if ($data->estimate_id) {
            $context_title = anchor(get_uri("evaluations/view/" . $data->estimate_id), get_estimate_id($data->estimate_id));
        } else if ($data->order_id) {
            $context_title = anchor(get_uri("arrangements/view/" . $data->order_id), get_order_id($data->order_id));
        } else if ($data->contract_id) {
            $context_title = anchor(get_uri("agreements/view/" . $data->contract_id), $data->contract_title ? $data->contract_title : "");
        } else if ($data->proposal_id) {
            $context_title = anchor(get_uri("plans/view/" . $data->proposal_id), get_proposal_id($data->proposal_id));
        } else if ($data->subscription_id) {
            $context_title = anchor(get_uri("membership/view/" . $data->subscription_id), $data->subscription_title ? $data->subscription_title : "");
        } else if ($data->expense_id) {
            $context_title = modal_anchor(get_uri("budgets/expense_details"), ($data->expense_title ? $data->expense_title : format_to_date($data->expense_date, false)), array("title" => app_lang("expense_details"), "data-post-id" => $data->expense_id, "data-modal-lg" => "1"));
        } else if ($data->ticket_id) {
            $context_title = anchor(get_uri("tokens/view/" . $data->ticket_id), $data->ticket_title ? $data->ticket_title : "");
        }
        ...
        if (get_array_value($tasks_edit_permissions, $data->id)) {
            $options .= modal_anchor(get_uri("responsibility/modal_form"), "<i data-feather='edit' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('edit_task'), "data-post-id" => $data->id));
        }
        if ($this->can_delete_tasks($data)) {
            $options .= js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete_task'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("responsibility/delete"), "data-action" => "delete-confirmation"));
        }
    ...
    function delete() {
        ...
        $id = $this->request->getPost('id');
        $info = $this->Responsibility_model->get_one($id);

        if (!$this->can_delete_tasks($info)) {
            app_redirect("forbidden");
        }

        if ($this->Responsibility_model->delete_task_and_sub_items($id)) {
    ...
    private function check_sub_tasks_statuses($status_id = 0, $parent_task_id = 0) {
        ...
        $sub_tasks = $this->Responsibility_model->get_details(array("parent_task_id" => $parent_task_id, "deleted" => 0))->getResult();
    ...
    private function _make_checklist_item_row($data, $return_type = "row") {
        ...
        $delete = ajax_anchor(get_uri("responsibility/delete_checklist_item/$data->id"), "<div class='float-end'><i data-feather='x' class='icon-16'></i></div>", array("class" => "delete-checklist-item", "title" => app_lang("delete_checklist_item"), "data-fade-out-on-success" => "#checklist-item-row-$data->id"));
        if (!$this->can_edit_tasks($data->task_id)) {
            $delete = "";
        }
    ...
    private function _make_sub_task_row($data, $return_type = "row") {
        ...
        $title = anchor(get_uri("responsibility/view/$data->id"), $data->title, array("class" => "font-13", "target" => "_blank"));
    ...
    function view($task_id = 0) {
        ...
        $model_info = $this->Responsibility_model->get_details(array("id" => $task_id))->getRow();
        ...
        $sub_tasks = $this->Responsibility_model->get_details(array("parent_task_id" => $task_id))->getResult();
        foreach ($sub_tasks as $sub_task) {
            $sub_tasks_array[] = $this->_make_sub_task_row($sub_task);
        }
        $view_data["sub_tasks"] = json_encode($sub_tasks_array);
        $view_data["total_sub_tasks"] = $this->Responsibility_model->count_sub_task_status(array("parent_task_id" => $task_id));
        $view_data["completed_sub_tasks"] = $this->Responsibility_model->count_sub_task_status(array("parent_task_id" => $task_id, "status_id" => 3));
        ...
        $view_data['parent_task_title'] = $this->Responsibility_model->get_one($model_info->parent_task_id)->title;
        ...
        if ($view_type == "details") {
            return $this->template->rander('responsibility/view', $view_data);
        } else {
            return $this->template->view('responsibility/view', $view_data);
        }
    }
    ...
    private function _make_dependency_tasks_view_data($task_ids = "", $task_id = 0, $type = "") {
        if ($task_ids) {
            ...
            $tasks_list = $this->Responsibility_model->get_details(array("task_ids" => $task_ids))->getResult();
    ...
    private function _make_dependency_tasks_row_data($task_info, $task_id, $type) {
        ...
        if ($this->can_edit_tasks($task_info)) {
            $tasks .= ajax_anchor(get_uri("responsibility/delete_dependency_task/$task_info->id/$task_id/$type"), "<div class='float-end'><i data-feather='x' class='icon-16'></i></div>", array("class" => "delete-dependency-task", "title" => app_lang("delete"), "data-fade-out-on-success" => "#dependency-task-row-$task_info->id", "data-dependency-type" => $type));
        }

        $tasks .= modal_anchor(get_uri("responsibility/view"), $task_info->title, array("data-post-id" => $task_info->id, "data-modal-lg" => "1"));
    ...
    private function _get_all_dependency_for_this_task_specific($task_ids = "", $task_id = 0, $type = "") {
        if ($task_id && $type) {
            ...
            $dependency_tasks = $this->Responsibility_model->get_all_dependency_for_this_task($task_id, $type);
    ...
    function delete_dependency_task($dependency_task_id, $task_id, $type) {
        ...
        validate_numeric_value($task_id);
        $task_info = $this->Responsibility_model->get_one($task_id);
        ...
        if ($type == "blocked_by") {
            $dependency_tasks_of_others = $this->Responsibility_model->get_one($dependency_task_id)->blocking;
        } else {
            $dependency_tasks_of_others = $this->Responsibility_model->get_one($dependency_task_id)->blocked_by;
        }
        ...
        if (!strpos($dependency_tasks_of_own, ',') && $dependency_tasks_of_own == $dependency_task_id) {
            $data = array($type => "");
            $this->Responsibility_model->update_custom_data($data, $task_id);
            ...
            $this->Responsibility_model->update_custom_data($data, $dependency_task_id);
        } else {
    ...
    function all_gantt() {
        ...
        return $this->template->rander("enterprises/gantt/index", $view_data);
    }
    ...
    function save_dependency_tasks() {
        ...
        $this->Responsibility_model->update_custom_data($data, $task_id);
        $dependency_task_info = $this->Responsibility_model->get_details(array("id" => $dependency_task))->getRow();
    ...
    function save_gantt_task_date() {
        ...
        $save_id = $this->Responsibility_model->save_gantt_task_date($data, $task_id);
        if ($save_id) {
    ...
    function save_task_from_excel_file() {
        ...
        foreach ($excel_file as $key => $value) { //rows
            ...
            //save task data
            $task_save_id = $this->Responsibility_model->ci_save($task_data);
            $sort = $task_save_id;
    ...
    private function _get_project_id($project = "") {
        ...
        $existing_project = $this->Enterprises_model->get_one_where(array("title" => $project, "deleted" => 0));
    ...
    private function _get_status_id($status = "") {
        ...
        $existing_status = $this->Responsibility_status_model->get_one_where(array("title" => $status, "deleted" => 0));
    ...
    function project_tasks($project_id) {
        ...
        $view_data['task_statuses'] = $this->Responsibility_status_model->get_details(array("exclude_status_ids" => $exclude_status_ids))->getResult();
        ...
        return $this->template->view("enterprises/tasks/index", $view_data);
    }
    ...
    function project_tasks_kanban($project_id) {
        ...
        return $this->template->view("enterprises/tasks/kanban/project_tasks", $view_data);
    }
    ...
    function all_tasks($tab = "", $status_id = 0, $priority_id = 0, $type = "", $deadline = "") {
        ...
        $projects = $this->Responsibility_model->get_my_projects_dropdown_list($this->login_user->id)->getResult();
        $projects_dropdown = array(array("id" => "", "text" => "- " . app_lang("project") . " -"));
        foreach ($projects as $project) {
        ...
        $view_data['task_statuses'] = $this->Responsibility_status_model->get_details()->getResult();
        ...
        return $this->template->rander("responsibility/all_tasks", $view_data);
    }
    ...
    function all_tasks_kanban() {
        ...
        return $this->template->rander("responsibility/kanban/all_tasks", $view_data);
    }
    ...
    function all_tasks_kanban_data() {
        ...
        if ($column_id) {
            ...
            $view_data["tasks"] = $this->Responsibility_model->get_kanban_details($options)->getResult();
            ...
            return $this->template->view('responsibility/kanban/kanban_column_items', $view_data);
        } else {
            $task_count_query_options = $options;
            $task_count_query_options["return_task_counts_only"] = true;
            $task_counts = $this->Responsibility_model->get_kanban_details($task_count_query_options)->getResult();
            ...
            $statuses = $this->Responsibility_status_model->get_details($task_status_options);
            ...
            foreach ($columns as $column) {
                ...
                if (get_array_value($column_tasks_count, $status_id)) {
                    ...
                    $tasks = $this->Responsibility_model->get_kanban_details($options)->getResult();
            ...
            return $this->template->view('responsibility/kanban/kanban_view', $view_data);
        }
    }
    ...
    function project_tasks_kanban_data($project_id = 0) {
        ...
        if ($column_id) {
            ...
            $exclude_status_ids = $this->get_removed_task_status_ids($project_id);
            $statuses = $this->Responsibility_status_model->get_details(array("hide_from_kanban" => 0, "exclude_status_ids" => $exclude_status_ids));
    ...
    function set_task_comments_as_read($task_id = 0) {
        if ($task_id) {
            validate_numeric_value($task_id);
            $this->Responsibility_model->set_task_comments_as_read($task_id, $this->login_user->id);
        }
    }
    ...
    function save_sub_task() {
        ...
        $data["sort"] = $this->Responsibility_model->get_next_sort_value($project_id, $data['status_id']);

        $save_id = $this->Responsibility_model->ci_save($data);
        ...
        if ($save_id) {
            ...
            $task_info = $this->Responsibility_model->get_details(array("id" => $save_id))->getRow();
    ...
    function save_task_status($id = 0) {
        ...
        $task_info = $this->Responsibility_model->get_details(array("id" => $id))->getRow();
    ...
    function update_task_info($id = 0, $data_field = "") {
        ...
        $task_info = $this->Responsibility_model->get_details(array("id" => $save_id))->getRow(); //get data after save
    ...
    function all_tasks_list_data($is_widget = 0) {
        ...
        if ($is_widget) {
            $todo_status_id = $this->Responsibility_status_model->get_one_where(array("key_name" => "done", "deleted" => 0));
            if ($todo_status_id) {
    ...
    function gantt($project_id = 0) {
        ...
        if ($project_id) {
            ...
            return $this->template->view("enterprises/gantt/index", $view_data);
        }
    }
    ...
    function gantt_data($project_id = 0, $group_by = "milestones", $milestone_id = 0, $user_id = 0, $status = "") {
        ...
        $gantt_data = $this->Enterprises_model->get_gantt_data($options);
    ...
    function batch_update_modal_form($task_ids = "") {
        ...
        if ($task_ids && $project_id) {
            ...
            return $this->template->view("responsibility/batch_update/modal_form", $view_data);
        } else {
            show_404();
        }
    }
    ...
    function save_comment() {
        ...
        if ($save_id) {
            ...
            $task_info = $this->Responsibility_model->get_one($comment_info->task_id);
    ...
    function download_comment_files($id) {
        ...
        $task_info = $this->Responsibility_model->get_one($info->task_id);
    ...
    private function _init_required_data_before_starting_import() {
        ...
        $projects = $this->Enterprises_model->get_projects_id_and_name()->getResult();
    ...
    private function _save_a_row_of_excel_data($row_data) {
        ...
        //save task data
        $saved_id = $this->Responsibility_model->ci_save($task_data);
        if (!$saved_id) {
            return false;
        }
