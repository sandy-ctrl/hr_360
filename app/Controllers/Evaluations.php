...
class Evaluations extends Security_Controller {
    ...
    function index() {
        ...
        if ($this->login_user->user_type === "staff") {
            ...
            return $this->template->rander("evaluations/index", $view_data);
        } else {
            //client view
            ...
            return $this->template->rander("patrons/estimates/client_portal", $view_data);
        }
    }

    //load the yearly view of estimate list
    function yearly() {
        return $this->template->view("evaluations/yearly_estimates");
    }
    ...
    function modal_form() {
        ...
        $model_info = $this->Evaluations_model->get_one($id);
        ...
        return $this->template->view('evaluations/modal_form', $view_data);
    }
    ...
    function save() {
        ...
        $estimate_id = $this->Evaluations_model->ci_save($estimate_data, $id);
        if ($estimate_id) {
            ...
            if ($is_clone && $main_estimate_id) {
                ...
                $estimate_items = $this->Evaluation_items_model->get_all_where(array("estimate_id" => $main_estimate_id, "deleted" => 0))->getResult();
                ...
                foreach ($estimate_items as $estimate_item) {
                    ...
                    $estimate_item = $this->Evaluation_items_model->ci_save($estimate_item_data);
                }
            } else {
    ...
    private function _copy_related_items_to_estimate($copy_items_from_proposal, $copy_items_from_contract, $copy_items_from_order, $estimate_id) {
        ...
        foreach ($copy_items as $data) {
            ...
            $this->Evaluation_items_model->ci_save($estimate_item_data);
        }
    }
    ...
    function update_estimate_status($estimate_id, $status, $is_modal = false) {
        ...
        $estmate_info = $this->Evaluations_model->get_one($estimate_id);
        ...
        if ($this->login_user->user_type == "client") {
            ...
            $estimate_id = $this->Evaluations_model->ci_save($estimate_data, $estimate_id);
    ...
    private function _create_project_from_estimate($estimate_id) {
        if ($estimate_id) {
            ...
            $estimate_info = $this->Evaluations_model->get_one($estimate_id);

            //don't create new project if there has already been created a new project with this estimate
            if (!$this->Enterprises_model->get_one_where(array("estimate_id" => $estimate_id))->id) {
                $data = array(
                ...
                $save_id = $this->Enterprises_model->ci_save($data);
    ...
    function delete() {
        ...
        $id = $this->request->getPost('id');
        $this->validate_estimate_access($id);
        $estimate_info = $this->Evaluations_model->get_one($id);

        if ($this->Evaluations_model->delete($id)) {
    ...
    function list_data() {
        ...
        $list_data = $this->Evaluations_model->get_details($options)->getResult();
        $result = array();
    ...
    private function _row_data($id) {
        ...
        $data = $this->Evaluations_model->get_details($options)->getRow();
    ...
    private function _make_row($data, $custom_fields) {
        $estimate_url = "";
        if ($this->login_user->user_type == "staff") {
            $estimate_url = anchor(get_uri("evaluations/view/" . $data->id), get_estimate_id($data->id));
        } else {
            //for client client
            $estimate_url = anchor(get_uri("evaluations/preview/" . $data->id), get_estimate_id($data->id));
        }

        $client = anchor(get_uri("patrons/view/" . $data->client_id), $data->company_name);
        if ($data->is_lead) {
            $client = anchor(get_uri("initiates/view/" . $data->client_id), $data->company_name);
        }
        ...
        if (get_setting("enable_comments_on_estimates") && $data->status !== "draft") {
            $comment_link = modal_anchor(get_uri("evaluations/comment_modal_form"), "<i data-feather='message-circle' class='icon-16'></i>", array("class" => "edit text-muted", "title" => app_lang("estimate") . " #" . $data->id . " " . app_lang("comments"), "data-post-estimate_id" => $data->id));
        }
        ...
        if ($this->_is_estimate_editable($data)) {
            $edit = modal_anchor(get_uri("evaluations/modal_form"), "<i data-feather='edit' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('edit_estimate'), "data-post-id" => $data->id));
        }

        $row_data[] = anchor(get_uri("evaluation/preview/" . $data->id . "/" . $data->public_key), "<i data-feather='external-link' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('estimate') . " " . app_lang("url"), "target" => "_blank"))
                . $edit
                . js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete_estimate'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("evaluations/delete"), "data-action" => "delete-confirmation"));
    ...
    function view($estimate_id = 0) {
        ...
        if ($estimate_id) {
            ...
            $view_data['comments'] = $this->Evaluation_comments_model->get_details($comments_options)->getResult();
            ...
            if ($view_data) {
                ...
                return $this->template->rander("evaluations/view", $view_data);
            } else {
                show_404();
            }
        }
    ...
    private function _get_estimate_total_view($estimate_id = 0) {
        $view_data["estimate_total_summary"] = $this->Evaluations_model->get_estimate_total_summary($estimate_id);
        $view_data["estimate_id"] = $estimate_id;
        $view_data["is_estimate_editable"] = $this->_is_estimate_editable($estimate_id);
        return $this->template->view('evaluations/estimate_total_section', $view_data, true);
    }
    ...
    function discount_modal_form() {
        ...
        $view_data['model_info'] = $this->Evaluations_model->get_one($estimate_id);

        return $this->template->view('evaluations/discount_modal_form', $view_data);
    }
    ...
    function save_discount() {
        ...
        $save_data = $this->Evaluations_model->ci_save($data, $estimate_id);
        if ($save_data) {
    ...
    function item_modal_form() {
        ...
        $view_data['model_info'] = $this->Evaluation_items_model->get_one($this->request->getPost('id'));
        if (!$estimate_id) {
            $estimate_id = $view_data['model_info']->estimate_id;
        }
        $view_data['estimate_id'] = $estimate_id;
        return $this->template->view('evaluations/item_modal_form', $view_data);
    }
    ...
    function save_item() {
        ...
        $estimate_item_id = $this->Evaluation_items_model->ci_save($estimate_item_data, $id);
        if ($estimate_item_id) {
            $options = array("id" => $estimate_item_id);
            $item_info = $this->Evaluation_items_model->get_details($options)->getRow();
    ...
    function delete_item() {
        ...
        $id = $this->request->getPost('id');
        $item_info = $this->Evaluation_items_model->get_one($id);
        if (!$this->_is_estimate_editable($item_info->estimate_id)) {
            app_redirect("forbidden");
        }

        if ($this->request->getPost('undo')) {
            if ($this->Evaluation_items_model->delete($id, true)) {
                $options = array("id" => $id);
                $item_info = $this->Evaluation_items_model->get_details($options)->getRow();
            ...
            if ($this->Evaluation_items_model->delete($id)) {
                $item_info = $this->Evaluation_items_model->get_one($id);
    ...
    function item_list_data($estimate_id = 0) {
        ...
        $list_data = $this->Evaluation_items_model->get_details(array("estimate_id" => $estimate_id))->getResult();
        $result = array();
        foreach ($list_data as $data) {
    ...
    private function _make_item_row($data) {
        ...
        return array(
            ...
            modal_anchor(get_uri("evaluations/item_modal_form"), "<i data-feather='edit' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('edit_estimate'), "data-post-id" => $data->id, "data-post-estimate_id" => $data->estimate_id))
            . js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("evaluations/delete_item"), "data-action" => "delete"))
        );
    }
    ...
    function preview($estimate_id = 0, $show_close_preview = false) {
        ...
        if ($estimate_id) {
            ...
            return $this->template->rander("evaluations/estimate_preview", $view_data);
        } else {
            show_404();
        }
    }
    ...
    function get_estimate_status_bar($estimate_id = 0) {
        ...
        $view_data["estimate_info"] = $this->Evaluations_model->get_details(array("id" => $estimate_id))->getRow();
        $view_data['estimate_status_label'] = $this->_get_estimate_status_label($view_data["estimate_info"]);
        return $this->template->view('evaluations/estimate_status_bar', $view_data);
    }
    ...
    function send_estimate_modal_form($estimate_id) {
        ...
        if ($estimate_id) {
            ...
            $estimate_info = $this->Evaluations_model->get_details($options)->getRow();
            ...
            return $this->template->view('evaluations/send_estimate_modal_form', $view_data);
        } else {
            show_404();
        }
    }
    ...
    function get_send_estimate_template($estimate_id = 0, $contact_id = 0, $return_type = "", $estimate_info = "", $contact_info = "") {
        ...
        $parser_data["PUBLIC_ESTIMATE_URL"] = get_uri("evaluation/preview/" . $estimate_info->id . "/" . $estimate_info->public_key);
        ...
        $parser_data["ESTIMATE_URL"] = get_uri("evaluations/preview/" . $estimate_info->id);
    ...
    function send_estimate() {
        ...
        if (send_app_mail($contact->email, $subject, $message, array("attachments" => array(array("file_path" => $attachement_url)), "cc" => $cc, "bcc" => $bcc_emails))) {
            ...
            if ($this->Evaluations_model->ci_save($status_data, $estimate_id)) {
    ...
    function update_item_sort_values($id = 0) {
        ...
        if ($sort_values) {
            ...
            foreach ($sort_array as $value) {
                ...
                $this->Evaluation_items_model->ci_save($data, $id);
            }
        }
    }
    ...
    function save_comment() {
        ...
        $comment_id = $this->Evaluation_comments_model->ci_save($comment_data);
        if ($comment_id) {
            $comments_options = array("id" => $comment_id);
            $view_data['comment'] = $this->Evaluation_comments_model->get_details($comments_options)->getRow();
            $comment_view = $this->template->view("evaluations/comment_row", $view_data, true);
    ...
    function delete_comment($id = 0) {
        ...
        $comment_info = $this->Evaluation_comments_model->get_one($id);
        ...
        if ($this->Evaluation_comments_model->delete($id) && $comment_info->files) {
    ...
    function download_comment_files($id) {
        ...
        $files = $this->Evaluation_comments_model->get_one($id)->files;
    ...
    function comment_modal_form() {
        ...
        return $this->template->view('evaluations/comment_form', $view_data);
    }
    ...
    function print_estimate($estimate_id = 0) {
        if ($estimate_id) {
            ...
            echo json_encode(array("success" => true, "print_view" => $this->template->view("evaluations/print_estimate", $view_data)));
        } else {
            echo json_encode(array("success" => false, app_lang('error_occurred')));
        }
    }
    ...
    function tasks($estimate_id) {
        ...
        return $this->template->view("evaluations/tasks/index", $view_data);
    }
    ...
    private function _is_estimate_editable($_estimate, $is_clone = 0) {
        if (get_setting("enable_estimate_lock_state")) {
            $estimate_info = is_object($_estimate) ? $_estimate : $this->Evaluations_model->get_one($_estimate);
