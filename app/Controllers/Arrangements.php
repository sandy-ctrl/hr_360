...
class Arrangements extends Security_Controller {
    ...
    function index() {
        ...
        if ($this->login_user->user_type === "staff") {
            $view_data['order_statuses'] = $this->Arrangement_status_model->get_details()->getResult();
            return $this->template->rander("arrangements/index", $view_data);
        } else {
            //client view
            ...
            return $this->template->rander("patrons/orders/client_portal", $view_data);
        }
    }
    ...
    function list_data() {
        ...
        $list_data = $this->Arrangements_model->get_details($options)->getResult();
        $result = array();
    ...
    private function _make_row($data, $custom_fields) {
        $order_url = "";
        if ($this->login_user->user_type == "staff") {
            $order_url = anchor(get_uri("arrangements/view/" . $data->id), get_order_id($data->id));
        } else {
            //for client
            $order_url = anchor(get_uri("outlet/order_preview/" . $data->id), get_order_id($data->id));
        }
        ...
        if ($data->invoices) {
            ...
            foreach ($invoices as $invoice) {
                ...
                if ($this->login_user->user_type == "staff") {
                    $invoice_links .= anchor(get_uri("bills/view/" . $invoice_id), $invoice_display_id);
                } else {
                    $invoice_links .= anchor(get_uri("bills/preview/" . $invoice_id), $invoice_display_id);
                }
            }
        }
        ...
        $row_data[] = modal_anchor(get_uri("arrangements/modal_form"), "<i data-feather='edit' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('edit_order'), "data-post-id" => $data->id))
                . js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete_order'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("arrangements/delete"), "data-action" => "delete"));
    ...
    function yearly() {
        return $this->template->view("arrangements/yearly_orders");
    }

    //load the custom view of order list
    function custom() {
        return $this->template->view("arrangements/custom_orders");
    }
    ...
    function modal_form() {
        ...
        $view_data['model_info'] = $this->Arrangements_model->get_one($this->request->getPost('id'));
        ...
        return $this->template->view('arrangements/modal_form', $view_data);
    }
    ...
    function save() {
        ...
        $order_info = $this->Arrangements_model->get_one($id);
        ...
        $order_id = $this->Arrangements_model->ci_save($order_data, $id);
        if ($order_id) {
    ...
    function delete() {
        ...
        if ($this->request->getPost('undo')) {
            if ($this->Arrangements_model->delete($id, true)) {
                echo json_encode(array("success" => true, "data" => $this->_row_data($id), "message" => app_lang('record_undone')));
            } else {
                echo json_encode(array("success" => false, app_lang('error_occurred')));
            }
        } else {
            if ($this->Arrangements_model->delete($id)) {
    ...
    function view($order_id = 0) {
        ...
        if ($order_id) {
            ...
            if ($view_data) {
                ...
                return $this->template->rander("arrangements/view", $view_data);
            } else {
                show_404();
            }
        }
    ...
    function get_order_item_suggestion() {
        ...
        $items = $this->Bill_items_model->get_item_suggestion($key, $this->login_user->user_type);
    ...
    function get_order_item_info_suggestion() {
        $item = $this->Bill_items_model->get_item_info_suggestion(array("item_id" => $this->request->getPost("item_id"), "user_type" => $this->login_user->user_type));
        if ($item) {
    ...
    function save_order_status($id = 0) {
        ...
        $save_id = $this->Arrangements_model->ci_save($data, $id);

        if ($save_id) {
            log_notification("order_status_updated", array("order_id" => $id));
            $order_info = $this->Arrangements_model->get_details(array("id" => $id))->getRow();
    ...
    private function _row_data($id) {
        ...
        $data = $this->Arrangements_model->get_details($options)->getRow();
    ...
    function discount_modal_form() {
        ...
        $view_data['model_info'] = $this->Arrangements_model->get_one($order_id);

        return $this->template->view('arrangements/discount_modal_form', $view_data);
    }
    ...
    function save_discount() {
        ...
        $save_data = $this->Arrangements_model->ci_save($data, $order_id);
        if ($save_data) {
    ...
    function item_list_data($order_id = 0) {
        ...
        $list_data = $this->Arrangement_items_model->get_details(array("order_id" => $order_id))->getResult();
        $result = array();
        foreach ($list_data as $data) {
    ...
    function file_preview($id = "", $key = "") {
        if ($id) {
            ...
            return $this->template->view("arrangements/file_preview", $view_data);
        } else {
            show_404();
        }
    }
    ...
    function details($order_id = 0) {
        ...
        if ($order_id) {
            ...
            return $this->template->view("arrangements/details", $view_data);
        } else {
            show_404();
        }
    }
    ...
    function tasks($order_id) {
        ...
        return $this->template->view("arrangements/tasks/index", $view_data);
    }
    ...
    function invoices($order_id) {
        ...
        validate_numeric_value($order_id);
        $view_data["order_id"] = $order_id;
        $view_data["order_info"] = $this->Arrangements_model->get_details(array("id" => $order_id))->getRow();

        $view_data["custom_field_headers"] = $this->Custom_fields_model->get_custom_field_headers_for_table("orders", $this->login_user->is_admin, $this->login_user->user_type);
        $view_data["custom_field_filters"] = $this->Custom_fields_model->get_custom_field_filters("orders", $this->login_user->is_admin, $this->login_user->user_type);

        return $this->template->view("arrangements/order_invoice_lists", $view_data);
    }
    ...
    function invoice_payment_list($order_id) {
        ...
        return $this->template->view("arrangements/order_invoice_payment_list", $view_data);
    }
    ...
    function orders_summary() {
        ...
        return $this->template->rander("arrangements/reports/orders_summary", $view_data);
    }
    ...
    function monthly_orders_summary() {
        ...
        $view_data["currencies_dropdown"] = $this->_get_currencies_dropdown(false);
        return $this->template->view("arrangements/reports/monthly_orders_summary", $view_data);
    }
    ...
    function custom_orders_summary() {
        ...
        $view_data["currencies_dropdown"] = $this->_get_currencies_dropdown(false);
        return $this->template->view("arrangements/reports/custom_orders_summary", $view_data);
    }
