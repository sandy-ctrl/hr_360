...
class Settings extends Security_Controller
{
    ...
    function invoices()
    {
        $last_invoice_id = $this->Bills_model->get_last_invoice_id();
    ...
    function save_invoice_settings()
    {
        ...
        foreach ($settings as $setting) {
            ...
            if ($setting === "initial_number_of_the_invoice") {
                $this->Bills_model->save_initial_number_of_invoice($value);
            }
        }
    ...
    function notification_modal_form()
    {
        ...
        if ($id) {
            ...
            if (in_array("team", $notify_to)) {
                $teams = $this->Unit_model->get_all_where(array("deleted" => 0))->getResult();
    ...
    function authorize_imap()
    {
        if (get_setting("enable_email_piping")) {
            ...
            app_redirect("token_types/index/imap");
        }
    }

    function estimates()
    {
        $estimate_info = $this->Evaluations_model->get_estimate_last_id();
    ...
    function save_estimate_settings()
    {
        ...
        foreach ($settings as $setting) {
            ...
            if ($setting === "initial_number_of_the_estimate") {
                $this->Evaluations_model->save_initial_number_of_estimate($value);
            }
        }
    ...
    function orders()
    {
        $order_info = $this->Arrangements_model->get_order_last_id();
    ...
    function save_order_settings()
    {
        ...
        foreach ($settings as $setting) {
            ...
            if ($setting === "initial_number_of_the_order") {
                $this->Arrangements_model->save_initial_number_of_order($value);
            }
        }
    ...
    function store()
    {
        ...
        $store_statuses = $this->Arrangement_status_model->get_details()->getResult();
    ...
    {
        $last_subscription_id = $this->Membership_model->get_last_subscription_id();
    ...
    function save_subscription_settings()
    {
        ...
        foreach ($settings as $setting) {
            ...
            if ($setting === "initial_number_of_the_subscription") {
                $this->Membership_model->save_initial_number_of_subscription($value);
            }
        }
