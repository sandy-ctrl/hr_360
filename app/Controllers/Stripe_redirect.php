...
class Stripe_redirect extends App_Controller {
    ...
    function index($payment_verification_code = "") {
        ...
        $existing = $this->Bill_payments_model->get_one_where(array("transaction_id" => $payment->id));
        if ($existing->id) {
            show_404();
        }
    ...
    function subscription($payment_verification_code = "") {
        ...
        $subscription_info = $this->Membership_model->get_details(array("id" => $stripe_ipn_info->subscription_id))->getRow();
        ...
        try {
            ...
            $this->Membership_model->ci_save($subscription_data, $stripe_ipn_info->subscription_id);
            ...
            $this->Patrons_model->ci_save($client_data, $stripe_ipn_info->client_id);
            ...
            app_redirect("membership/preview/$stripe_ipn_info->subscription_id");
        } catch (\Exception $ex) {
