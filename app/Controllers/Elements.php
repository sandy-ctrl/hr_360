...
class Elements extends Security_Controller {
    ...
    function index() {
        ...
        return $this->template->rander("elements/index", $view_data);
    }
    ...
    function modal_form() {
        ...
        $view_data['model_info'] = $this->Elements_model->get_one($this->request->getPost('id'));
        $view_data['categories_dropdown'] = $this->Item_categories_model->get_dropdown_list(array("title"));

        return $this->template->view('elements/modal_form', $view_data);
    }
    ...
    function save() {
        ...
        $item_id = $this->Elements_model->ci_save($item_data, $id);
        if ($item_id) {
            $options = array("id" => $item_id);
            $item_info = $this->Elements_model->get_details($options)->getRow();
    ...
    function delete() {
        ...
        if ($this->request->getPost('undo')) {
            if ($this->Elements_model->delete($id, true)) {
                $options = array("id" => $id);
                $item_info = $this->Elements_model->get_details($options)->getRow();
            ...
            if ($this->Elements_model->delete($id)) {
                $item_info = $this->Elements_model->get_one($id);
    ...
    function list_data() {
        ...
        $list_data = $this->Elements_model->get_details($options)->getResult();
        $result = array();
    ...
    private function _make_item_row($data) {
        ...
        return array(
            modal_anchor(get_uri("elements/view"), $show_in_client_portal_icon . $data->title, array("title" => app_lang("item_details"), "data-post-id" => $data->id)),
            nl2br($data->description ? $data->description : ""),
            $data->category_title ? $data->category_title : "-",
            $type,
            to_decimal_format($data->rate),
            modal_anchor(get_uri("elements/modal_form"), "<i data-feather='edit' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('edit_item'), "data-post-id" => $data->id))
            . js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("elements/delete"), "data-action" => "delete"))
        );
    }
    ...
    function view() {
        ...
        $model_info = $this->Elements_model->get_details(array("id" => $this->request->getPost('id'), "login_user_id" => $this->login_user->id))->getRow();
    ...
    function save_files_sort() {
        ...
        if ($id && $sort_values) {
            ...
            if ($item_info->id) {
                ...
                $this->Elements_model->ci_save($item_data, $id);
            }
        }
    }
    ...
    function import_items_modal_form() {
        ...
        return $this->template->view("elements/import_items_modal_form");
    }
    ...
    function save_item_from_excel_file() {
        ...
        foreach ($excel_file as $key => $value) { //rows
            ...
            //save item data
            $item_save_id = $this->Elements_model->ci_save($item_data);
            if (!$item_save_id) {
                continue;
            }
        }
