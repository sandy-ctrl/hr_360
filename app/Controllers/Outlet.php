...
class Outlet extends Security_Controller {
    ...
    function index($offset = 0, $limit = 20, $category_id = 0, $search = "") {
        ...
        //get all rows
        $all_items = $this->Elements_model->get_details($options)->resultID->num_rows;

        $options["offset"] = $offset;
        $options["limit"] = $limit;

        $view_data["items"] = $this->Elements_model->get_details($options)->getResult();
        ...
        $view_data["cart_items_count"] = count($this->Arrangement_items_model->get_details(array("created_by" => (isset($this->login_user->id) ? $this->login_user->id : 0), "created_by_hash" => $this->get_cookie_hash(), "processing" => true))->getResult());
        ...
        if ($offset) { //load more view
            return $this->template->view("elements/items_grid_data", $view_data);
        } else if ($item_search) { //search suggestions view
            echo json_encode(array("success" => true, "data" => $this->template->view("elements/items_grid_data", $view_data)));
        } else { //default view
            return $this->template->rander("elements/grid_view", $view_data);
        }
    }
    ...
    function item_view() {
        ...
        $model_info = $this->Elements_model->get_details(array("id" => $this->request->getPost('id'), "login_user_id" => (isset($this->login_user->id) ? $this->login_user->id : 0), "created_by_hash" => $this->get_cookie_hash()))->getRow();
        ...
        return $this->template->view('elements/view', $view_data);
    }
    ...
    function add_item_to_cart() {
        ...
        $item_info = $this->Elements_model->get_one($id);
        ...
        $save_id = $this->Arrangement_items_model->ci_save($order_item_data);
    ...
    function count_cart_items() {
        ...
        $cart_items_count = count($this->Arrangement_items_model->get_details(array("created_by" => (isset($this->login_user->id) ? $this->login_user->id : 0), "created_by_hash" => $this->get_cookie_hash(), "processing" => true))->getResult());
    ...
    function load_cart_items() {
        ...
        $view_data["items"] = $this->Arrangement_items_model->get_details($options)->getResult();

        if (isset($this->login_user->client_id)) {
            $view_data["client_info"] = $this->Patrons_model->get_one($this->login_user->client_id);
        }

        return $this->template->view("elements/cart/cart_items_list", $view_data);
    }
    ...
    function delete_cart_item() {
        ...
        $order_item_info = $this->Arrangement_items_model->get_one($order_item_id);
        $this->check_access_to_this_order_item($order_item_info);

        if ($this->Arrangement_items_model->delete($order_item_id)) {
    ...
    function change_cart_item_quantity($type = "") {
        ...
        $item_info = $this->Arrangement_items_model->get_one($id);
        ...
        $this->Arrangement_items_model->ci_save($data, $item_info->id);

        $options = array("id" => $id);
        $view_data["item"] = $this->Arrangement_items_model->get_details($options)->getRow();

        if (isset($this->login_user->client_id)) {
            $view_data["client_info"] = $this->Patrons_model->get_one($this->login_user->client_id);
        }

        echo json_encode(array("success" => true, 'message' => app_lang('record_saved'), "data" => $this->template->view("elements/cart/cart_item_data", $view_data), "cart_total_view" => $this->_get_cart_total_view()));
    }

    private function _get_cart_total_view() {
        $view_data = get_order_making_data(0, $this->get_cookie_hash());
        return $this->template->view('elements/cart/cart_total_section', $view_data);
    }
    ...
    private function check_accept_order_before_login_permission() {
        if (isset($this->login_user->id)) {
            ...
            if (!(get_setting("module_order") && get_setting("visitors_can_see_store_before_login") && get_setting("accept_order_before_login"))) {
                app_redirect('signin?redirect=' . get_uri("outlet/process_order"));
            }
        }
    }
    ...
    function process_order() {
        ...
        return $this->template->rander("arrangements/process_order", $view_data);
    }

    function item_list_data_of_login_user() {
        $this->check_access_to_store();
        $options = array("created_by" => (isset($this->login_user->id) ? $this->login_user->id : 0), "created_by_hash" => $this->get_cookie_hash(), "processing" => true);
        $list_data = $this->Arrangement_items_model->get_details($options)->getResult();
        $result = array();
    ...
    function item_modal_form() {
        ...
        $id = $this->request->getPost('id');
        $model_info = $this->Arrangement_items_model->get_one($id);
        ...
        return $this->template->view('arrangements/item_modal_form', $view_data);
    }
    ...
    function save_item() {
        ...
        if ($id) { //item added to order items
            ...
            $item_info = $this->Elements_model->get_one($item_id);
            $this->check_access_to_this_item($item_info);
        }
        ...
        $order_item_id = $this->Arrangement_items_model->ci_save($order_item_data, $id);
        if ($order_item_id) {
            ...
            if ($add_new_item_to_library) {
                ...
                $this->Elements_model->ci_save($library_item_data);
            }

            $options = array("id" => $order_item_id);
            $item_info = $this->Arrangement_items_model->get_details($options)->getRow();
    ...
    function update_item_sort_values($id = 0) {
        ...
        if ($sort_values) {
            ...
            foreach ($sort_array as $value) {
                ...
                $this->Arrangement_items_model->ci_save($data, $id);
            }
        }
    }
    ...
    function delete_item() {
        ...
        $id = $this->request->getPost('id');
        $order_item_info = $this->Arrangement_items_model->get_one($id);
        $this->check_access_to_this_order_item($order_item_info);

        if ($this->request->getPost('undo')) {
            if ($this->Arrangement_items_model->delete($id, true)) {
                $options = array("id" => $id);
                $item_info = $this->Arrangement_items_model->get_details($options)->getRow();
            ...
            if ($this->Arrangement_items_model->delete($id)) {
                $item_info = $this->Arrangement_items_model->get_one($id);
    ...
    private function _get_order_total_view($order_id = 0) {
        if ($order_id) {
            $view_data["order_total_summary"] = $this->Arrangements_model->get_order_total_summary($order_id);
            $view_data["order_id"] = $order_id;
            return $this->template->view('arrangements/order_total_section', $view_data);
        } else {
            $view_data = get_order_making_data(0, $this->get_cookie_hash());
            return $this->template->view('arrangements/processing_order_total_section', $view_data);
        }
    }
    ...
    function place_order() {
        ...
        $order_items = $this->Arrangement_items_model->get_details(array("created_by" => (isset($this->login_user->id) ? $this->login_user->id : 0), "created_by_hash" => $this->get_cookie_hash(), "processing" => true))->getResult();
        if (!$order_items) {
        ...
        $order_data = array(
            ...
            "status_id" => $this->Arrangement_status_model->get_first_status(),
        ...
        $order_id = $this->Arrangements_model->ci_save($order_data);
        ...
        if ($order_id) {
            ...
            foreach ($order_items as $order_item) {
                ...
                $this->Arrangement_items_model->ci_save($order_item_data, $order_item->id);
            }

            $redirect_to = "";
            if (isset($this->login_user->id) && $this->login_user->user_type == "staff") {
                $redirect_to = get_uri("arrangements/view/$order_id");
            } else {
                if (get_setting("show_payment_option_after_submitting_the_order")) {
                    $invoice_info = $this->Bills_model->get_one_where(array("order_id" => $order_id, "deleted" => 0));
                    if ($invoice_info->id) {
                    ...
                    $redirect_to = get_uri("bills/preview/$invoice_id");
                } else {
                    $redirect_to = get_uri("outlet/order_preview/$order_id");
                }
            }
    ...
    function order_preview($order_id = 0) {
        ...
        return $this->template->rander("arrangements/order_preview", $view_data);
    }
    ...
    private function create_new_client() {
        ...
        if ($user_info->id) {
            ...
            echo json_encode(array("success" => false, 'message' => app_lang("account_already_exists_for_your_mail") . " " . anchor("signin?redirect=" . get_uri("outlet/process_order"), app_lang("signin"))));
            exit();
        }
        ...
        if (get_setting("disallow_duplicate_client_company_name") == "1" && $this->Patrons_model->is_duplicate_company_name($company_name)) {
            echo json_encode(array("success" => false, 'message' => app_lang("account_already_exists_for_your_company_name") . " " . anchor(get_uri("signin?redirect=" . get_uri("outlet/process_order")), app_lang('signin'), array("class" => "text-white text-off"))));
            return false;
        }
        ...
        $client_id = $this->Patrons_model->ci_save($client_data);
        if (!$client_id) {
            show_404();
        }
