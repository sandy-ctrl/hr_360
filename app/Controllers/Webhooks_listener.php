...
class Webhooks_listener extends App_Controller {
    ...
    private function subscription_payment_succeeded($payloads) {
        ...
        $subscription_info = $this->Membership_model->get_one_where(array("stripe_subscription_id" => $stripe_subscription_id));
        ...
        $existing = $this->Bill_payments_model->get_one_where(array("transaction_id" => $payloads_data->payment_intent));
        if ($existing->id) {
            show_404();
        }
    ...
    private function subscription_payment_failed($payloads) {
        ...
        $this->Membership_model->ci_save($subscription_data, $subscription_info->id);
    }
