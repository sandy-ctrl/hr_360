...
class Bills extends Security_Controller
{
    ...
    function index($tab = "")
    {
        ...
        if ($this->login_user->user_type === "staff") {
            ...
            return $this->template->rander("bills/index", $view_data);
        } else {
            if (!$this->can_client_access("invoice")) {
                app_redirect("forbidden");
            }

            $view_data["client_info"] = $this->Patrons_model->get_one($this->login_user->client_id);
            $view_data['client_id'] = $this->login_user->client_id;
            $view_data['page_type'] = "full";
            return $this->template->rander("patrons/invoices/index", $view_data);
        }
    }
    ...
    function yearly()
    {
        return $this->template->view("bills/yearly_invoices");
    }
    ...
    function recurring()
    {
        $view_data["currencies_dropdown"] = $this->_get_currencies_dropdown();
        $view_data["can_edit_invoices"] = $this->can_edit_invoices();
        return $this->template->view("bills/recurring_invoices_list", $view_data);
    }

    //load the custom view of invoice list 
    function custom()
    {
        return $this->template->view("bills/custom_invoices_list");
    }
    ...
    function modal_form()
    {
        ...
        $model_info = $this->Bills_model->get_one($invoice_id);
        ...
        if ($estimate_id || $order_id || $proposal_id || $contract_id) {
            $info = null;
            if ($estimate_id) {
                $info = $this->Evaluations_model->get_one($estimate_id);
            } else if ($order_id) {
                $info = $this->Arrangements_model->get_one($order_id);
            } else if ($contract_id) {
        ...
        $projects = $this->Enterprises_model->get_dropdown_list(array("title"), "id", array("client_id" => $project_client_id, "project_type" => "client_project"));
        ...
        return $this->template->view('bills/modal_form', $view_data);
    }
    ...
    function recurring_modal_form()
    {
        ...
        return $this->template->view('bills/recurring_modal_form', $view_data);
    }
    ...
    function save()
    {
        ...
        if ($id) {
            $invoice_info = $this->Bills_model->get_one($id);
        ...
        $invoice_id = $this->Bills_model->save_invoice_and_update_total($invoice_data, $id);
        if ($invoice_id) {
            ...
            if ($is_clone && $main_invoice_id) {
                ...
                $invoice_items = $this->Bill_items_model->get_all_where(array("invoice_id" => $main_invoice_id, "deleted" => 0))->getResult();
                ...
                foreach ($invoice_items as $invoice_item) {
                    ...
                    $invoice_item = $this->Bill_items_model->ci_save($invoice_item_data);
                }
                $this->Bills_model->update_invoice_total_meta($invoice_id);
            } else {
    ...
    private function _get_recurring_data($invoice_id = 0)
    {
        ...
        if ($invoice_id) {
            ...
            $invoice_info = $this->Bills_model->get_one($invoice_id);
            if (!$bill_date) {
    ...
    function save_recurring_info()
    {
        ...
        $invoice_id = $this->Bills_model->ci_save($invoice_data, $id);
        if ($invoice_id) {
            echo json_encode(array("success" => true, "data" => $this->_row_data($invoice_id), 'id' => $invoice_id, 'message' => app_lang('record_saved')));
        }
    }
    ...
    private function _copy_related_items_to_invoice($copy_items_from_estimate, $copy_items_from_proposal, $copy_items_from_order, $copy_items_from_contract, $invoice_id)
    {
        ...
        if ($copy_items_from_estimate) {
            $copy_items = $this->Evaluation_items_model->get_details(array("estimate_id" => $copy_items_from_estimate))->getResult();
            ...
            $copy_items = $this->Arrangement_items_model->get_details(array("order_id" => $copy_items_from_order))->getResult();
        }
        ...
        foreach ($copy_items as $data) {
            ...
            $this->Bill_items_model->ci_save($invoice_item_data);
        }
        $this->Bills_model->update_invoice_total_meta($invoice_id);
    }
    ...
    function list_data()
    {
        ...
        $list_data = $this->Bills_model->get_details($options)->getResult();
        $result = array();
    ...
    function sub_invoices($recurring_invoice_id)
    {
        ...
        return $this->template->view("bills/sub_invoices", $view_data);
    }
    ...
    private function _row_data($id)
    {
        ...
        $data = $this->Bills_model->get_details($options)->getRow();
    ...
    private function _make_row($data, $custom_fields)
    {
        ...
        if ($this->login_user->user_type == "staff") {
            ...
            if ($data->main_invoice_id) {
                $main_invoice_url = anchor(get_uri("bills/view/" . $data->main_invoice_id), "<i data-feather='file-text' class='icon-18'></i>", array("title" => app_lang("main_invoice"), "class" => "ml10"));
                $link_class = "text-danger";
            } else if ($data->credit_note_id) {
                $credit_note_url = anchor(get_uri("bills/view/" . $data->credit_note_id), "<i data-feather='file-minus' class='icon-18'></i>", array("title" => app_lang("credit_note"), "class" => "ml10"));
            }

            $invoice_url = anchor(get_uri("bills/view/" . $data->id), $data->display_id, array("class" => $link_class)) . $main_invoice_url . $credit_note_url;
        } else {
            $link_class = "";
            if ($data->main_invoice_id) {
                $main_invoice_url = anchor(get_uri("bills/preview/" . $data->main_invoice_id), "<i data-feather='file-text' class='icon-18'></i>", array("title" => app_lang("main_invoice"), "class" => "ml10"));
                $link_class = "text-danger";
            } else if ($data->credit_note_id) {
                $credit_note_url = anchor(get_uri("bills/preview/" . $data->credit_note_id), "<i data-feather='file-minus' class='icon-18'></i>", array("title" => app_lang("credit_note"), "class" => "ml10"));
            }

            $invoice_url = anchor(get_uri("bills/preview/" . $data->id), $data->display_id, array("class" => $link_class)) . $main_invoice_url . $credit_note_url;
        }
        ...
        $row_data = array(
            $data->id,
            $invoice_url,
            anchor(get_uri("patrons/view/" . $data->client_id), $data->company_name),
    ...
    private function _make_options_dropdown($data)
    {
        ...
        $delete = '<li role="presentation">' . js_anchor("<i data-feather='x' class='icon-16'></i>" . app_lang('delete'), array('title' => app_lang('delete_invoice'), "class" => "delete dropdown-item", "data-id" => $data->id, "data-action-url" => get_uri("bills/delete"), "data-action" => "delete-confirmation")) . '</li>';

        $add_payment = '<li role="presentation">' . modal_anchor(get_uri("bill_payments/payment_modal_form"), "<i data-feather='plus-circle' class='icon-16'></i> " . app_lang('add_payment'), array("title" => app_lang('add_payment'), "data-post-invoice_id" => $data->id, "class" => "dropdown-item")) . '</li>';
    ...
    private function _make_recurring_row($data)
    {
        ...
        $invoice_url = anchor(get_uri("bills/view/" . $data->id), $data->display_id);
    ...
    function view($invoice_id = 0)
    {
        ...
        if ($invoice_id) {
            ...
            if ($view_data) {
                ...
                return $this->template->rander("bills/view", $view_data);
            } else {
                show_404();
            }
        }
    ...
    {
        $view_data["invoice_total_summary"] = $this->Bills_model->get_invoice_total_summary($invoice_id);
        ...
        return $this->template->view('bills/invoice_total_section', $view_data);
    }
    ...
    function item_modal_form()
    {
        ...
        $view_data['model_info'] = $this->Bill_items_model->get_one($this->request->getPost('id'));
        if (!$invoice_id) {
            $invoice_id = $view_data['model_info']->invoice_id;
        }
        $view_data['invoice_id'] = $invoice_id;
        return $this->template->view('bills/item_modal_form', $view_data);
    }
    ...
    function save_item()
    {
        ...
        if ($add_new_item_to_library) {
            ...
            $item_id = $this->Elements_model->ci_save($library_item_data);
        }
        ...
        $invoice_item_id = $this->Bill_items_model->save_item_and_update_invoice($invoice_item_data, $id, $invoice_id);
        if ($invoice_item_id) {
            $options = array("id" => $invoice_item_id);
            $item_info = $this->Bill_items_model->get_details($options)->getRow();
    ...
    function delete_item()
    {
        ...
        $id = $this->request->getPost('id');
        $item_info = $this->Bill_items_model->get_one($id);
        ...
        if ($this->request->getPost('undo')) {
            if ($this->Bill_items_model->delete_item_and_update_invoice($id, true)) {
                $options = array("id" => $id);
                $item_info = $this->Bill_items_model->get_details($options)->getRow();
            ...
            if ($this->Bill_items_model->delete_item_and_update_invoice($id)) {
                $item_info = $this->Bill_items_model->get_one($id);
    ...
    function item_list_data($invoice_id = 0)
    {
        ...
        $list_data = $this->Bill_items_model->get_details(array("invoice_id" => $invoice_id))->getResult();
    ...
    private function _make_item_row($data, $is_ediable)
    {
        ...
        return array(
            ...
            modal_anchor(get_uri("bills/item_modal_form"), "<i data-feather='edit' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('edit_invoice'), "data-post-id" => $data->id, "data-post-invoice_id" => $data->invoice_id))
                . js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("bills/delete_item"), "data-action" => "delete"))
        );
    }
    ...
    function update_item_sort_values($id = 0)
    {
        ...
        if ($sort_values) {
            ...
            foreach ($sort_array as $value) {
                ...
                $this->Bill_items_model->ci_save($data, $id);
            }
        }
    }
    ...
    function get_invoice_item_suggestion()
    {
        ...
        $items = $this->Bill_items_model->get_item_suggestion($key);
    ...
    {
        $item = $this->Bill_items_model->get_item_info_suggestion(array("item_id" => $this->request->getPost("item_id")));
        if ($item) {
    ...
    function preview($invoice_id = 0, $show_close_preview = false)
    {
        if ($invoice_id) {
            ...
            return $this->template->rander("bills/invoice_preview", $view_data);
        } else {
            show_404();
        }
    }
    ...
    function print_invoice($invoice_id = 0)
    {
        if ($invoice_id) {
            ...
            echo json_encode(array("success" => true, "print_view" => $this->template->view("bills/print_invoice", $view_data)));
        } else {
            echo json_encode(array("success" => false, app_lang('error_occurred')));
        }
    }
    ...
    function send_invoice_modal_form($invoice_id)
    {
        ...
        if ($invoice_id) {
            ...
            $invoice_info = $this->Bills_model->get_details($options)->getRow();
            ...
            return $this->template->view('bills/send_invoice_modal_form', $view_data);
        } else {
            show_404();
        }
    }
    ...
    function get_send_invoice_template($invoice_id = 0, $contact_id = 0, $return_type = "", $invoice_info = "", $contact_info = "")
    {
        ...
        if ($invoice_info->main_invoice_id) {
            ...
            $parser_data["CREDIT_NOTE_URL"] = get_uri("bills/preview/" . $invoice_info->id);
            ...
            $invoice_total_summary = $this->Bills_model->get_invoice_total_summary($invoice_id);
            ...
            $parser_data["INVOICE_URL"] = get_uri("bills/preview/" . $invoice_info->id);
        }
    ...
    function send_invoice()
    {
        ...
        if (send_app_mail($contact->email, $subject, $message, array("attachments" => $attachments, "cc" => $cc_array, "bcc" => $bcc_emails))) {
            ...
            if ($this->Bills_model->ci_save($status_data, $invoice_id)) {
    ...
    function get_invoice_status_bar($invoice_id = 0)
    {
        ...
        validate_numeric_value($invoice_id);
        $view_data["invoice_info"] = $this->Bills_model->get_details(array("id" => $invoice_id))->getRow();
        $view_data['invoice_status_label'] = $this->_get_invoice_status_label($view_data["invoice_info"]);
        return $this->template->view('bills/invoice_status_bar', $view_data);
    }
    ...
    function update_invoice_status($invoice_id = 0, $status = "")
    {
        ...
        if ($invoice_id && $status) {
            ...
            $this->Bills_model->update_invoice_status($invoice_id, $status);
            ...
            if ($status == "cancelled") {
                ...
                $this->Bills_model->ci_save($data, $invoice_id);
            }
    ...
    function discount_modal_form()
    {
        ...
        $view_data['model_info'] = $this->Bills_model->get_one($invoice_id);

        return $this->template->view('bills/discount_modal_form', $view_data);
    }
    ...
    function save_discount()
    {
        ...
        $save_data = $this->Bills_model->save_invoice_and_update_total($data, $invoice_id);
        if ($save_data) {
    ...
    function file_preview($id = "", $key = "")
    {
        if ($id) {
            ...
            return $this->template->view("bills/file_preview", $view_data);
        } else {
            show_404();
        }
    }
    ...
    function details($invoice_id)
    {
        ...
        if ($invoice_id) {
            ...
            if ($view_data) {
                ...
                return $this->template->view("bills/details", $view_data);
            } else {
                show_404();
            }
        }
    ...
    function payments($invoice_id)
    {
        ...
        if ($invoice_id) {
            ...
            return $this->template->view("bills/payments/index", $view_data);
        } else {
            show_404();
        }
    }
    ...
    function tasks($invoice_id)
    {
        ...
        $view_data["invoice_id"] = $invoice_id;
        return $this->template->view("bills/tasks/index", $view_data);
    }
    ...
    function create_credit_note_modal_form($invoice_id)
    {
        ...
        return $this->template->view('bills/create_credit_note_modal_form', $view_data);
    }
    ...
    function create_credit_note()
    {
        ...
        //create credit note
        $new_invoice_id = $this->Bills_model->ci_save($credit_note_data);

        if ($new_invoice_id) {
            //save credited status for main invoice
            $invoice_data = array("status" => "credited");
            $this->Bills_model->ci_save($invoice_data, $invoice_id);

            //create invoice items
            $items = $this->Bill_items_model->get_details(array("invoice_id" => $invoice_id))->getResult();
            foreach ($items as $item) {
                ...
                $this->Bill_items_model->ci_save($new_invoice_item_data);
            }
    ...
    function invoices_summary()
    {
        ...
        return $this->template->rander("bills/reports/invoices_summary", $view_data);
    }
    ...
    function monthly_invoices_summary()
    {
        ...
        $view_data["currencies_dropdown"] = $this->_get_currencies_dropdown(false);
        return $this->template->view("bills/reports/monthly_invoices_summary", $view_data);
    }
    ...
    function custom_invoices_summary()
    {
        ...
        $view_data["currencies_dropdown"] = $this->_get_currencies_dropdown(false);
        return $this->template->view("bills/reports/custom_invoices_summary", $view_data);
    }
    ...
    function invoices_summary_list_data()
    {
        ...
        $list_data = $this->Bills_model->get_invoices_summary($options)->getResult();
    ...
    private function _make_summary_row($data, $default_currency_symbol)
    {
        ...
        $row_data = array(
            anchor(get_uri("patrons/view/" . $data->client_id), $data->client_name),
            $data->invoice_count,
