...
class Leave_types extends Security_Controller {
    ...
    function modal_form() {
        $view_data['model_info'] = $this->Vacate_types_model->get_one($this->request->getPost('id'));
    ...
    function save() {
        ...
        $save_id = $this->Vacate_types_model->ci_save($data, $id);
        if ($save_id) {
    ...
    function delete() {
        ...
        if ($this->request->getPost('undo')) {
            if ($this->Vacate_types_model->delete($id, true)) {
                echo json_encode(array("success" => true, "data" => $this->_row_data($id), "message" => app_lang('record_undone')));
            } else {
                echo json_encode(array("success" => false, app_lang('error_occurred')));
            }
        } else {
            if ($this->Vacate_types_model->delete($id)) {
    ...
    function list_data() {
        $list_data = $this->Vacate_types_model->get_details()->getResult();
        $result = array();
    ...
    private function _row_data($id) {
        $options = array("id" => $id);
        $data = $this->Vacate_types_model->get_details($options)->getRow();
