...
class Vacates extends Security_Controller {
    ...
    function index($tab = "") {
        ...
        return $this->template->rander("vacates/index", $view_data);
    }
    ...
    function assign_leave_modal_form($applicant_id = 0) {
        ...
        $view_data['leave_types_dropdown'] = array("" => "-") + $this->Vacate_types_model->get_dropdown_list(array("title"), "id", array("status" => "active"));
        $view_data['form_type'] = "assign_leave";
        return $this->template->view('vacates/modal_form', $view_data);
    }
    ...
    function assign_leave() {
        ...
        $save_id = $this->Vacate_applications_model->ci_save($leave_data);
        if ($save_id) {
    ...
    function pending_approval() {
        return $this->template->view("vacates/pending_approval");
    }

    // load all applications tab 
    function all_applications() {
        return $this->template->view("vacates/all_applications");
    }
    ...
    function summary() {
        ...
        return $this->template->view("vacates/summary", $view_data);
    }
    ...
    function pending_approval_list_data() {
        ...
        $list_data = $this->Vacate_applications_model->get_list($options)->getResult();
    ...
    function summary_list_data() {
        ...
        $list_data = $this->Vacate_applications_model->get_summary($options)->getResult();
    ...
    private function _row_data($id) {
        $options = array("id" => $id);
        $data = $this->Vacate_applications_model->get_list($options)->getRow();
    ...
    private function _make_row($data) {
        ...
        $actions = modal_anchor(get_uri("vacates/application_details"), "<i data-feather='$option_icon' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('application_details'), "data-post-id" => $data->id));
        ...
        if ($this->can_delete_leave_application() && $can_manage_application) {
            $actions .= js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("vacates/delete"), "data-action" => "delete-confirmation"));
        }
    ...
    function application_details() {
        ...
        $info = $this->Vacate_applications_model->get_details_info($applicaiton_id);
        if (!$info) {
            show_404();
        }
        ...
        return $this->template->view("vacates/application_details", $view_data);
    }
    ...
    function update_status() {
        ...
        $applicatoin_info = $this->Vacate_applications_model->get_one($applicaiton_id);
        ...
        $save_id = $this->Vacate_applications_model->ci_save($leave_data, $applicaiton_id);
        if ($save_id) {
    ...
    function delete() {
        ...
        $applicatoin_info = $this->Vacate_applications_model->get_one($id);
        $this->access_only_allowed_members($applicatoin_info->applicant_id);

        if ($this->Vacate_applications_model->delete($id)) {
    ...
    function leave_info() {
        ...
        if ($this->request->isAJAX()) {
            ...
            return $this->template->rander("unit_members/leave_info", $view_data);
        }
    }
    ...
    private function _get_leave_types_dropdown_list_for_filter() {
        ...
        $leave_type = $this->Vacate_types_model->get_dropdown_list(array("title"), "id", array("status" => "active"));
    ...
    function file_preview($id = "", $key = "") {
        if ($id) {
            ...
            $leave_info = $this->Vacate_applications_model->get_one($id);
            ...
            return $this->template->view("vacates/file_preview", $view_data);
        } else {
            show_404();
        }
    }
    ...
    function import_leaves_modal_form() {
        ...
        return $this->template->view("vacates/import_leaves_modal_form");
    }
    ...
    function save_leave_from_excel_file() {
        ...
        foreach ($excel_file as $key => $value) { //rows
            ...
            //save leave data
            $leave_save_id = $this->Vacate_applications_model->ci_save($leave_data);
            if (!$leave_save_id) {
                continue;
            }
        }
    ...
    private function _get_leave_type_id($leave_type = "") {
        ...
        $existing_leave_type = $this->Vacate_types_model->get_one_where(array("title" => $leave_type, "deleted" => 0));
        if ($existing_leave_type->id) {
            ...
            return $this->Vacate_types_model->ci_save($leave_type_data);
        }
    }
