...
class Evaluation_requests extends Security_Controller {
    ...
    function index() {
        ...
        return $this->template->rander('evaluation_requests/index', $view_data);
    }

    //view estimate request
    function view_estimate_request($id = 0) {
        validate_numeric_value($id);
        $model_info = $this->Evaluation_requests_model->get_details(array("id" => $id))->getRow();
        ...
        if ($model_info->is_lead) {
            $view_data['lead_info'] = $this->Patrons_model->get_details(array("id" => $model_info->client_id))->getRow();
        }
        ...
        $view_data["estimates"] = $this->Evaluations_model->get_all_where(array("estimate_request_id" => $id))->getResult();

        return $this->template->rander('evaluation_requests/view_estimate_request', $view_data);
    }

    // download files 
    function download_estimate_request_files($id = 0) {
        $this->access_only_allowed_members();
        $info = $this->Evaluation_requests_model->get_one($id);
    ...
    function estimate_request_list_data() {
        ...
        $list_data = $this->Evaluation_requests_model->get_details($options)->getResult();
        $result = array();
    ...
    private function _make_estimate_request_row($data) {
        ...
        $edit = '<li role="presentation">' . modal_anchor(get_uri("evaluation_requests/edit_estimate_request_modal_form"), "<i data-feather='edit' class='icon-16'></i> " . app_lang('edit'), array("title" => app_lang('estimate_request'), "data-post-view" => "details", "data-post-id" => $data->id, "class" => "dropdown-item")) . '</li>';

        $request_status = $this->template->view("evaluation_requests/estimate_request_status_options", array("model_info" => $data));

        $add_estimate = '<li role="presentation">' . modal_anchor(get_uri("evaluations/modal_form"), "<i data-feather='plus-circle' class='icon-16'></i> " . app_lang('add_estimate'), array('title' => app_lang('add_estimate'), "data-post-estimate_request_id" => $data->id, "data-post-client_id" => $data->client_id, "class" => "dropdown-item")) . '</li>';

        $delete = '<li role="presentation">' . js_anchor("<i data-feather='x' class='icon-16'></i>" . app_lang('delete'), array('title' => app_lang('delete_estimate_form'), "class" => "delete dropdown-item", "data-id" => $data->id, "data-action-url" => get_uri("evaluation_requests/delete_estimate_request"), "data-action" => "delete-confirmation")) . '</li>';
        ...
        return array(
            anchor(get_uri("evaluation_requests/view_estimate_request/" . $data->id), app_lang("estimate_request") . " - " . $data->id),
            $client,
            $data->form_title,
            $assigned_to,
            $data->created_at,
    ...
    private function _estimate_request_row_data($id) {
        $options = array("id" => $id);
        $data = $this->Evaluation_requests_model->get_details($options)->getRow();
    ...
    function delete_estimate_request() {
        ...
        if ($this->Evaluation_requests_model->delete($id)) {
    ...
    function estimate_request_filed_list_data($id = 0) {
        validate_numeric_value($id);
        $model_info = $this->Evaluation_requests_model->get_one($id);
    ...
    function estimate_forms() {
        ...
        return $this->template->rander('evaluation_requests/estimate_forms');
    }
    ...
    function estimate_request_modal_form() {
        ...
        $view_data['model_info'] = $this->Evaluation_forms_model->get_one($this->request->getPost('id'));
        ...
        return $this->template->view('evaluation_requests/estimate_request_modal_form', $view_data);
    }
    ...
    function save_estimate_request_form() {
        ...
        $save_id = $this->Evaluation_forms_model->ci_save($data, $id);
        if ($save_id) {
    ...
    function estimate_forms_list_data() {
        ...
        $list_data = $this->Evaluation_forms_model->get_details()->getResult();
        $result = array();
        foreach ($list_data as $data) {
    ...
    private function _form_row_data($id) {
        $options = array("id" => $id);
        $data = $this->Evaluation_forms_model->get_details($options)->getRow();
        return $this->_make_form_row($data);
    }

    //prepare a row of estimates forms list
    private function _make_form_row($data) {
        $title = anchor(get_uri("evaluation_requests/edit_estimate_form/" . $data->id), $data->title, array("class" => "edit", "title" => app_lang('edit_estimate_form'), "data-post-id" => $data->id));
        ...
        $embedded_code = "";
        if ($data->public) {
            $embedded_code = modal_anchor(get_uri("evaluation_requests/embedded_code_modal_form"), "<i data-feather='code' class='icon-16'></i>", array("title" => app_lang('embed'), "class" => "edit", "data-post-id" => $data->id));
        }
        ...
        return array(
            ...
            modal_anchor(get_uri("evaluation_requests/estimate_request_modal_form"), "<i data-feather='edit' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('edit_form'), "data-post-id" => $data->id))
            . js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete_estimate_form'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("evaluation_requests/delete_estimate_request_form"), "data-action" => "delete"))
        );
    }
    ...
    function edit_estimate_form($id = 0) {
        ...
        $model_info = $this->Evaluation_forms_model->get_one($id);
        $view_data['model_info'] = $model_info;
        return $this->template->rander('evaluation_requests/edit_estimate_form', $view_data);
    }
    ...
    function edit_estimate_request_modal_form() {
        ...
        $model_info = $this->Evaluation_requests_model->get_one($this->request->getPost("id"));
        //prepare assign to list
        $assigned_to_dropdown = array("" => "-") + $this->Users_model->get_dropdown_list(array("first_name", "last_name"), "id", array("deleted" => 0, "user_type" => "staff"));
        $view_data['assigned_to_dropdown'] = $assigned_to_dropdown;
        $view_data['model_info'] = $model_info;

        return $this->template->view('evaluation_requests/edit_estimate_request_modal_form', $view_data);
    }
    ...
    function update_estimate_request() {
        ...
        $save_id = $this->Evaluation_requests_model->ci_save($data, $id);
        if ($save_id) {
    ...
    function preview_estimate_form($id = 0) {
        ...
        $model_info = $this->Evaluation_forms_model->get_one($id);
        $view_data['model_info'] = $model_info;
        return $this->template->rander('evaluation_requests/preview_estimate_form', $view_data);
    }
    ...
    function estimate_form_field_modal_form($estimate_form_id = 0) {
        ...
        return $this->template->view('evaluation_requests/estimate_form_field_modal_form', $view_data);
    }
    ...
    private function _make_form_field_row($data) {
        ...
        return array(
            $field,
            $data->sort,
            modal_anchor(get_uri("evaluation_requests/estimate_form_field_modal_form/" . $estimate_form_id), "<i data-feather='edit' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('edit_form'), "data-post-id" => $data->id))
            . js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete_estimate_form'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("evaluation_requests/estimate_form_field_delete"), "data-action" => "delete"))
        );
    }
    ...
    function request_an_estimate_modal_form() {
        ...
        $view_data["estimate_forms"] = $this->Evaluation_forms_model->get_all_where(array("status" => "active", "deleted" => 0))->getResult();
        return $this->template->view("evaluation_requests/request_an_estimate_modal_form", $view_data);
    }
    ...
    function submit_estimate_request_form($id = 0) {
        ...
        $model_info = $this->Evaluation_forms_model->get_one_where(array("id" => $id, "status" => "active", "deleted" => 0));
        ...
        if (get_setting("module_estimate_request") && $model_info->id) {
            ...
            return $this->template->rander('evaluation_requests/submit_estimate_request_form', $view_data);
        } else {
            show_404();
        }
    }
    ...
    function save_estimate_request() {
        ...
        $save_id = $this->Evaluation_requests_model->ci_save($request_data);
        if ($save_id) {
    ...
    function embedded_code_modal_form() {
        $model_info = $this->Evaluation_forms_model->get_one($this->request->getPost('id'));
        ...
        return $this->template->view('evaluation_requests/embedded_code_modal_form', $view_data);
    }
