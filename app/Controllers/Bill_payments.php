...
class Bill_payments extends Security_Controller {
    ...
    function index() {
        if ($this->login_user->user_type === "staff") {
            ...
            return $this->template->rander("bills/payment_received", $view_data);
        } else {
            ...
            return $this->template->rander("patrons/payments/index", $view_data);
        }
    }
    ...
    function yearly() {
        return $this->template->view("bills/yearly_payments");
    }

    //load custom payment list
    function custom() {
        return $this->template->view("bills/custom_payments_list");
    }
    ...
    function payment_modal_form() {
        ...
        $view_data['model_info'] = $this->Bill_payments_model->get_one($this->request->getPost('id'));
        ...
        if (!$invoice_id) {
            ...
            $invoices = $this->Bills_model->get_invoices_dropdown_list()->getResult();
        ...
        if (!$view_data['model_info']->amount && $invoice_id) {
            $amount = to_decimal_format($this->Bills_model->get_invoice_total_summary($invoice_id)->balance_due);
        }
        ...
        return $this->template->view('bills/payment_modal_form', $view_data);
    }
    ...
    function save_payment() {
        ...
        $invoice_payment_id = $this->Bill_payments_model->ci_save($invoice_payment_data, $id);
        if ($invoice_payment_id) {
            ...
            $item_info = $this->Bill_payments_model->get_details($options)->getRow();
    ...
    function delete_payment() {
        ...
        if ($this->request->getPost('undo')) {
            if ($this->Bill_payments_model->delete($id, true)) {
                $options = array("id" => $id);
                $item_info = $this->Bill_payments_model->get_details($options)->getRow();
            ...
            if ($this->Bill_payments_model->delete($id)) {
                $item_info = $this->Bill_payments_model->get_one($id);
    ...
    function payment_list_data($invoice_id = 0) {
        ...
        $list_data = $this->Bill_payments_model->get_details($options)->getResult();
        $result = array();
    ...
    private function _make_payment_row($data) {
        ...
        if ($this->login_user->user_type == "staff") {
            $invoice_url = anchor(get_uri("bills/view/" . $data->invoice_id), $data->display_id);
        } else {
            $invoice_url = anchor(get_uri("bills/preview/" . $data->invoice_id), $data->display_id);
        }
        return array(
            $invoice_url,
            $data->payment_date,
            ...
            modal_anchor(get_uri("bill_payments/payment_modal_form"), "<i data-feather='edit' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('edit_payment'), "data-post-id" => $data->id, "data-post-invoice_id" => $data->invoice_id,))
            . js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("bill_payments/delete_payment"), "data-action" => "delete"))
        );
    }
    ...
    function yearly_chart() {
        $view_data["currencies_dropdown"] = $this->_get_currencies_dropdown();
        return $this->template->view("bills/yearly_payments_chart", $view_data);
    }
    ...
    function yearly_chart_data() {
        ...
        if ($year) {
            $currency = $this->request->getPost("currency");
            $payments = $this->Bill_payments_model->get_yearly_payments_chart($year, $currency);
            $values = array();
    ...
    function payments_summary() {
        ...
        $view_data['payment_method_dropdown'] = $this->get_payment_method_dropdown();
        return $this->template->rander("bills/reports/yearly_payments_summary", $view_data);
    }
    ...
    function yearly_payment_summary_list_data() {
        ...
        $list_data = $this->Bill_payments_model->get_yearly_summary_details($options)->getResult();
    ...
    function clients_payment_summary() {
        $view_data["currencies_dropdown"] = $this->_get_currencies_dropdown(false);
        $view_data['payment_method_dropdown'] = $this->get_payment_method_dropdown();
        return $this->template->view("bills/reports/clients_payment_summary", $view_data);
    }
    ...
    function clients_payment_summary_list_data() {
        ...
        $list_data = $this->Bill_payments_model->get_clients_summary_details($options)->getResult();
