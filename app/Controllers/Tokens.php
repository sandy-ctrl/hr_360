...
class Tokens extends Security_Controller {

    protected $Token_templates_model;

    function __construct() {
        parent::__construct();
        $this->init_permission_checker("ticket");

        $this->Token_templates_model = model('App\Models\Token_templates_model');
    }
    ...
    function index($status = "", $ticket_type_id = 0) {
        ...
        if ($this->login_user->user_type === "staff") {
            ...
            return $this->template->rander("tokens/tickets_list", $view_data);
        } else {
            ...
            return $this->template->rander("patrons/tickets/index", $view_data);
        }
    }
    ...
    function modal_form() {
        ...
        $ticket_info = $this->Tokens_model->get_one($this->request->getPost("id"));

        $projects = $this->Enterprises_model->get_dropdown_list(array("title"), "id", array("client_id" => $ticket_info->client_id, "project_type" => "client_project"));
        if ($this->login_user->user_type == "client") {
            $projects = $this->Enterprises_model->get_dropdown_list(array("title"), "id", array("client_id" => $this->login_user->client_id, "project_type" => "client_project"));
        ...
        $model_info = $this->Tokens_model->get_one($id);
        $project_id = $this->request->getPost('project_id');

        //here has a project id. now set the client from the project
        if ($project_id) {
            $client_id = $this->Enterprises_model->get_one($project_id)->client_id;
        ...
        $view_data['ticket_types_dropdown'] = $this->Token_types_model->get_dropdown_list(array("title"), "id", $where);

        $view_data['model_info'] = $model_info;
        $view_data['client_id'] = $ticket_info->client_id;
        $view_data['clients_dropdown'] = array("" => "-") + $this->Patrons_model->get_dropdown_list(array("company_name"), "id", array("is_lead" => 0));
        $view_data['show_project_reference'] = get_setting('project_reference_in_tickets');
        ...
        if ($this->login_user->user_type == "client") {
            ...
            $view_data['projects_dropdown'] = $this->Enterprises_model->get_dropdown_list(array("title"), "id", array("project_type" => "client_project"));
        }
        ...
        return $this->template->view('tokens/modal_form', $view_data);
    }
    ...
    function get_project_suggestion($client_id = 0) {
        ...
        $projects = $this->Enterprises_model->get_dropdown_list(array("title"), "id", array("client_id" => $client_id, "project_type" => "client_project"));
        $suggestion = array(array("id" => "", "text" => "-"));
        foreach ($projects as $key => $value) {
            $suggestion[] = array("id" => $key, "text" => $value);
        }
    ...
    function save() {
        ...
        $ticket_id = $this->Tokens_model->ci_save($ticket_data, $id);
        ...
        if ($ticket_id) {
            ...
            if (!$id) {
                ...
                $ticket_comment_id = $this->Token_comments_model->ci_save($comment_data);
    ...
    function list_data($is_widget = 0) {
        ...
        $result = $this->Tokens_model->get_details($all_options);
    ...
    function ticket_list_data_of_client($client_id, $is_widget = 0) {
        ...
        $list_data = $this->Tokens_model->get_details($options)->getResult();
        $result = array();
        foreach ($list_data as $data) {
    ...
    private function _row_data($id) {
        ...
        $data = $this->Tokens_model->get_details($options)->getRow();
    ...
    private function _make_row($data, $custom_fields) {
        ...
        $title = anchor(get_uri("tokens/view/" . $data->id), $data->title);
        ...
        $row_data = array(
            $data->id,
            anchor(get_uri("tokens/view/" . $data->id), get_ticket_id($data->id), array("class" => "js-ticket", "data-id" => $data->id, "title" => "")),
            $title,
            $data->company_name ? anchor(get_uri("patrons/view/" . $data->client_id), $data->company_name) : ($data->creator_name . " [" . app_lang("unknown_client") . "]"),
            $data->project_title ? anchor(get_uri("enterprises/view/" . $data->project_id), $data->project_title) : "-",
        ...
        if ($this->login_user->user_type == "staff") {
            $edit = '<li role="presentation">' . modal_anchor(get_uri("tokens/modal_form"), "<i data-feather='edit' class='icon-16'></i> " . app_lang('edit'), array("title" => app_lang('edit'), "data-post-view" => "details", "data-post-id" => $data->id, "class" => "dropdown-item")) . '</li>';

            //show option to close/open the tickets
            $status = "";
            if ($data->status === "closed") {
                $status = '<li role="presentation">' . js_anchor("<i data-feather='check-circle' class='icon-16'></i> " . app_lang('mark_as_open'), array('title' => app_lang('mark_as_open'), "class" => "dropdown-item", "data-action-url" => get_uri("tokens/save_ticket_status/$data->id/open"), "data-action" => "update")) . '</li>';
            } else {
                $status = '<li role="presentation">' . js_anchor("<i data-feather='check-circle' class='icon-16'></i> " . app_lang('mark_as_closed'), array('title' => app_lang('mark_as_closed'), "class" => "dropdown-item", "data-action-url" => get_uri("tokens/save_ticket_status/$data->id/closed"), "data-action" => "update")) . '</li>';
            }

            $assigned_to = "";
            if ($data->assigned_to === "0") {
                $assigned_to = '<li role="presentation">' . js_anchor("<i data-feather='user' class='icon-16'></i> " . app_lang('assign_to_me'), array('title' => app_lang('assign_myself_in_this_ticket'), "data-action-url" => get_uri("tokens/assign_to_me/$data->id"), "data-action" => "update", "class" => "dropdown-item")) . '</li>';
            }
            ...
            if ($this->can_delete_tickets()) {
                $delete_ticket = '<li role="presentation">' . js_anchor("<i data-feather='x' class='icon-16'></i>" . app_lang('delete'), array('title' => app_lang('delete'), "class" => "delete dropdown-item", "data-id" => $data->id, "data-action-url" => get_uri("tokens/delete"), "data-action" => "delete-confirmation")) . '</li>';
            }
            ...
            $modal_view = modal_anchor(get_uri("tokens/view"), "<i data-feather='tablet' class='icon-16'></i>", array("class" => "action-option", "title" => app_lang('ticket_info') . " #$data->id", "data-post-id" => $data->id, "data-post-view_type" => "modal_view", "data-modal-fullscreen" => "1", "data-modal-custom-bg" => "1"));
    ...
    function view($ticket_id = 0) {
        ...
        if ($ticket_id) {
            ...
            $ticket_info = $this->Tokens_model->get_details($options)->getRow();
            ...
            if ($ticket_info) {
                ...
                $view_data['comments'] = $this->Token_comments_model->get_details($comments_options)->getResult();
                ...
                if ($view_type == "modal_view") {
                    return $this->template->view("tokens/view", $view_data);
                } else {
                    return $this->template->rander("tokens/view", $view_data);
                }
            } else {
                show_404();
            }
    ...
    function delete() {
        ...
        if ($this->Tokens_model->delete_ticket_and_sub_items($id)) {
    ...
    function save_comment() {
        ...
        $comment_id = $this->Token_comments_model->ci_save($comment_data);
        if ($comment_id) {
            ...
            $this->Tokens_model->ci_save($ticket_data, $ticket_id);

            $comments_options = array("id" => $comment_id);
            $view_data['comment'] = $this->Token_comments_model->get_details($comments_options)->getRow();
            $comment_view = $this->template->view("tokens/comment_row", $view_data);
    ...
    function save_ticket_status($ticket_id = 0, $status = "closed") {
        ...
        if ($ticket_id) {
            ...
            $save_id = $this->Tokens_model->ci_save($data, $ticket_id);
            if ($save_id) {
                if ($status == "open") {
                    ...
                    $this->Tokens_model->ci_save($closed_data, $ticket_id);
                }
    ...
    function download_comment_files($id) {
        ...
        $files = $this->Token_comments_model->get_one($id)->files;
    ...
    function ticket_templates() {
        ...
        return $this->template->rander("tokens/templates/index");
    }

    private function can_view_ticket_template($id = 0) {
        if ($id) {
            $template_info = $this->Token_templates_model->get_one($id);
    ...
    function ticket_template_modal_form() {
        ...
        $view_data['ticket_types_dropdown'] = array("" => "-") + $this->Token_types_model->get_dropdown_list(array("title"), "id", $where);

        $id = $this->request->getPost('id');
        $this->can_edit_ticket_template($id);

        $view_data['model_info'] = $this->Token_templates_model->get_one($id);

        return $this->template->view('tokens/templates/modal_form', $view_data);
    }
    ...
    function save_ticket_template() {
        ...
        $save_id = $this->Token_templates_model->ci_save($ticket_template_data, $id);
    ...
    function delete_ticket_template() {
        ...
        if ($this->Token_templates_model->delete($id)) {
            echo json_encode(array("success" => true, 'message' => app_lang('record_deleted')));
        } else {
            echo json_encode(array("success" => false, 'message' => app_lang('record_cannot_be_deleted')));
        }
    ...
    function ticket_template_list_data($view_type = "", $ticket_type_id = 0) {
        ...
        $list_data = $this->Token_templates_model->get_details($options)->getResult();
    ...
    private function _row_data_for_ticket_templates($id) {
        ...
        $data = $this->Token_templates_model->get_details($options)->getRow();
    ...
    private function _make_row_for_ticket_templates($data, $view_type = "") {
        ...
        if ($view_type == "modal") {
            ...
            $title = modal_anchor(get_uri("tokens/ticket_template_view/" . $data->id), $data->title, array("class" => "edit", "title" => app_lang('ticket_template'), "data-post-id" => $data->id));
        ...
        $actions = modal_anchor(get_uri("tokens/ticket_template_view/" . $data->id), "<i data-feather='cloud-lightning' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('template_details'), "data-modal-title" => app_lang('template'), "data-post-id" => $data->id));
        if ($data->created_by == $this->login_user->id || $this->login_user->is_admin) {
            $actions = modal_anchor(get_uri("tokens/ticket_template_modal_form"), "<i data-feather='edit' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('edit_template'), "data-post-id" => $data->id))
                    . js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("tokens/delete_ticket_template"), "data-action" => "delete-confirmation"));
        }
    ...
    function ticket_template_view($id) {
        ...
        return $this->template->view('tokens/templates/view', $view_data);
    }
    ...
    function insert_template_modal_form() {
        ...
        return $this->template->view("tokens/templates/insert_template_modal_form", $view_data);
    }
    ...
    function add_client_modal_form($ticket_id = 0) {
        if ($ticket_id) {
            ...
            $view_data['clients_dropdown'] = array("" => "-") + $this->Patrons_model->get_dropdown_list(array("company_name"));
            $view_data['ticket_id'] = $ticket_id;

            return $this->template->view("tokens/add_client_modal_form", $view_data);
        }
    }
    ...
    function settings_modal_form() {
        return $this->template->view('tokens/settings/modal_form');
    }
    ...
    private function _get_ticket_types_dropdown_list_for_filter($ticket_type_id = 0) {
        ...
        $ticket_type = $this->Token_types_model->get_dropdown_list(array("title"), "id", $where);
    ...
    function batch_update_modal_form($ticket_ids = "") {
        ...
        return $this->template->view("tokens/batch_update/modal_form", $view_data);
    }
        ...
        if ($batch_fields) {
            ...
            if ($ticket_ids) {
                ...
                foreach ($tickets_ids_array as $id) {
                    $this->validate_ticket_access($id);
                    $this->Tokens_model->ci_save($data, $id);
                }
    ...
    function delete_comment($id = 0) {
        ...
        $comment_info = $this->Token_comments_model->get_one($id);
        ...
        if ($this->Token_comments_model->delete($id) && $comment_info->files) {
    ...
    function merge_ticket_modal_form() {
        ...
        $ticket_info = $this->Tokens_model->get_one($ticket_id);
        ...
        return $this->template->view('tokens/merge_ticket_modal_form', $view_data);
    }
    ...
    function save_merge_ticket() {
        ...
        $save_id = $this->Tokens_model->ci_save($ticket_data, $merge_with_ticket_id);

        if ($save_id) {
            $comments = $this->Token_comments_model->get_all_where(array("ticket_id" => $merge_with_ticket_id))->getResult();
            foreach ($comments as $comment) {
                $comment_data["ticket_id"] = $ticket_id;

                $this->Token_comments_model->ci_save($comment_data, $comment->id);
            }

            $merged_ticket_data["status"] = "closed";
            $this->Tokens_model->ci_save($merged_ticket_data, $merge_with_ticket_id);
    ...
    function tasks($ticket_id) {
        ...
        return $this->template->view("tokens/tasks/index", $view_data);
    }
    ...
    function tickets_chart_report() {
        ...
        return $this->template->rander("tokens/reports/chart_report_container", $view_data);
    }
    ...
    function tickets_chart_report_data() {
        ...
        $options["status"] = "open";
        $open_tickets_list = $this->Tokens_model->get_ticket_statistics($options)->getResult();
        ...
        $closed_tickets_list = $this->Tokens_model->get_ticket_statistics($options)->getResult();
        ...
        return $this->template->view("tokens/reports/chart_report_view", $view_data);
    }

    private function _get_clients_dropdown() {
        $clients_dropdown = array(array("id" => "", "text" => "- " . app_lang("client") . " -"));

        $clients_list = $this->Patrons_model->get_dropdown_list(array("company_name"), "id", array("is_lead" => 0));
