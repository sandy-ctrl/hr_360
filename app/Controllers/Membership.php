...
class Membership extends Security_Controller {
    ...
    private function _can_update_subscription_status($subscription_id) {
        if ($this->login_user->user_type == "client") {
            $subscription_info = $this->Membership_model->get_one($subscription_id);
    ...
    function index() {
        ...
        if ($this->login_user->user_type === "staff") {
            ...
            return $this->template->rander("membership/index", $view_data);
        } else {
            ...
            return $this->template->rander("patrons/subscriptions/index", $view_data);
        }
    }
    ...
    function modal_form() {
        ...
        $model_info = $this->Membership_model->get_one($subscription_id);
        ...
        return $this->template->view('membership/modal_form', $view_data);
    }
    ...
    function save() {
        ...
        if ($id) {
            $subscription_info = $this->Membership_model->get_one($id);
        ...
        $subscription_id = $this->Membership_model->ci_save($subscription_data, $id);
        if ($subscription_id) {
    ...
    function subscription_list_data_of_client($client_id) {
        ...
        $list_data = $this->Membership_model->get_details($options)->getResult();
        $result = array();
    ...
    private function _row_data($id) {
        ...
        $data = $this->Membership_model->get_details($options)->getRow();
    ...
    private function _make_row($data, $custom_fields) {
        ...
        if ($this->login_user->user_type == "staff") {
            $subscription_url = anchor(get_uri("membership/view/" . $data->id), get_subscription_id($data->id));
        } else {
            $subscription_url = anchor(get_uri("membership/preview/" . $data->id), get_subscription_id($data->id));
        }
    ...
    private function _make_options_dropdown($data) {
        ...
        if ($data->status !== "active" && $data->status !== "cancelled") {
            $options .= '<li role="presentation">' . modal_anchor(get_uri("membership/modal_form"), "<i data-feather='edit' class='icon-16'></i> " . app_lang('edit'), array("title" => app_lang('edit_subscription'), "data-post-id" => $data->id, "class" => "dropdown-item")) . '</li>';
        }

        if ($data->status !== "active") {
            $options .= '<li role="presentation">' . js_anchor("<i data-feather='x' class='icon-16'></i>" . app_lang('delete'), array('title' => app_lang('delete_subscription'), "class" => "delete dropdown-item", "data-id" => $data->id, "data-action-url" => get_uri("membership/delete"), "data-action" => "delete-confirmation")) . '</li>';
        }
    ...
    function view($subscription_id = 0) {
        ...
        return $this->template->rander("membership/view", $view_data);
    }
    ...
    private function _get_subscription_total_view($subscription_id = 0) {
        $view_data["subscription_total_summary"] = $this->Membership_model->get_subscription_total_summary($subscription_id);
        $view_data["subscription_id"] = $subscription_id;
        $view_data["can_edit_subscriptions"] = $this->can_edit_subscriptions();
        return $this->template->view('membership/subscription_total_section', $view_data);
    }
    ...
    function item_modal_form() {
        ...
        $view_data['subscription_id'] = $subscription_id;
        return $this->template->view('membership/item_modal_form', $view_data);
    }
    ...
    private function _make_item_row($data) {
        ...
        return array(
            ...
            modal_anchor(get_uri("membership/item_modal_form"), "<i data-feather='edit' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('edit_subscription'), "data-post-id" => $data->id))
        );
    }
    ...
    function preview($subscription_id = 0) {
        if ($subscription_id) {
            ...
            $view_data['subscription_preview'] = view("membership/subscription_pdf", $view_data);
            ...
            return $this->template->rander("membership/subscription_preview", $view_data);
        } else {
            show_404();
        }
    }
    ...
    function update_subscription_status($subscription_id = 0, $status = "") {
        ...
        if ($subscription_id && $status) {
            ...
            $this->Membership_model->update_subscription_status($subscription_id, $status);
            ...
            if ($status == "cancelled") {
                ...
                $this->Membership_model->ci_save($data, $subscription_id);
    ...
    function activate_as_stripe_subscription_modal_form($subscription_id) {
        ...
        validate_numeric_value($subscription_id);
        $this->validate_subscription_access($subscription_id);
        $view_data["subscription_info"] = $this->Membership_model->get_one($subscription_id);
        ...
        return $this->template->view('membership/activate_as_stripe_subscription_modal_form', $view_data);
    }
    ...
    function activate_as_stripe_subscription() {
        ...
        $subscription_info = $this->Membership_model->get_details(array("id" => $subscription_id))->getRow();
        ...
        $save_id = $this->Membership_model->ci_save($subscription_data, $subscription_id);
        if ($save_id) {
    ...
    function file_preview($id = "", $key = "") {
        if ($id) {
            ...
            return $this->template->view("membership/file_preview", $view_data);
        } else {
            show_404();
        }
    }
    ...
    function activate_as_internal_subscription_modal_form($subscription_id) {
        ...
        return $this->template->view('membership/activate_as_internal_subscription_modal_form', $view_data);
    }
    ...
    function activate_as_internal_subscription() {
        ...
        $subscription_info = $this->Membership_model->get_subscription_total_summary($subscription_id);
        ...
        if ($subscription_id) {
            $this->Membership_model->update_subscription_status($subscription_id, "active");
    ...
    function details($subscription_id) {
        ...
        return $this->template->view("membership/details", $view_data);
    }
    ...
    function invoices($subscription_id) {
        ...
        if ($subscription_id) {
            $view_data['subscription_id'] = $subscription_id;
            $view_data['subscription_info'] = $this->Membership_model->get_details(array("id" => $subscription_id))->getRow();

            $view_data["custom_field_headers"] = $this->Custom_fields_model->get_custom_field_headers_for_table("invoices", $this->login_user->is_admin, $this->login_user->user_type);
            $view_data["custom_field_filters"] = $this->Custom_fields_model->get_custom_field_filters("invoices", $this->login_user->is_admin, $this->login_user->user_type);

            return $this->template->view("membership/invoices/index", $view_data);
        }
    }
    ...
    function tasks($subscription_id) {
        ...
        return $this->template->view("membership/tasks/index", $view_data);
    }
