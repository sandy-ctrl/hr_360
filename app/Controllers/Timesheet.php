...
class Timesheet extends Security_Controller {
    ...
    function index($tab = "") {
        ...
        return $this->template->rander("timesheet/index", $view_data);
    }
    ...
    function modal_form() {
        ...
        return $this->template->view('timesheet/modal_form', $view_data);
    }
    ...
    function note_modal_form($user_id = 0) {
        ...
        $view_data['model_info'] = $this->Attendance_model->get_one($this->request->getPost('id'));
        return $this->template->view('timesheet/note_modal_form', $view_data);
    }
    ...
    function attendance_info() {
        ...
        if ($this->request->isAJAX()) {
            ...
            return $this->template->rander("unit_members/attendance_info", $view_data);
        }
    }
    ...
    private function _make_row($data) {
        ...
        $option_links = modal_anchor(get_uri("timesheet/modal_form"), "<i data-feather='edit' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('edit_attendance'), "data-post-id" => $data->id))
                . js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete_attendance'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("timesheet/delete"), "data-action" => "delete"));
        ...
        $note_link = modal_anchor(get_uri("timesheet/note_modal_form"), "<i data-feather='message-circle' class='icon-16'></i>", array("class" => "edit text-muted", "title" => app_lang("note"), "data-post-id" => $data->id));
        if ($data->note) {
            $note_link = modal_anchor(get_uri("timesheet/note_modal_form"), "<i data-feather='message-circle' class='icon-16 icon-fill-secondary'></i>", array("class" => "edit text-muted", "title" => $note_title, "data-modal-title" => app_lang("note"), "data-post-id" => $data->id));
        }
    ...
    function custom() {
        $view_data['team_members_dropdown'] = json_encode($this->_get_members_dropdown_list_for_filter());
        return $this->template->view("timesheet/custom_list", $view_data);
    }

    //load the clocked in members list view of attendance list 
    function members_clocked_in() {
        return $this->template->view("timesheet/members_clocked_in");
    }
    ...
    function summary() {
        $view_data['team_members_dropdown'] = json_encode($this->_get_members_dropdown_list_for_filter());
        return $this->template->view("timesheet/summary_list", $view_data);
    }
    ...
    function summary_details() {
        $view_data['team_members_dropdown'] = json_encode($this->_get_members_dropdown_list_for_filter());
        return $this->template->view("timesheet/summary_details_list", $view_data);
    }
    ...
    function clock_in_out() {
        return $this->template->view("timesheet/clock_in_out");
    }
    ...
    private function _make_clock_in_out_row($data) {
        if (isset($data->attendance_id)) {
            ...
            $view_data = modal_anchor(get_uri("timesheet/note_modal_form/$data->id"), "<i data-feather='log-out' class='icon-16'></i> " . app_lang('clock_out'), array("class" => "btn btn-default", "title" => app_lang('clock_out'), "id" => "timecard-clock-out", "data-post-id" => $data->attendance_id, "data-post-clock_out" => 1, "data-post-id" => $data->id));
        } else {
            $status = "<div class='mb15'>" . app_lang('not_clocked_id_yet') . "</div>";
            $view_data = js_anchor("<i data-feather='log-in' class='icon-16'></i> " . app_lang('clock_in'), array('title' => app_lang('clock_in'), "class" => "btn btn-default spinning-btn", "data-action-url" => get_uri("timesheet/log_time/$data->id"), "data-action" => "update", "data-inline-loader" => "1", "data-post-id" => $data->id));
        }
