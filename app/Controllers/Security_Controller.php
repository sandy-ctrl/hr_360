...
class Security_Controller extends App_Controller {
    ...
    protected function _get_currencies_dropdown($support_empty_value = true) {
        $used_currencies = $this->Bills_model->get_used_currencies_of_client()->getResult();
    ...
    protected function _get_projects_dropdown_for_income_and_expenses($type = "all") {
        $projects = $this->Bill_payments_model->get_used_projects($type)->getResult();
    ...
    protected function get_conversion_rate_with_currency_symbol() {
        ...
        $clients = $this->Patrons_model->get_conversion_rate_with_currency_symbol()->getResult();
    ...
    protected function can_access_this_lead($lead_id = 0) {
        ...
        if ($this->login_user->is_admin) {
            ...
            $lead_info = $this->Patrons_model->get_one($lead_id);
    ...
    protected function _make_order_item_row($data) {
        ...
        return array(
            ...
            modal_anchor(get_uri("outlet/item_modal_form"), "<i data-feather='edit' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('edit_item'), "data-post-id" => $data->id, "data-post-order_id" => $data->order_id))
            . js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("outlet/delete_item"), "data-action" => "delete"))
        );
    }
    ...
    protected function is_invoice_editable($_invoice, $is_clone = 0) {
        if (get_setting("enable_invoice_lock_state")) {
            $invoice_info = is_object($_invoice) ? $_invoice : $this->Bills_model->get_one($_invoice);
