...
class Initiates extends Security_Controller {
    ...
    function index() {
        ...
        return $this->template->rander("initiates/index", $view_data);
    }
    ...
    function modal_form() {
        ...
        return $this->template->view('initiates/modal_form', $view_data);
    }
    ...
    private function make_lead_modal_form_data($lead_id = 0) {
        ...
        $view_data['model_info'] = $this->Patrons_model->get_one($lead_id);
    ...
    private function _make_row($data, $custom_fields) {
        ...
        $row_data = array(
            anchor(get_uri("initiates/view/" . $data->id), $data->company_name),
        ...
        $row_data[] = modal_anchor(get_uri("initiates/modal_form"), "<i data-feather='edit' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('edit_lead'), "data-post-id" => $data->id))
                . js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete_lead'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("initiates/delete"), "data-action" => "delete-confirmation"));
    ...
    function view($client_id = 0, $tab = "") {
        ...
        if ($client_id) {
            ...
            $lead_info = $this->Patrons_model->get_details($options)->getRow();
            $this->validate_lead_access($client_id);
            ...
            if ($lead_info && $lead_info->is_lead) {
                ...
                return $this->template->rander("initiates/view", $view_data);
            } else {
                show_404();
            }
        } else {
    ...
    function estimates($client_id) {
        if ($client_id) {
            validate_numeric_value($client_id);
            $this->validate_lead_access($client_id);
            $view_data["lead_info"] = $this->Patrons_model->get_one($client_id);
            ...
            return $this->template->view("initiates/estimates/estimates", $view_data);
        }
    }
    ...
    function estimate_requests($client_id) {
        if ($client_id) {
            ...
            $this->validate_lead_access($client_id);
            $view_data['client_id'] = $client_id;
            return $this->template->view("initiates/estimates/estimate_requests", $view_data);
        }
    }
    ...
    function notes($client_id) {
        if ($client_id) {
            ...
            $this->validate_lead_access($client_id);
            $view_data['client_id'] = $client_id;
            return $this->template->view("initiates/notes/index", $view_data);
        }
    }
    ...
    function files($client_id) {
        ...
        $view_data['client_id'] = $client_id;
        return $this->template->view("initiates/files/index", $view_data);
    }
    ...
    function file_modal_form() {
        ...
        $view_data['client_id'] = $client_id;
        return $this->template->view('initiates/files/modal_form', $view_data);
    }
    ...
    private function _make_file_row($data) {
        ...
        $description = "<div class='float-start'>" .
                js_anchor(remove_file_prefix($data->file_name), array('title' => "", "data-toggle" => "app-modal", "data-sidebar" => "0", "data-url" => get_uri("initiates/view_file/" . $data->id)));
        ...
        $options = anchor(get_uri("initiates/download_file/" . $data->id), "<i data-feather='download-cloud' class='icon-16'></i>", array("title" => app_lang("download")));

        $options .= js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete_file'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("initiates/delete_file"), "data-action" => "delete-confirmation"));
    ...
    function view_file($file_id = 0) {
        ...
        if ($file_info) {
            ...
            return $this->template->view("initiates/files/view", $view_data);
        } else {
            show_404();
        }
    }
    ...
    function contact_profile($contact_id = 0, $tab = "") {
        ...
        $view_data['lead_info'] = $this->Patrons_model->get_one($view_data['user_info']->client_id);
        $view_data['tab'] = clean_data($tab);
        if ($view_data['user_info']->user_type === "lead") {
            ...
            return $this->template->rander("initiates/contacts/view", $view_data);
        } else {
            show_404();
        }
    }
    ...
    function contacts($client_id) {
        if ($client_id) {
            ...
            return $this->template->view("initiates/contacts/index", $view_data);
        }
    }
    ...
    function add_new_contact_modal_form() {
        ...
        return $this->template->view('initiates/contacts/modal_form', $view_data);
    }
    ...
    function contact_general_info_tab($contact_id = 0) {
        if ($contact_id) {
            ...
            return $this->template->view('initiates/contacts/contact_general_info_tab', $view_data);
        }
    }
    ...
    function company_info_tab($client_id = 0) {
        if ($client_id) {
            ...
            return $this->template->view('initiates/contacts/company_info_tab', $view_data);
        }
    }
    ...
    private function _make_contact_row($data, $custom_fields) {
        ...
        $contact_link = anchor(get_uri("initiates/contact_profile/" . $data->id), $full_name . $primary_contact);
        ...
        $row_data[] = js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete_contact'), "class" => "delete", "data-id" => "$data->id", "data-action-url" => get_uri("initiates/delete_contact"), "data-action" => "delete"));
    ...
    function save_lead_status($id = 0) {
        ...
        $save_id = $this->Patrons_model->ci_save($data, $id);
    ...
    function all_leads_kanban() {
        ...
        return $this->template->rander("initiates/kanban/all_leads", $view_data);
    }
    ...
    function all_leads_kanban_data() {
        ...
        $view_data["leads"] = $this->Patrons_model->get_leads_kanban_details($options)->getResult();
        ...
        return $this->template->view('initiates/kanban/kanban_view', $view_data);
    }
    ...
    function save_lead_sort_and_status() {
        ...
        $this->Patrons_model->ci_save($data, $id);
    }
    ...
    function make_client_modal_form($lead_id = 0) {
        ...
        return $this->template->view('initiates/migration/modal_form', $view_data);
    }
    ...
    function save_as_client() {
        ...
        if ($client_id) {
            ...
            $client_info = $this->Patrons_model->get_details(array("id" => $client_id))->getRow();
            ...
            if (get_setting("disallow_duplicate_client_company_name") == "1" && $this->Patrons_model->is_duplicate_company_name($company_name, $client_id)) {
                echo json_encode(array("success" => false, 'message' => app_lang("account_already_exists_for_your_company_name")));
                exit();
            }

            $save_client_id = $this->Patrons_model->ci_save($data, $client_id);
            ...
            if ($save_client_id) {
                ...
                echo json_encode(array("success" => true, 'redirect_to' => get_uri("patrons/view/$save_client_id"), "message" => app_lang('record_saved')));
            } else {
                echo json_encode(array("success" => false, app_lang('error_occurred')));
            }
        }
    ...
    function proposals($client_id) {
        ...
        if ($client_id) {
            ...
            return $this->template->view("initiates/proposals/index", $view_data);
        }
    }
    ...
    function contracts($client_id) {
        if ($client_id) {
            ...
            return $this->template->view("initiates/contracts/index", $view_data);
        }
    }
    ...
    function tasks($client_id) {
        ...
        return $this->template->view("initiates/tasks/index", $view_data);
    }
    ...
    private function _save_a_row_of_excel_data($row_data) {
        ...
        //save lead data
        $saved_id = $this->Patrons_model->ci_save($lead_data);
        if (!$saved_id) {
            return false;
        }
    ...
    function converted_to_client_report() {
        ...
        return $this->template->rander("initiates/reports/converted_to_client", $view_data);
    }
    ...
    function converted_to_client_charts_data() {
        ...
        return $this->template->view("initiates/reports/converted_to_client_monthly_chart", $view_data);
    }
    ...
    private function _converted_to_client_chart_day_wise_data($options, $days_of_month) {
        ...
        $converted_result = $this->Patrons_model->get_converted_to_client_statistics($options)->getResult();
    ...
    function team_members_summary() {
        ...
        return $this->template->view("initiates/reports/team_members_summary", $view_data);
    }
    ...
    function team_members_summary_data() {
        ...
        $list_data = $this->Patrons_model->get_leads_team_members_summary($options)->getResult();
