...
class Budgets extends Security_Controller {
    ...
    function index() {
        ...
        return $this->template->rander("budgets/index", $view_data);
    }

    //get categories dropdown
    private function _get_categories_dropdown() {
        $categories = $this->Budget_categories_model->get_all_where(array("deleted" => 0), 0, 0, "title")->getResult();
    ...
    function yearly() {
        return $this->template->view("budgets/yearly_expenses");
    }
    ...
    function summary() {
        ...
        return $this->template->rander("budgets/reports/expenses_summary");
    }

    //load custom expenses list
    function custom() {
        return $this->template->view("budgets/custom_expenses");
    }

    //load the recurring view of expense list 
    function recurring() {
        return $this->template->view("budgets/recurring_expenses_list");
    }
    ...
    function modal_form() {
        ...
        $model_info = $this->Budgets_model->get_one($this->request->getPost('id'));
        $view_data['categories_dropdown'] = $this->Budget_categories_model->get_dropdown_list(array("title"));
        ...
        $view_data['projects_dropdown'] = array("0" => "-") + $this->Enterprises_model->get_dropdown_list(array("title"));
        ...
        return $this->template->view('budgets/modal_form', $view_data);
    }
    ...
    function save() {
        ...
        $expense_info = $this->Budgets_model->get_one($id);
        ...
        $save_id = $this->Budgets_model->ci_save($data, $id);
        if ($save_id) {
    ...
    function delete() {
        ...
        if ($this->Budgets_model->delete($id)) {
            //delete the files
    ...
    function list_data($recurring = false) {
        ...
        $list_data = $this->Budgets_model->get_details($options)->getResult();
    ...
    private function _row_data($id) {
        ...
        $data = $this->Budgets_model->get_details($options)->getRow();
    ...
    private function _make_row($data, $custom_fields) {
        ...
        if ($data->recurring_expense_id) {
            ...
            $description .= modal_anchor(get_uri("budgets/expense_details"), app_lang("original_expense"), array("title" => app_lang("expense_details"), "data-post-id" => $data->recurring_expense_id));
        }
        ...
        if ($data->files) {
            ...
            if (count($files)) {
                foreach ($files as $key => $value) {
                    ...
                    $file_download_link = anchor(get_uri("budgets/download_files/" . $data->id), "<i data-feather='download'></i>", array("title" => app_lang("download")));
                    $files_link .= js_anchor("<i data-feather='$link'></i>", array('title' => "", "data-toggle" => "app-modal", "data-sidebar" => "0", "class" => "float-start mr10", "title" => remove_file_prefix($file_name), "data-url" => get_uri("budgets/file_preview/" . $data->id . "/" . $key)));
                }
            }
        }
        ...
        $row_data = array(
            $data->expense_date,
            modal_anchor(get_uri("budgets/expense_details"), format_to_date($data->expense_date, false), array("title" => app_lang("expense_details"), "data-post-id" => $data->id, "data-modal-lg" => "1")),
            $data->category_title,
            $data->title,
            $description,
        ...
        $row_data[] = modal_anchor(get_uri("budgets/modal_form"), "<i data-feather='edit' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('edit_expense'), "data-post-id" => $data->id))
                . js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete_expense'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("budgets/delete"), "data-action" => "delete-confirmation"));
    ...
    function file_preview($id = "", $key = "") {
        if ($id) {
            ...
            return $this->template->view("budgets/file_preview", $view_data);
        } else {
            show_404();
        }
    }
    ...
    function yearly_chart() {
        return $this->template->view("budgets/reports/yearly_chart");
    }
    ...
    function yearly_chart_data() {
        ...
        if ($year) {
            $expenses = $this->Budgets_model->get_yearly_expenses_chart($year);
            $values = array();
    ...
    function income_vs_expenses() {
        ...
        return $this->template->rander("budgets/reports/income_vs_expenses_chart", $view_data);
    }
    ...
    function income_vs_expenses_chart_data() {
        ...
        if ($year) {
            $expenses_data = $this->Budgets_model->get_yearly_expenses_chart($year, $project_id);
            $payments_data = $this->Bill_payments_model->get_yearly_payments_chart($year, "", $project_id);
    ...
    function income_vs_expenses_summary() {
        $view_data["projects_dropdown"] = $this->_get_projects_dropdown_for_income_and_expenses();
        return $this->template->view("budgets/reports/income_vs_expenses_summary", $view_data);
    }
    ...
    function income_vs_expenses_summary_list_data() {
        ...
        if ($year) {
            $expenses_data = $this->Budgets_model->get_yearly_expenses_chart($year[0], $project_id);
            $payments_data = $this->Bill_payments_model->get_yearly_payments_chart($year[0], "", $project_id);
    ...
    function expense_details() {
        ...
        $info = $this->Budgets_model->get_details($options)->getRow();
        if (!$info) {
            show_404();
        }
        ...
        return $this->template->view("budgets/expense_details", $view_data);
    }
    ...
    function summary_list_data() {
        ...
        $list_data = $this->Budgets_model->get_summary_details($options)->getResult();
    ...
    function save_expense_from_excel_file() {
        ...
        foreach ($excel_file as $key => $value) { //rows
            ...
            //save expense data
            $expense_save_id = $this->Budgets_model->ci_save($expense_data);
            if (!$expense_save_id) {
                continue;
            }
    ...
    private function _get_category_id($category = "") {
        ...
        $existing_category = $this->Budget_categories_model->get_one_where(array("title" => $category, "deleted" => 0));
        if ($existing_category->id) {
            ...
            return $this->Budget_categories_model->ci_save($category_data);
        }
    }
    ...
    private function _get_client_id($client = "") {
        ...
        $existing_client = $this->Patrons_model->get_one_where(array("company_name" => $client, "is_lead" => 0, "deleted" => 0));
    ...
    function tasks($expense_id) {
        ...
        return $this->template->view("budgets/tasks/index", $view_data);
    }

    //load the expenses monthly summary view
    function monthly_summary() {
        return $this->template->view("budgets/reports/monthly_summary");
    }

    //load the expenses custom summary view
    function custom_summary() {
        return $this->template->view("budgets/reports/custom_summary");
    }

    //load the expenses category chart view
    function category_chart() {
        return $this->template->view("budgets/reports/category_chart_container");
    }
    ...
    function category_chart_view() {
        ...
        $categories_data = $this->Budgets_model->get_summary_details($options)->getResult();
        ...
        return $this->template->view("budgets/reports/category_chart", $view_data);
    }
