...
class Enterprises extends Security_Controller
{
    ...
    protected $Responsibility_priority_model;
    ...
    public function __construct()
    {
        ...
        $this->Responsibility_priority_model = model("App\Models\Responsibility_priority_model");
    }
    ...
    private function can_delete_projects($project_id = 0)
    {
        if ($this->login_user->user_type == "staff") {
            ...
            if ($project_id) {
                $project_info = $this->Enterprises_model->get_one($project_id);
    ...
    function index()
    {
        app_redirect("enterprises/all_projects");
    }
    ...
    function all_projects($status_id = 0)
    {
        ...
        if ($this->login_user->user_type === "staff") {
            ...
            return $this->template->rander("enterprises/index", $view_data);
        } else {
            ...
            return $this->template->rander("patrons/enterprises/index", $view_data);
        }
    }
    ...
    function modal_form()
    {
        ...
        $view_data['model_info'] = $this->Enterprises_model->get_one($project_id);
        if ($client_id) {
        ...
        return $this->template->view('enterprises/modal_form', $view_data);
    }
    ...
    private function _get_clients_dropdown_with_permission()
    {
        ...
        if ($this->login_user->is_admin || get_array_value($this->login_user->permissions, "client")) {
            ...
            $clients = $this->Patrons_model->get_details(array("show_own_clients_only_user_id" => $this->show_own_clients_only_user_id(), "client_groups" => $access_client->allowed_client_groups))->getResult();
    ...
    function save()
    {
        ...
        $save_id = $this->Enterprises_model->ci_save($data, $id);
        if ($save_id) {
            ...
            if (!$id) {
                ...
                if ($estimate_id) {
                    ...
                    $this->Evaluations_model->ci_save($data, $estimate_id);
                }

                //created from order? save the project id
                if ($order_id) {
                    $data = array("project_id" => $save_id);
                    $this->Arrangements_model->ci_save($data, $order_id);
                }
    ...
    function clone_project_modal_form()
    {
        ...
        $view_data['clients_dropdown'] = $this->Patrons_model->get_dropdown_list(array("company_name"), "id", array("is_lead" => 0));
        ...
        return $this->template->view('enterprises/clone_project_modal_form', $view_data);
    }
    ...
    function save_cloned_project()
    {
        ...
        //add new project
        $new_project_id = $this->Enterprises_model->ci_save($data);

        //old project info
        $old_project_info = $this->Enterprises_model->get_one($project_id);
        ...
        $tasks = $this->Responsibility_model->get_all_where(array("project_id" => $project_id, "deleted" => 0, "parent_task_id" => 0))->getResult();
        foreach ($tasks as $task) {
            ...
            //add new task
            $new_taks_id = $this->Responsibility_model->ci_save($task_data);
        ...
        $tasks = $this->Responsibility_model->get_all_tasks_where_have_dependency($project_id)->getResult();
        foreach ($tasks as $task) {
            if (array_key_exists($task->id, $task_ids)) {
                ...
                if ($task->blocked_by) {
                    ...
                    if ($new_blocked_by_tasks) {
                        ...
                        $this->Responsibility_model->ci_save($blocked_by_task_data, $task_ids[$task->id]);
                    }
                }
                ...
                if ($task->blocking) {
                    ...
                    if ($new_blocking_tasks) {
                        ...
                        $this->Responsibility_model->ci_save($blocking_task_data, $task_ids[$task->id]);
                    }
                }
            }
        }
    ...
    function delete()
    {
        ...
        if ($this->Enterprises_model->delete_project_and_sub_items($id)) {
    ...
    function list_data()
    {
        ...
        $list_data = $this->Enterprises_model->get_details($options)->getResult();
        $result = array();
    ...
    private function _row_data($id)
    {
        ...
        $data = $this->Enterprises_model->get_details($options)->getRow();
    ...
    private function _make_row($data, $custom_fields)
    {
        ...
        $title = anchor(get_uri("enterprises/view/" . $data->id), $data->title);
        if ($data->labels_list) {
        ...
        if ($this->can_edit_projects($data->id)) {
            $optoins .= modal_anchor(get_uri("enterprises/modal_form"), "<i data-feather='edit' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('edit_project'), "data-post-id" => $data->id));
        }

        if ($this->can_delete_projects($data->id)) {
            $optoins .= js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete_project'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("enterprises/delete"), "data-action" => "delete-confirmation"));
        }
        ...
        if ($data->company_name) {
            $client_name = anchor(get_uri("patrons/view/" . $data->client_id), $data->company_name);
        }

        $row_data = array(
            anchor(get_uri("enterprises/view/" . $data->id), $data->id),
            $title,
            $client_name,
            $price,
            $data->start_date,
    ...
    function view($project_id = 0, $tab = "")
    {
        ...
        return $this->template->rander("enterprises/details_view", $view_data);
    }
    ...
    private function _get_project_info_data($project_id)
    {
        ...
        $project_info = $this->Enterprises_model->get_details($options)->getRow();
    ...
    {
        $view_data["projects"] = $this->Enterprises_model->get_starred_projects($this->login_user->id)->getResult();
        return $this->template->view('enterprises/star/projects_list', $view_data);
    }
    ...
    function overview($project_id)
    {
        ...
        $view_data = $this->_get_project_info_data($project_id);
        $view_data["task_statuses"] = $this->Responsibility_model->get_task_statistics(array("project_id" => $project_id))->task_statuses;
        ...
        return $this->template->view('enterprises/overview', $view_data);
    }
    ...
    function add_remove_star($project_id, $type = "add")
    {
        if ($project_id) {
            ...
            if ($type === "add") {
                $this->Enterprises_model->add_remove_star($project_id, $this->login_user->id, $type = "add");
                return $this->template->view('enterprises/star/starred', $view_data);
            } else {
                $this->Enterprises_model->add_remove_star($project_id, $this->login_user->id, $type = "remove");
                return $this->template->view('enterprises/star/not_starred', $view_data);
            }
        }
    }
    ...
    function overview_for_client($project_id)
    {
        ...
        if ($this->login_user->user_type === "client") {
            ...
            return $this->template->view('enterprises/overview_for_client', $view_data);
        }
    }
    ...
    function project_member_modal_form()
    {
        ...
        return $this->template->view('enterprises/project_members/modal_form', $view_data);
    }
    ...
    private function _make_project_member_row($data, $can_send_message_to_client = false)
    {
        ...
        if (get_setting("module_message") && ($this->login_user->id != $data->user_id)) {
            $link = modal_anchor(get_uri("memos/modal_form/" . $data->user_id), "<i data-feather='mail' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('send_message')));
        }
        ...
        if ($this->can_add_remove_project_members()) {
            $delete_link = js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete_member'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("enterprises/delete_project_member"), "data-action" => "delete"));
    ...
    function stop_timer_modal_form($project_id)
    {
        ...
        if ($project_id) {
            ...
            $task_info = $this->Responsibility_model->get_details($options)->getRow();
            ...
            return $this->template->view('enterprises/timesheets/stop_timer_modal_form', $view_data);
        }
    }
    ...
    private function _get_timesheet_tasks_dropdown($project_id, $return_json = false)
    {
        ...
        $tasks = $this->Responsibility_model->get_details($options)->getResult();
    ...
    function timer($project_id, $timer_status = "start")
    {
        ...
        if ($timer_status === "start") {
            if ($this->request->getPost("task_timer")) {
                echo modal_anchor(get_uri("enterprises/stop_timer_modal_form/" . $project_id), "<i data-feather='clock' class='icon-16'></i> " . app_lang('stop_timer'), array("class" => "btn btn-danger", "title" => app_lang('stop_timer'), "data-post-task_id" => $task_id));
            } else {
                $view_data = $this->_get_project_info_data($project_id);
                return $this->template->view('enterprises/project_timer', $view_data);
            }
        } else {
    ...
    function timesheets($project_id)
    {
        ...
        return $this->template->view("enterprises/timesheets/index", $view_data);
    }
    ...
    function timelog_modal_form()
    {
        ...
        return $this->template->view('enterprises/timesheets/modal_form', $view_data);
    }
    ...
    private function _make_timesheet_row($data, $custom_fields)
    {
        ...
        $project_title = anchor(get_uri("enterprises/view/" . $data->project_id), $data->project_title);
        $task_title = modal_anchor(get_uri("responsibility/view"), $data->task_title, array("title" => app_lang('task_info') . " #$data->task_id", "data-post-id" => $data->task_id, "data-modal-lg" => "1"));

        $client_name = "-";
        if ($data->timesheet_client_company_name) {
            $client_name = anchor(get_uri("patrons/view/" . $data->timesheet_client_id), $data->timesheet_client_company_name);
        }
        ...
        $options = modal_anchor(get_uri("enterprises/timelog_modal_form"), "<i data-feather='edit' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('edit_timelog'), "data-post-id" => $data->id))
            . js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete_timelog'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("enterprises/delete_timelog"), "data-action" => "delete"));
    ...
    function timesheet_summary($project_id)
    {
        ...
        return $this->template->view("enterprises/timesheets/summary_list", $view_data);
    }
    ...
    {
        $projects = $this->Enterprises_model->get_dropdown_list(array("title"));
    ...
    private function _get_all_projects_dropdown_list_for_timesheets_filter()
    {
        ...
        $projects = $this->Enterprises_model->get_details($options)->getResult();
    ...
    function all_timesheets()
    {
        ...
        return $this->template->rander("enterprises/timesheets/all_timesheets", $view_data);
    }
    ...
    function all_timesheet_summary()
    {
        ...
        return $this->template->view("enterprises/timesheets/all_summary_list", $view_data);
    }
    ...
    function milestones($project_id)
    {
        ...
        return $this->template->view("enterprises/milestones/index", $view_data);
    }
    ...
    function milestone_modal_form()
    {
        ...
        return $this->template->view('enterprises/milestones/modal_form', $view_data);
    }
    ...
    private function _make_milestone_row($data)
    {
        ...
        $optoins = "";
        if ($this->can_edit_milestones()) {
            $optoins .= modal_anchor(get_uri("enterprises/milestone_modal_form"), "<i data-feather='edit' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('edit_milestone'), "data-post-id" => $data->id));
        }

        if ($this->can_delete_milestones()) {
            $optoins .= js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete_milestone'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("enterprises/delete_milestone"), "data-action" => "delete"));
        }
    ...
    function comments($project_id)
    {
        ...
        return $this->template->view("enterprises/comments/index", $view_data);
    }
    ...
    function save_comment()
    {
        ...
        if ($save_id) {
            ...
            if ($this->request->getPost("reload_list")) {
                $view_data['comments'] = $this->Project_comments_model->get_details($options)->getResult();
                $response_data = $this->template->view("enterprises/comments/comment_list", $view_data);
            }
    ...
    function view_comment_replies($comment_id)
    {
        ...
        return $this->template->view("enterprises/comments/reply_list", $view_data);
    }
    ...
    function comment_reply_form($comment_id, $type = "project", $type_id = 0)
    {
        ...
        return $this->template->view("enterprises/comments/reply_form", $view_data);
    }
    ...
    function files($project_id)
    {
        ...
        return $this->template->view("enterprises/files/index", $view_data);
    }
    ...
    function view_file($file_id = 0)
    {
        ...
        if ($file_info) {
            ...
            $view_data['current_url'] = get_uri("enterprises/view_file/" . $file_id);
            return $this->template->view("enterprises/files/view", $view_data);
        } else {
            show_404();
        }
    }
    ...
    function file_modal_form()
    {
        ...
        return $this->template->view('enterprises/files/modal_form', $view_data);
    }
    ...
    private function _make_file_row($data, $custom_fields)
    {
        ...
        $description = "<div class='float-start text-wrap'>" .
            js_anchor(remove_file_prefix($data->file_name), array('title' => "", "data-toggle" => "app-modal", "data-sidebar" => "1", "data-group" => "project-files", "data-url" => get_uri("enterprises/view_file/" . $data->id)));
        ...
        $options = anchor(get_uri("enterprises/download_file/" . $data->id), "<i data-feather='download-cloud' class='icon-16'></i>", array("title" => app_lang("download")));
        if ($this->can_add_files()) {
            $options .= modal_anchor(get_uri("enterprises/file_modal_form"), "<i data-feather='edit' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('edit_files'), "data-post-id" => $data->id));
        }
        if ($this->can_delete_files($data->uploaded_by)) {
            $options .= js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete_file'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("enterprises/delete_file"), "data-action" => "delete-confirmation"));
        }
    ...
    function notes($project_id)
    {
        ...
        $this->access_only_team_members();
        $view_data['project_id'] = $project_id;
        return $this->template->view("enterprises/notes/index", $view_data);
    }
    ...
    function history($offset = 0, $log_for = "", $log_for_id = "", $log_type = "", $log_type_id = "")
    {
        ...
        return $this->template->view("enterprises/history/index", $view_data);
    }
    ...
    function members($project_id = 0)
    {
        ...
        $this->access_only_team_members();
        $view_data['project_id'] = $project_id;
        return $this->template->view("enterprises/project_members/index", $view_data);
    }
    ...
    function payments($project_id)
    {
        ...
        $this->access_only_team_members();
        if ($project_id) {
            $view_data['project_info'] = $this->Enterprises_model->get_details(array("id" => $project_id))->getRow();
            $view_data['project_id'] = $project_id;
            return $this->template->view("enterprises/payments/index", $view_data);
        }
    }
    ...
    function invoices($project_id, $client_id = 0)
    {
        ...
        if ($project_id) {
            ...
            return $this->template->view("enterprises/invoices/index", $view_data);
        }
    }
    ...
    function expenses($project_id)
    {
        ...
        if ($project_id) {
            ...
            return $this->template->view("enterprises/expenses/index", $view_data);
        }
    }
    ...
    function change_status($project_id, $status_id)
    {
        if ($project_id && $this->can_edit_projects() && $status_id) {
            ...
            $save_id = $this->Enterprises_model->ci_save($status_data, $project_id);
    ...
    function settings_modal_form()
    {
        ...
        $task_statuses = $this->Responsibility_status_model->get_details()->getResult();
        ...
        $view_data["project_info"] = $this->Enterprises_model->get_one($project_id);

        return $this->template->view('enterprises/settings/modal_form', $view_data);
    }
    ...
    function get_projects_of_selected_client_for_filter()
    {
        ...
        $client_id = $this->request->getPost("client_id");
        if ($client_id) {
            $projects = $this->Enterprises_model->get_all_where(array("client_id" => $client_id, "deleted" => 0), 0, 0, "title")->getResult();
            $projects_dropdown = array(array("id" => "", "text" => "- " . app_lang("project") . " -"));
            foreach ($projects as $project) {
                $projects_dropdown[] = array("id" => $project->id, "text" => $project->title);
            }
    ...
    private function _get_clients_dropdown()
    {
        ...
        $clients = $this->Patrons_model->get_dropdown_list(array("company_name"), "id", array("is_lead" => 0));
    ...
    function timesheet_chart($project_id = 0)
    {
        ...
        return $this->template->view("enterprises/timesheets/timesheet_chart", $view_data);
    }
    ...
    function like_comment($comment_id = 0)
    {
        if ($comment_id) {
            ...
            return $this->template->view("enterprises/comments/like_comment", array("comment" => $comment));
        }
    }
    ...
    function contracts($project_id)
    {
        ...
        if ($project_id) {
            ...
            return $this->template->view("enterprises/contracts/index", $view_data);
        }
    }
    ...
    function pin_comment($comment_id = 0)
    {
        if ($comment_id) {
            ...
            if ($save_id) {
                ...
                $save_data = $this->template->view("enterprises/comments/pinned_comments", array("pinned_comments" => $pinned_comments));
    ...
    function tickets($project_id)
    {
        ...
        if ($project_id) {
            ...
            return $this->template->view("enterprises/tickets/index", $view_data);
        }
    }
    ...
    function file_category($project_id = 0)
    {
        ...
        $view_data["project_id"] = $project_id;
        $view_data['can_add_files'] = $this->can_add_files();
        return $this->template->view("enterprises/files/category/index", $view_data);
    }
    ...
    private function _make_file_category_row($data, $project_id = 0)
    {
        $options = "";
        if ($this->can_add_files()) {
            $options .= modal_anchor(get_uri("enterprises/file_category_modal_form"), "<i data-feather='edit' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('edit_category'), "data-post-id" => $data->id, "data-post-project_id" => $project_id));
        }

        if ($this->can_delete_files()) {
            $options .= js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete_category'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("enterprises/delete_file_category"), "data-action" => "delete", "data-post-project_id" => $project_id));
        }
    ...
    function file_category_modal_form()
    {
        ...
        $view_data['project_id'] = $project_id;
        return $this->template->view('enterprises/files/category/modal_form', $view_data);
    }
    ...
    function show_my_open_timers()
    {
        ...
        return $this->template->view("enterprises/open_timers", $view_data);
    }
    ...
    function task_timesheet($task_id, $project_id)
    {
        ...
        return $this->template->view("responsibility/task_timesheet", $view_data);
    }
    ...
    {
        if ($task_id) {
            app_redirect("responsibility/view/" . $task_id);
        }
    }
    ...
    function team_members_summary()
    {
        ...
        return $this->template->rander("enterprises/reports/team_members_summary", $view_data);
    }
    ...
    function team_members_summary_data()
    {
        ...
        $list_data = $this->Enterprises_model->get_team_members_summary($options)->getResult();
        $result = array();
        foreach ($list_data as $data) {
    ...
    function clients_summary()
    {
        ...
        return $this->template->view("enterprises/reports/clints_summary", $view_data);
    }
    ...
    function clients_summary_data()
    {
        ...
        $list_data = $this->Enterprises_model->get_clients_summary($options)->getResult();
        $result = array();
        foreach ($list_data as $data) {
    ...
    private function _make_clients_summary_row($data)
    {
        ...
        $client_name = anchor(get_uri("patrons/view/" . $data->client_id), $data->client_name);
    ...
    private function _init_required_data_before_starting_import()
    {
        ...
        $clients = $this->Patrons_model->get_clients_id_and_name()->getResult();
    ...
    private function _save_a_row_of_excel_data($row_data)
    {
        ...
        //save project data
        $saved_id = $this->Enterprises_model->ci_save($project_data);
        if (!$saved_id) {
            return false;
        }
