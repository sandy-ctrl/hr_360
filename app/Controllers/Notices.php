...
class Notices extends Security_Controller {
    ...
    function index() {
        ...
        return $this->template->rander("notices/index", $view_data);
    }
    ...
    function form($id = 0) {
        ...
        $view_data['model_info'] = $this->Notices_model->get_one($id);
        ...
        return $this->template->rander('notices/modal_form', $view_data);
    }
    ...
    function view($id = "") {
        if ($id) {
            ...
            $announcement = $this->Notices_model->get_details($options)->getRow();
            if ($announcement) {
                $view_data['announcement'] = $announcement;

                //mark the announcement as read for loged in user
                $this->Notices_model->mark_as_read($id, $this->login_user->id);
                return $this->template->rander("notices/view", $view_data);
            }
        }
    ...
    private function _prepare_access_options($options = array()) {
        ...
        if ($this->login_user->user_type === "client") {
            $group_ids = $this->Patrons_model->get_one($this->login_user->client_id)->group_ids;
            if ($group_ids) {
    ...
    function save() {
        ...
        if ($id) {
            $expense_info = $this->Notices_model->get_one($id);
        ...
        $save_id = $this->Notices_model->ci_save($data, $id);
        ...
        if ($save_id) {
            ...
            echo json_encode(array("success" => true, "recirect_to" => get_uri("notices/form/" . $save_id), 'id' => $save_id, 'message' => app_lang('record_saved')));
        } else {
    ...
    function download_announcement_files($id = 0) {
        ...
        $info = $this->Notices_model->get_details($options)->getRow();
    ...
    function delete() {
        ...
        if ($this->request->getPost('undo')) {
            if ($this->Notices_model->delete($id, true)) {
                echo json_encode(array("success" => true, "data" => $this->_row_data($id), "message" => app_lang('record_undone')));
            } else {
                echo json_encode(array("success" => false, app_lang('error_occurred')));
            }
        } else {
            if ($this->Notices_model->delete($id)) {
    ...
    function list_data() {
        ...
        $list_data = $this->Notices_model->get_details($options)->getResult();
        $result = array();
    ...
    private function _row_data($id) {
        $options = array("id" => $id);
        $data = $this->Notices_model->get_details($options)->getRow();
    ...
    private function _make_row($data) {
        ...
        $option = "";
        if ($this->access_type === "all") {
            $option = anchor(get_uri("notices/form/" . $data->id), "<i data-feather='edit' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('edit_announcement')))
                    . js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete_announcement'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("notices/delete"), "data-action" => "delete"));
        }
        return array(
            anchor(get_uri("notices/view/" . $data->id), $data->title, array("class" => "", "title" => app_lang('view'))),
