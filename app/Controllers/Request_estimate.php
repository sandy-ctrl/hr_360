...
class Request_estimate extends App_Controller {
    ...
    function index() {
        ...
        $view_data["estimate_forms"] = $this->Evaluation_forms_model->get_all_where(array("status" => "active", "public" => "1", "deleted" => 0))->getResult();
    ...
    function form($id = 0, $embedded = 0) {
        ...
        $model_info = $this->Evaluation_forms_model->get_one_where(array("id" => $id, "public" => "1", "status" => "active", "deleted" => 0));
    ...
    function save_estimate_request() {
        ...
        if ($user_info->id) {
            ...
            $lead_id = $this->Patrons_model->ci_save($leads_data);
    ...
    private function _make_form_field_row($data) {
        ...
        return array(
            ...
            . js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("evaluation_requests/estimate_form_field_delete"), "data-action" => "delete"))
        );
    }
