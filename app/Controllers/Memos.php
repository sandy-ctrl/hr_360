...
class Memos extends Security_Controller {
    ...
    function modal_form($user_id = 0) {
        ...
        if ($user_id) {
            ...
            $users = $this->Memos_model->get_users_for_messaging($this->get_user_options_for_query())->getResult();
        ...
        return $this->template->view('memos/modal_form', $view_data);
    }
    ...
    function inbox($auto_select_index = "") {
        ...
        return $this->template->rander("memos/index", $view_data);
    }
    ...
    function list_data($mode = "inbox") {
        ...
        $list_data = $this->Memos_model->get_list($options)->getResult();
    ...
    function view($message_id = 0, $mode = "", $reply = 0) {
        ...
        $view_data["message_info"] = $this->Memos_model->get_details($options)->row;
        ...
        $this->Memos_model->set_message_status_as_read($view_data["message_info"]->id, $this->login_user->id);

        $replies_options = array("message_id" => $message_id, "user_id" => $this->login_user->id, "limit" => 4);
        $messages = $this->Memos_model->get_details($replies_options);
        ...
        echo json_encode(array("success" => true, "data" => $this->template->view("memos/view", $view_data), "message_id" => $message_id));
    }
    ...
    function send_message() {
        ...
        $save_id = $this->Memos_model->ci_save($message_data);
    ...
    function reply($is_chat = 0) {
        ...
        $message_info = $this->Memos_model->get_one($message_id);
        ...
        if ($message_info->id) {
            ...
            if ($save_id) {
                ...
                $this->Memos_model->clear_deleted_status($message_id);
                ...
                if ($is_chat) {
                    ...
                    $view_data['reply_info'] = $this->Memos_model->get_details($options)->row;
                    echo json_encode(array("success" => true, 'message' => app_lang('message_sent'), 'data' => $this->template->view("memos/reply_row", $view_data)));
                }
    ...
    private function _load_more_messages($message_id, $last_message_id, $top_message_id) {
        ...
        $view_data["replies"] = $this->Memos_model->get_details($replies_options)->result;
        $view_data["message_id"] = $message_id;

        $this->Memos_model->set_message_status_as_read($message_id, $this->login_user->id);

        return $this->template->view("memos/reply_rows", $view_data);
    }
    ...
    function count_notifications() {
        ...
        $notifiations = $this->Memos_model->count_notifications($this->login_user->id, $this->login_user->message_checked_at, $this->request->getPost("active_message_id"), $this->get_allowed_user_ids());
    ...
    function get_notifications() {
        ...
        $view_data['notifications'] = $this->Memos_model->get_list($options)->getResult();
        echo json_encode(array("success" => true, 'notification_list' => $this->template->view("memos/notifications", $view_data)));
    }
    ...
    function download_message_files($message_id = "") {
        validate_numeric_value($message_id);
        $model_info = $this->Memos_model->get_one($message_id);
    ...
    function delete_my_messages($id = 0) {
        ...
        $this->Memos_model->delete_messages_for_user($id, $this->login_user->id);
    }
    ...
    function chat_list() {
        ...
        $view_data['messages'] = $this->Memos_model->get_chat_list($options)->getResult();

        return $this->template->view("memos/chat/tabs", $view_data);
    }

    function users_list($type) {
        $view_data["users"] = $this->Memos_model->get_users_for_messaging($this->get_user_options_for_query($type))->getResult();
        ...
        return $this->template->view("memos/chat/team_members", $view_data);
    }
    ...
    function view_chat() {
        ...
        if ($this->request->getPost("is_first_load") == "1") {
            $view_data["first_message"] = $this->Memos_model->get_details(array("id" => $message_id, "user_id" => $this->login_user->id))->row;
            echo $this->template->view("memos/chat/message_title", $view_data);
        }
    ...
    private function _load_messages($message_id, $last_message_id, $top_message_id, $another_user_id = "") {
        ...
        return $this->template->view("memos/chat/message_items", $view_data);
    }
    ...
    function get_active_chat() {
        ...
        $view_data["message_id"] = $message_id;
        return $this->template->view("memos/chat/active_chat", $view_data);
    }
    ...
    function get_chatlist_of_user() {
        ...
        return $this->template->view("memos/chat/get_chatlist_of_user", $view_data);
    }
